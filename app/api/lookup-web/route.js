...
import { webSearch } from '@/lib/agent/web-search.mjs';
...
export async function GET(request) {
  ...
  if (q.length < 3) {
    return NextResponse.json({ ok: false, results: [], reason: 'Type a little more first.' }, { headers: noStore });
  }
  ...
  if (!apiKey || !model) {
    return NextResponse.json({ ok: false, results: [], reason: 'Web search is not set up on this server.' }, { headers: noStore });
  }

  try {
    // Steered at UK health services, since that is what is being looked up.
    const { ok, results, reason } = await webSearch({
      apiKey,
      model,
      query: `${q} — UK NHS or healthcare service: phone number, address and contact details`,
    });
    return NextResponse.json({ ok, results: (results || []).slice(0, 5), reason: reason || '' }, { headers: noStore });
  } catch (e) {
    return NextResponse.json({ ok: false, results: [], reason: 'Web search is unavailable.' }, { headers: noStore });
  }
}
