...
// The icon says which kind of message it is, so it stays — but bare, in the
// type's colour, rather than on a tinted badge.
const TYPE = {
  ...
  return (
    <div className="riva-notify-host">
      {items.map((it) => {
        ...
        return (
          <div key={it.id} className="riva-notify" style={s('display:flex;align-items:flex-start;gap:10px;background:#fff;border:1px solid #d8dde0;border-radius:12px;padding:13px 14px;box-shadow:0 6px 24px rgba(33,43,50,.14);')}>
            <span style={s('flex:none;display:flex;margin-top:2px;color:' + t.accent + ';')}><Svg w={16} sw={2.2}>{t.icon}</Svg></span>
