  ...
  return (
    <div className="riva-contact-results" style={s('animation:rivaAnswerIn .3s cubic-bezier(.2,.7,.3,1) both;')}>
      <div style={s('display:flex;align-items:center;gap:12px;margin:8px 0 0;')}>
        <span style={s('flex:none;display:flex;color:#e0554f;')}><Svg w={22} sw={2.2}>{Icons.phone}</Svg></span>
        <h1 style={s('flex:1;min-width:0;font-size:28px;font-weight:700;letter-spacing:-0.02em;margin:0;')}>Contacts</h1>
      </div>
      <div aria-live="polite" style={s('display:flex;flex-wrap:wrap;align-items:center;gap:6px 14px;margin:8px 0 18px;font-size:14px;color:#9a9aa3;')}>
      ...
      {v.contacts.length === 0 && (
        <p style={s('margin:0;font-size:15px;color:#9a9aa3;')}>
          {v.contactQuery
        ...
        {v.contacts.map((r, i) => (
          <React.Fragment key={r.key}>
            ...
            {r.group !== (v.contacts[i - 1] || {}).group && (
              <div style={s('padding:' + (i ? '22px' : '0') + ' 0 8px;font-size:11.5px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#63636c;')}>
                {r.group}
              </div>
            )}
            <div role="option" aria-selected={r.isSelected} className="riva-contact-row"
              style={s('display:flex;align-items:center;gap:14px;padding:11px 12px;border-radius:12px;border:1px solid ' + (r.isSelected ? '#e0554f;background:#221a1a;' : '#26262a;background:#141416;') + 'margin-bottom:6px;')}>
              <div style={s('flex:1;min-width:0;display:flex;flex-direction:column;gap:3px;')}>
                ...
                {r.detail && (
                  <span style={s('font-size:13px;color:#9a9aa3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;')}>{r.detail}</span>
                )}
                {r.emails.length > 0 && (
                  ...
                  <span key={p.tel + p.display} style={s('display:inline-flex;align-items:center;gap:2px;')}>
                    ...
                    <Hover tag="button" type="button" onClick={p.onCopy}
                      ...
                      base={'display:inline-flex;align-items:center;gap:7px;border-radius:999px 7px 7px 999px;padding:7px 10px 7px 13px;font:inherit;font-size:15px;font-weight:700;cursor:pointer;border:1px solid '
                        + (p.isCopied ? '#56c98a;background:#12211a;color:#7fdcaa;' : '#2a2a2e;background:#151518;color:#e0554f;')}
      ...
      {v.contactRegisterSearching && v.contacts.length > 0 && (
        <p style={s('margin:10px 4px 0;font-size:14px;color:#63636c;')}>Searching the register…</p>
      )}
    </div>
  );
}
