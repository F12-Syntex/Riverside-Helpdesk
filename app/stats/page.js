...
/* ------------------------------------------------------------------ *
 * /stats — the activity audit log.
 *
 * Everything done in the app: the pages opened, the questions asked, the
 * buttons pressed, the requests made and anything that failed — grouped
 * by the machine it was done on rather than by IP address, because every
 * machine in the building shares one public address and a laptop changes
 * address between the car park and the consulting room. See
 * lib/audit/machine.js for why that matters and what is stored instead.
 *
 * Deliberately not linked from the tools page or the menu. It is reached
 * by typing the address, and it is listed at /index with that noted.
 * ------------------------------------------------------------------ */

import React from 'react';
import { s, Hover, Svg, Icons } from '../_components/ui';
import AppHeader from '../_components/AppHeader';
import ActivityChart from '../_components/stats/ActivityChart';
...
const CARD = 'background:#fff;border:1px solid #d8e1e5;border-radius:12px;';

// A headline number and what it counts. No plot, so nothing to hover.
function Tile({ value, caption }) {
  return (
    <div style={s(CARD + 'padding:16px 18px;')}>
      <div style={s('font-size:30px;font-weight:800;letter-spacing:-0.02em;color:#212b32;line-height:1.1;')}>{value}</div>
...
/**
 * One machine. Selecting it filters the whole page to what was done there.
 *
...
// A plain list of "thing — count", for the top pages and tools.
function Ranked({ title, rows, labelKey, empty }) {
    ...
    <div style={s(CARD + 'padding:18px 20px;')}>
      ...
      {rows.length ? (
        <ol style={s('list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:9px;')}>
          {rows.map((row) => (
            <li key={row[labelKey] + (row.path || '')}>
              <div style={s('display:flex;justify-content:space-between;gap:12px;font-size:14.5px;color:#212b32;')}>
                <span style={s('min-width:0;overflow-wrap:anywhere;')}>{row[labelKey]}</span>
...
export default function StatsPage() {
  ...
  return (
    <div style={s('min-height:100vh;background:#f0f4f5;display:flex;flex-direction:column;')}>
      <AppHeader subtitle="Audit log" />

      <main style={s('flex:1;width:100%;max-width:1100px;margin:0 auto;padding:36px 24px 64px;')}>
        <h1 style={s('font-size:30px;margin:0 0 6px;letter-spacing:-0.02em;')}>Activity audit log</h1>
        <p style={s('font-size:16.5px;color:#4c6272;margin:0 0 6px;line-height:1.55;max-width:70ch;')}>
          Every page opened, question asked, action taken and request that failed &mdash; grouped by the machine it was
          done on. Machines are identified by an id the browser generates and keeps, not by IP address, so a machine
          stays one machine when it moves between the surgery&rsquo;s network and a home one.
        </p>
        <p style={s('font-size:14px;color:#768692;margin:0 0 24px;line-height:1.5;max-width:70ch;')}>
        ...
        {/* Filters, in one row above everything they filter. */}
        ...
        {selected && (
          <div style={s('display:flex;flex-wrap:wrap;align-items:center;gap:10px;margin-bottom:18px;padding:11px 15px;background:#e8f1f8;border:1px solid #aac7e0;border-radius:10px;font-size:15px;color:#00437e;')}>
            ...
            <Hover tag="button" type="button" onClick={() => setMachineId('')}
        ...
        <p style={s('font-size:15px;color:#4c6272;margin:0 0 14px;')}>
          {machines.length
            ? 'Every machine the app has been opened on. Give one a name and it is used everywhere in the log.'
