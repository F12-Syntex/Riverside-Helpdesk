...
export default function KnowledgePage() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const [a, b] = await Promise.all([fetch(`/api/knowledge?kind=${encodeURIComponent(kind)}&q=${encodeURIComponent(query)}`).then((r) => r.json()), fetch('/api/knowledge/conflicts').then((r) => r.json())]);
      setEntries(a.entries || []); setConflicts(b.conflicts || []);
  ...
  const sync = async () => {
    ...
    try { const r = await fetch('/api/knowledge/sync', { method: 'POST' }); const x = await r.json(); if (!r.ok) throw new Error(x.error); setStatus(`Migrated ${x.summary.documents} documents, ${x.summary.notes} notes and ${x.summary.contacts} contacts${x.summary.errors.length ? `; ${x.summary.errors.length} items need attention` : ''}.`); await load(); }
    catch (e) { setStatus(`Migration failed: ${e.message}`); }
  };
  const analyse = async () => {
    setStatus('Analysing sources and comparing claims…'); let total = 0;
    try {
      for (let batch = 0; batch < 50; batch++) { const r = await fetch('/api/knowledge/analyse', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }); const x = await r.json(); if (!r.ok) throw new Error(x.error); total += (x.processed || []).length; setStatus(`Analysed ${total} sources…`); if (!x.remaining) break; }
  ...
  const editById = (id) => { const found = entries.find((e) => e.id === id); if (found) setEditor(found); else { setTab('library'); setQuery(''); setStatus('Find and edit the highlighted source in the library.'); } };
