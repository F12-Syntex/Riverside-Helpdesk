...
export default function KnowledgePage() {
  ...
  return <div style={s('min-height:100vh;background:#f0f4f5;')}>
    ...
    <main style={s('max-width:1120px;margin:0 auto;padding:34px 22px 70px;')}>
      ...
      {status && <div style={s('background:#e8f1f8;border-radius:8px;padding:12px 15px;margin-bottom:20px;')}>{status}</div>}
      ...
      </> : <div>{!conflicts.length ? <div style={s('background:#fff;border:1px solid #d8dde0;border-radius:10px;padding:28px;')}><h2 style={s('margin:0 0 6px;')}>No open contradictions</h2><p style={s('margin:0;color:#4c6272;')}>New or changed sources are checked automatically.</p></div> : conflicts.map((c) => <article key={c.id} style={s('background:#fff;border-radius:8px;padding:20px;margin-bottom:14px;')}><h2 style={s('font-size:19px;margin:0 0 6px;')}>{c.subject}: {c.predicate}</h2><p style={s('margin:0 0 13px;color:#4c6272;line-height:1.45;')}>{c.reason} · {Math.round(Number(c.confidence || 0) * 100)}% confidence</p><div style={s('display:grid;grid-template-columns:1fr 1fr;gap:12px;')}><div style={s('background:#f0f4f5;padding:13px;border-radius:6px;')}><strong>{c.titleA}</strong><p style={s('margin:7px 0;')}>{c.valueA}</p>{c.quoteA && <blockquote style={s('margin:7px 0 11px;padding-left:9px;color:#4c6272;')}>{c.quoteA}</blockquote>}<button onClick={() => editById(c.entryA)} style={s(BTN + 'padding:7px 10px;background:#fff;color:#005eb8;')}>Edit this source</button></div><div style={s('background:#f0f4f5;padding:13px;border-radius:6px;')}><strong>{c.titleB}</strong><p style={s('margin:7px 0;')}>{c.valueB}</p>{c.quoteB && <blockquote style={s('margin:7px 0 11px;padding-left:9px;color:#4c6272;')}>{c.quoteB}</blockquote>}<button onClick={() => editById(c.entryB)} style={s(BTN + 'padding:7px 10px;background:#fff;color:#005eb8;')}>Edit this source</button></div></div><div style={s('display:flex;gap:10px;margin-top:13px;')}><button onClick={() => resolve(c.id, 'resolved')} style={s(BTN + 'background:#007f3b;color:#fff;')}>Mark resolved</button><button onClick={() => resolve(c.id, 'ignored')} style={s(BTN + 'background:#e8edee;color:#212b32;')}>Not a real conflict</button></div></article>)}</div>}
    </main>
