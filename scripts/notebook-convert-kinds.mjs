...
//   - A page is converted only if it comes out COMPLETE. A typed note that is
//     missing a required field is a draft, and a draft is never served, so
//     converting it would take a working page out of the answers. Those stay
//     notes and are listed for a person to fill in.
...
//   node scripts/notebook-convert-kinds.mjs . --apply      # applies the plan
...
async function plan() {
  const rows = await sql`SELECT id, parent_id AS "parentId", title, body, is_section AS "isSection", kind, fields, status, output_tag AS "tag" FROM notes`;
  ...
  for (const { r, kind, def, folder } of out) {
    ...
    results.push({ ...base, fields, dropped, issues: issues.map((i) => i.message),
      action: issues.length ? 'keep-note' : 'convert', status: statusFor(kind, fields) });
...
function report(results) {
  for (const action of ['convert', 'keep-note', 'skip']) {
...
if (!APPLY) {
  ...
  const todo = saved.results.filter((x) => x.action === 'convert');
