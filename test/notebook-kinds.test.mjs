...
// incomplete card is a draft whose values are never served, and the values are written
...
test('a draft answers from its prose only, and a card with no prose still is served', () => {
  const notes = [
    ...
    { id: 3, parentId: 1, title: 'Half written', body: 'notes', kind: 'ersReferral', fields: { specialty: 'Dermatology' }, status: 'draft' },
    { id: 4, parentId: 1, title: 'Prose', body: 'Do the thing.', kind: 'note', status: 'live' },
    { id: 5, parentId: 1, title: 'Empty draft', body: '', kind: 'ersReferral', fields: { specialty: 'ENT' }, status: 'draft' },
  ];
  const sources = buildFullNotebookSources(notes);
  const titles = sources.map((s) => s.docTitle);
  assert.deepEqual(titles, [
    'Notebook: Referrals / Audiology', 'Notebook: Referrals / Half written', 'Notebook: Referrals / Prose',
  ]);
  ...
  // A draft keeps answering from what was written on it, and its half-filled
  // values never reach the reader.
  assert.equal(sources[1].text, 'notes');
  assert.equal(sources[1].status, 'draft');
  // A plain page is its page, untouched.
  assert.equal(sources[2].text, 'Do the thing.');
});
