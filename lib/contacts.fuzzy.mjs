...
export function prepareContacts(entries) {
  return (entries || []).map((entry) => ({
    ...
    // What the entry is also called. The sheet names things the way the person
    // who wrote it says them, and the person searching says it their own way:
    // "the surgery" has to find "The Riverside Practice", and no amount of
    // fuzziness on the label alone can do that.
    aliasesLc: (entry.aliases || []).map((a) => String(a).toLowerCase()),
...
// match on the name of the thing itself. An alias is another name for the
// entry, so it ranks above an address and below the label.
...
function scoreTerm(row, term) {
  ...
  for (const alias of row.aliasesLc) {
    const hit = bestHit(alias, term);
    if (hit) score = Math.max(score, hit.score * ALIAS_WEIGHT);
  }
