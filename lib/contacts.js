...
import { prepareContacts, searchPrepared } from './contacts.fuzzy.mjs';
...
export function matchContactsIn(entries, query, limit = 5) {
  ...
  for (const e of entries) {
    const hay = (e.label + ' ' + (e.aliases || []).join(' ') + ' ' + (e.emails || []).join(' ')).toLowerCase();
    const seen = new Set();
    let score = 0;
...
// ---- the directory, browsed ------------------------------------------------
// The matcher above answers a typed question; the two below are for browsing
// the whole directory in the Contacts sheet, where the rules are different: an
// empty query means "show me everything", and a near-miss should still find
// the entry (lib/contacts.fuzzy.mjs).

const PREPARED = prepareContacts(DATA);

/** Every entry, in the order the file holds them. */
export function allContacts() {
  return DATA;
}

/** Fuzzy search for the contacts page: [{ entry, score, indices }], best first. */
export function searchContacts(query, limit = 0) {
  return searchPrepared(PREPARED, query, limit);
}
...
// The separator class includes dots, non-breaking spaces and tabs as well as the
// usual spaces/brackets/slashes/hyphens, so a number written "0300.123.4567" or
// with an nbsp is still caught and never slips past redaction.
