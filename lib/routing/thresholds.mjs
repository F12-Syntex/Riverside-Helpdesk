...
// Starting values. hitCos is deliberately conservative: a wrong page rendered
...
//
// minMargin is a COSINE gap between the best page and the runner-up (see
// ./decision.mjs), not a gap between fused scores. 0.05 is the starting guess:
// two pages within 0.05 cosine of each other are close enough that asking is
// cheaper than choosing. Like the other two it is a guess, and the only honest
// way to set it is against the golden set.
