...
//
// EVERYWHERE THAT ROW WOULD OTHERWISE BE. The question log was the only place
// a question was stored when this was written, and it is not any more: a tap
// on a "which did you mean?" card teaches the router by storing the wording as
// a trigger phrase (lib/routing/triggers.mjs). That is the same text under
// another name, so POST /api/routing/learn checks this too. Anything else that
// comes to keep question text has to check it as well — the switch is a
// promise about the text, not about one table.
