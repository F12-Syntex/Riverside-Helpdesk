...
import { isCollectableQuestion, unansweredReason } from './gaps.mjs';
import { noteUnansweredQuestion } from './open.js';
...
const MAX = { question: 2000, answer: 20000, source: 400, template: 60, model: 120, error: 300, provenance: 40000 };
const MAX_ROWS = 200;
 ...
 * @param {object} [turn.provenance] why the reader saw what they saw — see ./provenance.mjs
 */
export async function recordQuestion(turn = {}) {
  ...
  // Serialised here rather than by the driver, so a provenance object that
  // somehow cannot be stringified costs its own field and not the whole row.
  let provenance = '{}';
  try {
    const json = JSON.stringify(turn.provenance || {});
    if (json && json.length <= MAX.provenance) provenance = json;
  } catch (e) { /* the turn is worth more than its footnotes */ }
  ...
  try {
    ...
    await sql`
      INSERT INTO question_log
        (turn_id, machine_id, question, outcome, template, source, answer, model, duration_ms, images, attachments, error, provenance)
      VALUES (
  ...
  // A TURN THAT COULD NOT BE ANSWERED IS A QUESTION SOMEBODY STILL HAS.
  //
  // Read off the row that was just written rather than signalled from the
  // answer path, so every route that ends a turn is covered by construction —
  // including the ones added after this was written. The list it feeds is
  // /questions; the rules are in ./gaps.mjs.
  //
  // It hangs off the log deliberately, which means the machine that has
  // switched logging off contributes nothing here either: recordQuestion is
  // not called at all in that case, and the switch is a promise about the
  // text, not about one table (see ./opt-out.mjs).
  const reason = unansweredReason({ outcome, template: turn.template, answer: turn.answer });
  if (reason && isCollectableQuestion(question)) {
    await noteUnansweredQuestion({
      question,
      reason,
      turnId: cut(turn.turnId, 40),
      machineId,
    });
  }
}
...
/**
 * Record that somebody closed one item on the unresolved items panel.
 *
 * PER ITEM, NOT PER PANEL. The receptionist's real action is "I have sorted the
 * ramipril", and a single per-panel dismissal records only that somebody
 * clicked once — which is the same row whether they dealt with four items or
 * none. Per item is also what makes this log usable as an eval set later: it
 * says which decomposed requests actually needed doing.
 *
 * It does NOT hard-block anything. A receptionist who cannot close a panel
 * learns to ignore panels, and an ignored panel is worse than no panel — so
 * this records the decision and never gates the interaction on it.
 *
 * Appends rather than replaces, so two presses on one turn both survive, and
 * never throws.
 */
export async function recordDismissal({ turnId = '', itemId = '', label = '', machineId = '' } = {}) {
  const turn = cut(turnId, 40);
  const item = cut(itemId, 20);
  if (!turn || !item) return;

  const entry = JSON.stringify({
    item,
    label: cut(label, 200),
    machineId: isMachineId(machineId) ? machineId : '',
  ...
  try {
    ...
    await sql`
      UPDATE question_log
         SET dismissed = dismissed || ${entry}::jsonb
       WHERE turn_id = ${turn}
         AND NOT (dismissed @> ${'[{"item":"' + item + '"}]'}::jsonb)
    `;
  } catch (e) {
    console.warn('[questions] could not record a dismissal:', String(e).slice(0, 200));
  }
}
...
export async function listQuestions(options = {}) {
  ...
  return sql`
    SELECT
      ...
      q.images, q.attachments, q.error, q.at, q.provenance, q.dismissed,
...
/**
 * What the log says about each Notebook page: how often an answer stood on
 * it, how often that answer was marked wrong, and how often the practice's
 * notes were found wanting.
 *
 * `source` holds the pages an answer was built from as "Notebook: A / B / C"
 * strings joined by " · " — the same string the assistant cites, so it is the
 * join key for the Notebook map. Split with unnest so a two-page answer counts
 * for both pages. The verdict join is the one summariseQuestions uses.
 *
 * @returns {Promise<Record<string, {asked:number, bad:number, flagged:number, lastAsked:string|null}>>}
 */
export async function summariseBySource({ since = null } = {}) {
  await Promise.all([ensureQuestionLogSchema(), ensureFeedbackSchema()]);
  const sql = getSql();
  const sinceArg = since ? new Date(since).toISOString() : '';
  const rows = await sql`
    SELECT trim(src) AS source,
           count(*)::int AS asked,
           count(*) FILTER (WHERE q.template LIKE '%:not-recorded' OR q.outcome = 'failed')::int AS flagged,
           count(*) FILTER (WHERE v.verdict IN ('wrong', 'outdated', 'incomplete'))::int AS bad,
           max(q.at) AS "lastAsked"
    FROM question_log q
    CROSS JOIN LATERAL unnest(string_to_array(q.source, ' · ')) AS src
    LEFT JOIN LATERAL (
      SELECT f.verdict
      FROM answer_feedback f
      WHERE (q.turn_id <> '' AND f.turn_id = q.turn_id)
    ...
    WHERE q.source <> '' AND trim(src) <> ''
      AND (${sinceArg} = '' OR q.at >= nullif(${sinceArg}, '')::timestamptz)
    GROUP BY trim(src)
  `;
  const out = {};
  for (const r of rows) out[r.source] = { asked: r.asked, bad: r.bad, flagged: r.flagged, lastAsked: r.lastAsked || null };
  return out;
}
