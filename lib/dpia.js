  ...
  steps: [
    ...
    },
    {
      ...
      body: [
        ...
        'Individuals’ rights over staff data are handled through the practice’s existing data-protection policies; a Notebook note and its attachments can be deleted at any time, which also removes the knowledge and embeddings derived from it. Two stores do not yet honour that pattern: the audit log has no retention limit, purge job or deletion route; and the question log holds the question and the answer indefinitely on the same terms. The open-questions list is the one store of typed text with a deletion route in the interface — any row can be removed from /questions by anyone who can open the page — but it has no retention period either, and a question answered two years ago stays on it until somebody removes it.',
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          { source: 'A staff member types patient details into a question. The text goes to the AI provider, is stored in the question log in full alongside the answer that was given, and is stored truncated in the audit log.', likelihood: 'Possible', severity: 'Severe', overall: 'High' },
          ...
          { source: 'Patient consultation or hospital-document text pasted into Signpost, Reason or Code a document still carries identifiers when it is sent to the AI provider. The pages ask staff to remove them; nothing enforces it. The assistant’s Coding mode is now in the same position by design: neither the name-and-address redaction nor the patient-data screen runs on a message sent under it, because the letter it is handed identifies a patient by definition and both guards refused or mangled the input the mode exists for. Unlike the standalone helpers, a Coding turn goes through /api/agent, so the paste is written to the question log in full, and truncated to the audit log, unless logging is switched off for that computer.', likelihood: 'Probable', severity: 'Significant', overall: 'High' },
    ...
    },
    {
      ...
      table: {
        ...
        rows: [
          ...
          { risk: 'Patient details entered into a question.', options: 'On-screen “never enter patient information” warning (in place); automatic screening that removes names and addresses from a question before it is sent, in the browser and again at the endpoint, with the reader told what was taken (in place, on every mode except Coding — see the pasted-text risk below for why that one is exempt from this and from the screen); a second screen on the Super speed model role, run before the message is sent and over the already-redacted text, which REFUSES to send a message carrying a date of birth, an NHS number, a hospital or record number, a patient’s own contact details or any other identifier the local rules cannot recognise, and names the kind to remove in a popup without repeating the detail (in place); note that this screen fails open, so a message still goes when OpenRouter is unreachable and the local redaction is all that has run (accepted); stop storing question text in the audit log, or shorten and time-limit it (to do); time-limit the question log, which keeps the question and the answer in full (to do).', effect: 'Reduced', residual: 'High', approved: 'No' },
          ...
          { risk: 'Identifiers left in text pasted into the reception helpers, and into the assistant’s Coding mode.', options: 'Instruction to remove identifiers before pasting (in place); nothing is stored server-side by the standalone helpers (in place); names and addresses redacted out of a typed question before the call (in place for every mode EXCEPT Coding, where both that redaction and the patient-data screen are deliberately off — the letter is the input, and a guard that refuses or edits it takes the mode with it); say so where staff are told what the mode is for, so the duty to remove identifiers before pasting is on the mode that carries it (in place, in the mode’s own description); the same redaction for text pasted in as an attached document, and for the Signpost, Reason and Code a document boxes (to do); decide whether a Coding turn should be kept out of the question log and the audit log, which currently hold the paste as pasted (to do); confirm the lawful basis and privacy-notice cover for this processing in the IG review (to do).', effect: 'Reduced', residual: 'Medium', approved: 'No' },
