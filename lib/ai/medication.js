...
// the practice-document answer's (0.5): this tool makes medical claims, so we require the
...
const SYSTEM_INTRO =
  ...
  + 'Never use an em dash or en dash (— or –): restructure with a comma, colon, brackets or a full stop instead. '
  + 'Avoid negative contractions: write "do not" and "cannot", never "don\'t" or "can\'t".\n\n'
  ...
  + 'NEVER invent a source URL or a quote. Every quote MUST be copied word for word from a page you actually retrieved with the search tool. If you cannot find authoritative information for the medicine, set "found" to false — but ALWAYS reply with the JSON object described below, never with a sentence or an apology.';

// The exact JSON shape we want back, with the per-point grounding rule.
const JSON_SHAPE =
  'Your ENTIRE reply must be a single JSON object — it must start with { and end with } — with no preamble, no apology, no markdown fences and no commentary. Never reply with a sentence instead of JSON, not even to decline. Use this exact shape:\n'
  + '{\n'
  + '  "found": true,\n'
  + '  "emergency": false,\n'
  ...
  + '- GROUNDING — for EVERY point, AND for "summary" and "alsoKnownAs", you MUST provide both: (a) "url" — the exact source page you took it from; and (b) "quote" — a SHORT run of words copied VERBATIM (word for word, not reworded) from THAT page, proving it. If you cannot quote a source for a point, simply leave that point out and keep the rest — a part you cannot ground must NEVER make you abandon the whole answer or reply in prose. "summary" and "alsoKnownAs" are OPTIONAL: include each only if you can quote a source for it, otherwise omit that field entirely.\n'
  + '- Each "quote" MUST be valid JSON string text: a SHORT single-line run of about 5–20 words copied from ONE line of the page, with any double quotes escaped (\\") and NO line breaks, tabs or other control characters inside it.\n'
  ...
  + '- Set "found": false ONLY when the name is not a real medicine at all (you could not identify any medicine it might be). If you found ANY authoritative source for the medicine, "found" is true — include whatever you could ground and omit the rest. When you do decline, STILL return JSON: {"found": false, "name": "<the name as given>", "summary": {"text": "one short sentence saying it could not be found in NHS, BNF or eMC sources"}, "sections": []}.';
...
export function parseMedicationJson(raw) {
  ...
  try {
    ...
    // A verbatim quote sometimes carries a raw line break or tab, which is
    // invalid inside a JSON string and would otherwise sink the whole reply
    // ("Unterminated string"). Replacing control characters with spaces makes
    // the JSON parseable; quote verification later is whitespace-insensitive,
    // so this never changes which source backs a point.
    try { o = JSON.parse(str.replace(/[ -]+/g, ' ')); } catch (e2) { o = null; }
  }
