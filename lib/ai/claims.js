...
${JSON.stringify(sources)}`;
  try {
    const res = await fetch(URL, {
      ...
      headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json', 'HTTP-Referer': 'https://riverside-practice.local', 'X-Title': 'Riverside Claim Extractor' },
      body: JSON.stringify({ model, temperature: 0, messages: [{ role: 'user', content: prompt }], provider: { data_collection: 'deny' } }),
    });
...
${JSON.stringify(compact)}`;
  const res = await fetch(URL, {
    method: 'POST', signal: AbortSignal.timeout(45000),
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json', 'HTTP-Referer': 'https://riverside-practice.local', 'X-Title': 'Riverside Contradiction Classifier' },
    body: JSON.stringify({ model, temperature: 0, messages: [{ role: 'user', content: prompt }], provider: { data_collection: 'deny' } }),
  });
