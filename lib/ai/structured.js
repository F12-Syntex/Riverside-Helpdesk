...
export async function readStructured({ openrouter, model, schema, prompt, maxOutputTokens = 2000, role = 'fast', phase = '', turnId = '' }) {
  try {
    const out = await generateObject({ model: openrouter(model), schema, temperature: 0, maxOutputTokens, prompt });
