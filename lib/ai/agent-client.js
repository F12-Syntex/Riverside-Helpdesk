...
export async function askAgent({ question, history = '', customGuides = [], images = [], attachments = [], refresh = false, template = '', signal = null }, onEvent) {
  const res = await fetch('/api/agent', {
    ...
    body: JSON.stringify({ question, history, customGuides, images, attachments, refresh, template }),
  });
  if (!res.ok || !res.body) {
