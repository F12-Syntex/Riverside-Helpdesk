...
const JSON_SHAPE =
  ...
  + 'FORMATTING (both kinds) — make the answer scan like a well-formatted note. Inside "intro", "tip", "urgencyReason", "summary" and every "text" field (steps, actions, red flags) you may use this inline markup and nothing else: '
  ...
  + '<span style="color:#d5281b">red for never-do or emergency actions</span>, <span style="color:#007f3b">green for always-do confirmations</span>, <span style="color:#005eb8">blue for key informational callouts</span>. '
  + 'No other HTML tags or attributes, and no markdown headings, lists or tables inside a field. NEVER put formatting inside the "quote" fields, "message" or "patientMessage" — those must stay plain verbatim text.\n\n'
...
export function parseAiJson(raw) {
  ...
  const item = (x) => (x && typeof x === 'object')
    ? { text: stripInlineSources(x.text), source: parseInt(x.source, 10) || 0, quote: typeof x.quote === 'string' ? x.quote : '' }
    : { text: stripInlineSources(x), source: 0, quote: '' };
  ...
  try {
    ...
    if (/^\s*\{\s*"/.test(str)) {
      const grab = (key) => {
        ...
        return m ? stripInlineSources(JSON.parse('"' + m[1] + '"')) : '';
      };
      const steps = [];
      const stepRe = /"text"\s*:\s*"((?:[^"\\]|\\.)*)"/g;
      for (let m; (m = stepRe.exec(str)) && steps.length < 8;) {
        const t = stripInlineSources(JSON.parse('"' + m[1] + '"'));
        if (t && !steps.some((s2) => s2.text === t)) steps.push({ text: t, source: 0, quote: '' });
      }
      const intro = grab('intro');
      const tip = grab('tip');
      if (intro || steps.length) {
        return { kind: 'answer', answerable: true, intro, steps, message: '', messageSource: 0, messageQuote: '', tip };
      }
    }
    ...
    return { kind: 'answer', answerable: lines.length > 0, intro: '', steps: lines.slice(0, 6).map((t) => ({ text: stripInlineSources(t), source: 0, quote: '' })), message: '', messageSource: 0, messageQuote: '', tip: '' };
  }
}
