...
const JSON_SHAPE =
  ...
  + '- "actions": 0 to 4 items — ONLY things that must be acted on or acknowledged NOW, before the document is filed away and forgotten. Two kinds qualify:\n'
  + '  (a) routing the document to whoever in the practice must review it immediately — a prescription, medication change or anything a pharmacist must check means "send to pharmacist to r/v"; anything needing a doctor’s prompt review (an abnormal result, an urgent request to the GP, a safeguarding concern) means "urgent GP r/v" or similar;\n'
  + '  (b) things the GP themselves must do now: medication changes to make on the record, monitoring or bloods to arrange, results or referrals to chase or make, follow-up the GP (not the hospital) must organise.\n'
  + '  Anything that can simply be looked at later, when the document is next read, is NOT an action — leave it out. An action addressed to anyone outside the practice — the hospital’s own follow-up or review, a community team, a nurse or clinic elsewhere, the patient — is NOT an action either. Each item is a terse imperative fragment of roughly 2–8 words, using common GP shorthand where natural (pt, d/c, FU, r/v, meds, 2/52). NEVER summarise findings, history, examinations or hospital-side plans — the reviewer reads the document itself. Most documents only inform (routine d/c, FYI copy letter, normal result, hospital doing its own follow-up): then "actions" MUST be an empty array.\n'
