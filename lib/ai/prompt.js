...
export function buildAskPrompt({ question, catalog = '', extracts = [], history = '', guideCatalog = '', contacts = [], conflicts = [], imageCount = 0 }) {
  ...
  if (conflicts.length) {
    prompt += 'OPEN KNOWLEDGE CONTRADICTIONS — these have been detected and need staff review:\n'
      ...
      + '\nDo not silently choose between these claims or blend them. If one affects the answer, say the sources conflict, give both attributed versions, and direct the reader to Knowledge manager > Contradictions to correct the source of truth.\n\n';
  }
