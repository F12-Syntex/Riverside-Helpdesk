...
const JSON_SHAPE =
  ...
  + '- "date": the date of the clinical event the document describes (discharge date, clinic/attendance date, date of report); fall back to the letter’s own date. Always dd-mm-yyyy. Empty string if the document shows no date at all.\n'
  ...
  + '- "actions": 0 to 4 items — ONLY things the GP themselves must act on or acknowledge NOW: medication changes to make on the record, monitoring or bloods the GP must arrange, results or referrals the GP must chase or make, follow-up the GP (not the hospital) must organise, safeguarding concerns. An action addressed to ANYONE else — the hospital’s own follow-up or review, a community team, a nurse or clinic elsewhere, the patient — is NOT a GP action and must be left out entirely. Each item is a terse imperative fragment of roughly 2–8 words, using common GP shorthand where natural (pt, d/c, FU, r/v, meds, 2/52). NEVER summarise findings, history, examinations or hospital-side plans — the GP reviews the document itself when it is needed. Most documents only inform (routine d/c, FYI copy letter, normal result, hospital doing its own follow-up): then "actions" MUST be an empty array.\n'
