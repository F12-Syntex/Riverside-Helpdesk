...
// The same gap, but it may not cross the writer claiming the thing for
// themselves. "I am my mother-in-law's carer and I have my own appointment to
// move" is a sentence about the writer's appointment, and refusing it is the
// wallpaper this rule cannot afford: a receptionist who is warned about their
// own request stops reading the warnings.
//
// Only "my own" is excluded, not every "my". Erring toward refusing is right
// here — a false positive costs one conversation and the other error is a
// disclosure — so the exclusion is the narrowest one that fixes the case.
const NEAR_NOT_MINE = '(?:(?!\\bmy own\\b)[^.!?]){0,60}';
...
export const CONFIDENTIALITY_RULES = [
  {
    ...
    pattern: new RegExp('\\bmy\\s+' + RELATION + '(?:\'s|s\'|’s)' + NEAR_NOT_MINE + RECORD, 'i'),
  },
  {
