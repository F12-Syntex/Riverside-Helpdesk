...
const PREFERENCE = /\b(?:evenings?|mornings?|afternoons?|after work|before work|weekends?|saturdays?|sundays?|lunchtimes?|earliest|any ?time|whenever|convenient|suits? me|prefer\w*|if possible|(?:ring|call|phone|contact) me (?:back )?(?:after|before|any time|whenever))\b/i;

// A real job, as opposed to a preference about when one happens.
//
// Access needs are on this list deliberately. An interpreter, a chaperone or a
// step-free room is not a preference about timing — somebody has to arrange it,
// and it is the kind of thing that gets read, nodded at and then not booked. A
// card mentioning it is not the same as it being done, which is the whole
// distinction the panel exists to hold.
const REAL_JOB = /(?:\bfit note\b|\bsick note\b|\bprescription\b|\brepeat\b|\bmedication\b|\bletter\b|\bform\b|\bresults?\b|\breferral\b|\bregister\b|\bregistration\b|\bcancel\b|\bscan\b|\btest\b|\binterpreter\b|\btranslat\w+|\bchaperone\b|\bstep.?free\b|\bwheelchair\b|\bhome visit\b|\bdouble (?:slot|appointment)\b)/i;
