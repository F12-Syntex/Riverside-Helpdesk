...
export async function applyAllClean({ runId, turnId = '' }) {
  ...
  const event = applied.length ? { kind: 'appliedMany', titles: applied.map((a) => a.title), noteIds: applied.map((a) => a.noteId), failed: failed.length } : null;
  return { applied, failed, remaining: Math.max(0, items.length - APPLY_BATCH), event, state: await buildState(await getDefragRun(run.id), event) };
}
