...
/** How many pages one "apply everything that passed" call writes before handing back. */
...
export async function stepRun({ runId, openrouter, roles, turnId = '' }) {
  ...
  if (run.status === 'scanning') {
    ...
    if (run.cursor < chunks.length) {
      const merged = await judgeCandidates({ candidates: chunks[run.cursor], openrouter, model: roles.reasoning.model, turnId });
  ...
  if (run.status === 'proposing') {
    ...
    if (!next) {
      if (!items.some((i) => i.status === 'blocked')) await updateDefragRun({ id: run.id, status: 'review' });
      return buildState(await getDefragRun(run.id));
    }
    ...
    if (out.error) {
      ...
      // not be reached is.
      await updateDefragItem({ id: next.id, status: out.status === 400 ? 'skipped' : 'failed', detail: out.error });
    } else {
...
/**
 * Put one side's wording on the other side's page. The edit itself is pure and
 * narrow (lib/notebook/settle.mjs): exactly the line the reader was shown, only
 * if it is still there exactly as it was read. Snapshotted first, so the
 * decision can be undone from the page's history like any other change.
 */
async function replaceStatement({ winner, loser, turnId }) {
  const note = await getNote(loser.noteId);
  if (!note) return { error: 'That page no longer exists.' };
  const edit = settleStatement({
    body: String(note.body || ''),
    sentenceId: loser.sentenceId,
    expect: String(loser.text || ''),
    winner: String(winner.text || ''),
  });
  if (edit.error) return { error: edit.error };
  if (edit.unchanged) return { ok: true, unchanged: true };
  const result = await applyRevisionedUpdate({ id: loser.noteId, body: edit.body, reason: 'contradiction', turnId });
  if (result.error) return { error: result.error };
  return { ok: true, revisionId: result.revisionId, next: edit.to };
}

/**
 * The reader settles one flag.
 *
 *   'a' / 'b'   that side is right; the other page is corrected to match,
 *               snapshotted first so it can be undone
 *   'resolved'  they fixed it themselves on the page
 *   'dismiss'   the two pages do not in fact disagree
 *
 * @param {{ id: number, decision: 'a'|'b'|'resolved'|'dismiss', note?: string, turnId?: string }} args
 */
export async function decideContradiction({ id, decision, note = '', turnId = '' }) {
  ...
  let resolution = String(note || '').slice(0, 300);
  let revisionId = null;
  if (decision === 'a' || decision === 'b') {
    const winner = decision === 'a' ? flag.sideA : flag.sideB;
    const loser = decision === 'a' ? flag.sideB : flag.sideA;
    const out = await replaceStatement({ winner, loser, turnId });
    if (out.error) return { error: out.error, status: 409 };
    revisionId = out.revisionId || null;
    resolution = `Kept the wording on “${winner.title}”. “${loser.title}” was changed to match.` + (resolution ? ' ' + resolution : '');
    // The page just changed, so any proposal already made for it describes text
    // that no longer exists.
    const items = await listDefragItems(flag.runId);
    for (const item of items.filter((i) => i.noteId === loser.noteId)) {
      if (item.proposalId && item.proposalStatus === 'draft') await updateProposal({ id: item.proposalId, status: 'superseded' });
      if (item.status !== 'applied') await updateDefragItem({ id: item.id, status: 'pending', proposalId: null, detail: '' });
    }
  } else if (decision === 'dismiss') {
    resolution = resolution || 'Both can be right — not a disagreement.';
  } else if (decision === 'resolved') {
    resolution = resolution || 'Settled on the page by hand.';
  } else {
    return { error: 'A decision must be “a”, “b”, “resolved” or “dismiss”.', status: 400 };
  }

  await updateContradiction({ id: flag.id, status: decision === 'dismiss' ? 'dismissed' : 'resolved', resolution });
  ...
  return { ok: true, revisionId, state: run ? await buildState(await getDefragRun(run.id)) : null };
}
 ...
 */
export async function applyAllClean({ runId, turnId = '' }) {
  const run = await getDefragRun(runId);
  if (!run) return { error: 'No such run.', status: 404 };
  const items = (await listDefragItems(run.id)).map(shapeItem).filter((i) => i.status === 'proposed' && i.review && i.review.clean);
  const applied = [];
  const failed = [];
  for (const item of items.slice(0, APPLY_BATCH)) {
    const out = await applyProposal({ proposalId: item.proposalId, turnId });
    if (out.error) {
    ...
    await updateDefragItem({ id: item.id, status: 'applied', detail: '' });
    applied.push({ title: item.title, revisionId: out.revisionId });
  }
  return { applied, failed, remaining: Math.max(0, items.length - APPLY_BATCH), state: await runState(run.id) };
}
