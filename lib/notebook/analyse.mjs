...
/** Section titles from the root down to this row — the path the assistant cites. */
function pathBuilder(rows) {
...
/**
 * Every page of the Notebook, cut into sentences, with the path a reader (and
 * the assistant) knows it by. The map, the rules and the coherence sweep all
 * start here, so all three mean the same thing by "a page".
 *
 * @param {Array} notes        listNotes() rows
 * @param {Array} attachments  listAttachments() rows
 * @returns {Array<{noteId:number,title:string,row:object,page:object,sentences:Array,path:string[]}>}
 */
export function notebookPages(notes = [], attachments = []) {
  ...
  const pathOf = pathBuilder(rows);

  return rows
    .filter((r) => r.parentId != null && !r.isSection)
    .map((r) => {
      const page = pageByNote.get(r.id) || { docTitle: 'Notebook: ' + pathOf(r).join(' / '), text: '', images: [] };
      return { noteId: r.id, title: String(r.title || 'Untitled'), row: r, page, sentences: splitSentences(page.text), path: pathOf(r) };
    });
}
...
export function analyseNotebook(notes = [], attachments = [], signals = {}) {
  ...
  const pages = notebookPages(rows, attachments);
