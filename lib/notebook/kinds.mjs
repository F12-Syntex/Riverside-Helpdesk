...
import { emailForm, ers, pathology } from '../templates/blocks.mjs';
...
export function normaliseFields(kind, raw) {
  ...
  const src = def.legacy ? { ...base, ...def.legacy(base) } : base;
  const out = {};
...
const EMAIL_REFERRAL = {
  ...
  short: 'Email',
  summary: 'One referral emailed using the form the doctor created',
  ...
  fields: [
    ...
    f('form', 'Form', 'text', { hint: 'Optional. The form the doctor fills in, as it is named in EMIS — "RP Echo".' }),
    f('to', 'Email', 'text', { hint: 'Optional. The address, character for character, where the form does not carry it.' }),
    f('notes', 'Additional notes', 'textarea', { hint: 'Optional. Never a patient name, date of birth or NHS number.' }),
  ],
  // Nothing is required: an emailed referral is always the doctor's form sent
  // by email, so a card with no boxes filled in still says the whole of it.
  required: [],
  // The AccurX-window shape this replaced kept a rule for the address, an
  // organisation, an attachment and dictated wording. A note saved in that
  // shape and not yet in this one (no `notes` key) has them carried into the
  // notes, so what the practice wrote is not dropped on its next save.
  legacy: (raw) => {
    if (raw.notes !== undefined) return {};
    const said = (v) => String(v == null ? '' : v).trim();
    const lines = [
      said(raw.toRule) && 'Email: ' + said(raw.toRule),
      said(raw.org) && 'Organisation: ' + said(raw.org),
      said(raw.attach) && said(raw.attach) !== 'EMIS file' && 'Attach: ' + said(raw.attach),
      said(raw.body),
    ].filter(Boolean);
  ...
  render: (fields) => emailForm({ form: fields.form, to: fields.to, notes: fields.notes }),
  searchText: (fields, note) => [note?.title, fields.service, ...(fields.aliases || []), fields.form, fields.to]
    .filter(Boolean).join(' \u00b7 '),
};
