...
//
// One proposal, one page, per call. The whole-Notebook run (lib/notebook/run.js)
// is a loop over exactly these steps — it adds a queue and a coherence sweep in
// front of them, and can weaken none of them.
import { z } from 'zod';
...
/**
 * A stored proposal, shaped for review, with no model call: the code checks are
 * re-run against the page as it is NOW and the meaning verdicts already
 * recorded are read back. `stale` says the page has moved on since the proposal
 * was made, which is the one thing a reader cannot see from the diff alone.
 */
export async function loadProposal({ proposalId }) {
  ...
  const { note, page, path, typed } = ctx;
  const source = String(note.body || '').replace(/\r\n/g, '\n');
  const stale = hashBody(source) !== proposal.sourceHash;
  const validation = validateProposal({ sourceBody: source, proposal: { body: proposal.body, map: proposal.map, dropped: [] }, page, typed });
  const meaning = proposal.meaning || { ok: false, verdicts: {}, changed: [], unsure: [], missing: [], model: '' };
  return { ...shape(proposal, validation, meaning, note, path), stale };
}
