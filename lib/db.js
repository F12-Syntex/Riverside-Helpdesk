...
export function ensureNotebookSchema() {
  ...
  _notebookSchemaReady = (async () => {
    ...
    // THE OUTPUT TAG IS GONE. A page's shape used to be inherited from the
    // folder above it, so dragging it somewhere else silently changed how it
    // answered and nobody was told. The shape is a property of the note now —
    // see `kind` — and the column is dropped rather than left to be read by
    // something that has not been updated.
    await sql`ALTER TABLE notes DROP COLUMN IF EXISTS output_tag`;
