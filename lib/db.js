...
// from DEV_DATABASE_URL when that is set, and from DATABASE_URL otherwise —
// see resolveDatabaseUrl() below.
//
...
//
// DATABASE_URL is the practice's live database: production serves from it, and
// so did every preview and every developer's machine, which is how a schema
// change on a branch was able to take the live Notebook down. A branch that
// should not touch live data sets DEV_DATABASE_URL — on Vercel, as a Preview
// variable scoped to the branch; locally, in .env.local — and everything here
// follows it. Unset, nothing changes: DATABASE_URL is used as before.
...
export function getSql() {
  if (!_sql) {
    ...
    if (!url) throw new Error('Neither DEV_DATABASE_URL nor DATABASE_URL is set — add one to .env.local.');
