...
export function ensureNotebookSchema() {
  ...
  _notebookSchemaReady = (async () => {
    ...
    // What shape the answers from this part of the Notebook come back in —
    // '' for the page as it is written, or one of OUTPUT_TAG_IDS
    // (lib/templates/output-tags.mjs). Set on a section and inherited by
    // everything beneath it, so the practice states it once per folder.
    await sql`ALTER TABLE notes ADD COLUMN IF NOT EXISTS output_tag text NOT NULL DEFAULT ''`;
...
let _answerCacheSchemaReady = null;

// Answers already given, so the same question is not researched twice.
//
// A staff question costs a full agent turn — several model calls and several
// searches — and the same handful of questions are asked every day, in slightly
// different words each time. One row per answered question: the canonical key
// (an exact-wording hit needs no model call at all), the question's embedding
// (so a rewording still finds it), and the finished payload the browser gets.
//
// Freshness is not a guess. `fingerprint` is the state of the Notebook the
// answer was written from and `model` the model that wrote it; a row is only
// served while both still match, so editing a note or changing the model
// retires every answer that predates the change. `created_at` bounds the rest.
//
// Kept independent of every other schema: the cache must never be the reason a
// question cannot be answered, so a failure here is swallowed by the caller and
// the question is answered the slow way.
export function ensureAnswerCacheSchema() {
  if (_answerCacheSchemaReady) return _answerCacheSchemaReady;
  const sql = getSql();
  _answerCacheSchemaReady = (async () => {
    ...
    `;
    // Every read filters on these two before it looks at anything else.
    await sql`CREATE INDEX IF NOT EXISTS answer_cache_fresh_idx ON answer_cache (model, fingerprint)`;
    // The reworded-question path: nearest neighbour by cosine distance.
    await sql`CREATE INDEX IF NOT EXISTS answer_cache_embedding_idx ON answer_cache USING hnsw (embedding vector_cosine_ops)`;
  })().catch((e) => {
    _answerCacheSchemaReady = null;
    throw e;
  });
  return _answerCacheSchemaReady;
}
