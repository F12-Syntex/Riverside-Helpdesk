...
// THE CONDITIONS MEDICINES ARE NOT ROUTINELY PRESCRIBED FOR, from the NHS
// "Prescribing of medicines to treat minor illnesses is changing" poster and
// the practice's own "Self-care and minor illness advice" page. A request for a
// GP appointment or a prescription for any of these is the community pharmacy's
// (a minor illness referral, with CPSAS free medicines for those eligible) —
// unless something about THIS patient takes it off the list: it is not minor,
// not infrequent, has already failed, or they are unwell with it.
//
// Kept as data so the reader's prompt, the pharmacy entry and the card all say
// the same list. The poster's wording, one entry per tile.
export const SELF_CARE_CONDITIONS = [
  'Acute sore throat', 'Conjunctivitis', 'Coughs, colds and nasal congestion', 'Mild irritant dermatitis',
  'Cradle cap', 'Dandruff', 'Diarrhoea (adults)', 'Mouth ulcers',
  'Dry eyes / sore tired eyes', 'Earwax', 'Excessive sweating', 'Oral thrush',
  'Haemorrhoids', 'Head lice', 'Indigestion and heartburn', 'Sunburn',
  'Infant colic', 'Infrequent cold sores of the lip', 'Infrequent constipation', 'Threadworms',
  ...
  'Minor pain, discomfort and fever (aches and sprains, headache, period pain, back pain)',
  'Warts and verrucae', 'Travel sickness', 'Prevention of tooth decay',
  'Teething / mild toothache', 'Ringworm / athlete’s foot', 'Sun protection',
];
...
export const DESTINATIONS = [
  ...
  },
  {
    ...
    covers: 'Three kinds of referral. (1) A Pharmacy First pathway within its age range — the pharmacist can TREAT, including antibiotics or antivirals: uncomplicated UTI in women 16 to 64, shingles 18 and over, impetigo 1 and over, infected insect bites 1 and over, sinusitis 12 and over, sore throat 5 and over, acute otitis media (earache) 1 to 17. (2) A minor illness referral for anything reasonably minor: cough, cold, flu, sore throat, earache, blocked ear or wax, red, sticky or watery eye, eyelid problems, mouth ulcers, rashes, acne, allergic reaction, hay fever, headache, sprains, strains and aches, diarrhoea, vomiting, constipation, pain passing urine, vaginal discharge or itch, thrush, scabies, head lice, threadworms, athlete’s foot, teething, sleep difficulties, scratches, grazes and wound dressings. (3) The practice’s self-care list — medicines that can be bought are NOT routinely prescribed for these, so a request for a GP appointment or a prescription for one of them is the pharmacy’s: ' + SELF_CARE_CONDITIONS.join(', ').toLowerCase() + '. Eligible patients (under 16, 16 to 18 in full-time education, on Universal Credit or another benefit giving free prescriptions, HC2, homeless, asylum seekers and refugees, care leavers) get those medicines free through CPSAS, but only if we refer them. Referring rather than signposting transfers clinical responsibility to the pharmacist, who assesses, treats and follows the patient up; if the pharmacist thinks it needs more, they send the patient back to us.',
    refuses: 'Anybody outside the age range for that pathway — the pharmacist cannot treat and the request stays with us. A patient who is systemically unwell rather than just locally unwell. UTI in a man, in a pregnant or breastfeeding patient, with a urinary catheter, or recurrent (2 episodes in 6 months or 3 in 12) — clinical review here; and a UTI with fever, shivering, back or side pain, vomiting or marked illness is a possible kidney infection for the duty doctor today. Bullous impetigo (large fluid-filled blisters). Shingles in pregnancy, in an immunosuppressed patient, or affecting the eye or the tip of the nose — the duty doctor’s. A bite or sting that is rapidly getting worse. The same problem presenting again after pharmacy treatment has already failed, or a "minor" problem that keeps coming back or has gone on for months, which points to a long-term condition for a GP. Anything the patient is asking to have examined for a reason the pharmacist cannot examine it.',
  },
  {
    id: 'midwife',
    rank: 1,
  ...
  },
  {
    ...
    refuses: 'Anything the patient describes as getting worse or as having changed — reassess for the duty list. Minor illness inside a Pharmacy First pathway with nothing complicating it. Anything on the self-care list (hay fever, heartburn, a cold, mild acne, earwax, head lice and the rest) where the patient wants an appointment or a prescription for something they could buy — that is the community pharmacy, even when they asked for a GP. An MSK problem in an adult with no red flags, which is the FCP’s and is faster. Reviews that belong to the nursing team. Purely administrative requests, which are a doctor task rather than an appointment. New patients who have not had their new patient check. A GP appointment is NOT the safe default here — the practice’s own rules already provide the default, and naming it only ever takes a message away from a service that would have dealt with it.',
  },
  ...
  /* ------------------------------------------------------------ a doctor today */
  {
    ...
    covers: 'Same-day assessment by a doctor, and the default for anything that cannot confidently be placed elsewhere: new symptoms over hours or days in a patient who is unwell with them, fever in somebody very young, frail or immunosuppressed, a known long-term condition getting worse, a suspected infection needing examination, pain that is new, severe or escalating, a patient seen recently and calling back no better, a young child or a baby who is unwell, a housebound, elderly, frail or palliative patient asking to be seen (they can deteriorate quickly and may need a home visit), and anything that worries the reader with no clear alternative.',
...
// THE SECOND QUESTION A DOCTOR APPOINTMENT ASKS, AND THE ONE THE CARD USED TO
// LEAVE TO THE READER.
//
// "A GP appointment here" is not one thing to book. The guide's own words for
// it are "a booked telephone OR face-to-face appointment", and the duty doctor
// page opens its actioning list with "decide whether an examination is likely:
// no means a telephone consultation, yes means a face-to-face slot" — an
// instruction to make a decision, printed on a card that gave no help with it.
// A card that says where a message goes and stops has answered half of what
// reception does next.
//
// The test below is the practice's, transcribed from docs/routing.md
// ("Telephone or face-to-face", under the duty doctor). It is not a clinical
// judgement and it does not diagnose: it asks what the doctor would have to DO
// in the room, which is a question about the appointment rather than about the
// patient.
export const APPOINTMENT_MODE_TEST =
  'The test is whether the doctor will need to look at, listen to, or feel something. If yes, or probably, book face-to-face.';

// AND ITS TIE-BREAK, WHICH IS THE WHOLE SAFETY MODEL OF THIS ONE. Face-to-face
// is the direction it is safe to be wrong in, and the guide says so in as many
// words. Everything below fails that way: an "unsure" reading books a room.
export const APPOINTMENT_MODE_DEFAULT =
  'If in doubt, book face-to-face. Converting that to a phone call is far easier than the reverse.';

export const FACE_TO_FACE_EXAMPLES = [
  'Rashes, lumps, swelling and wounds',
  'Ear or throat problems',
  'Chest or abdominal pain',
  'Anything the patient wants shown to somebody',
];

export const TELEPHONE_EXAMPLES = [
  'Medication side effects',
  'Review of a previous consultation',
  'Results',
  'Advice about a condition the patient already has',
];

// The disclosure the card folds all of that away under, in the guide's order.
export const APPOINTMENT_MODE_RULES = [
  APPOINTMENT_MODE_TEST,
  '**Face-to-face** — ' + FACE_TO_FACE_EXAMPLES.join(', ').toLowerCase() + '.',
  '**Telephone** — ' + TELEPHONE_EXAMPLES.join(', ').toLowerCase() + '.',
  APPOINTMENT_MODE_DEFAULT,
  'This is about what the doctor would have to **do**, not about how serious it is. Where the request goes was decided before this and nothing here changes it.',
];

/**
 * What each answer is called, and what reception books off it.
 *
 * `unsure` is a real answer and it is deliberately not a third slot type: the
 * guide has already said what to do when nobody can tell, so it books the same
 * thing face-to-face books and says why on the card. What it must not do is
 * quietly present a guess as a decision — see `hedged`.
 */
export const APPOINTMENT_MODES = {
  faceToFace: { id: 'faceToFace', label: 'Face-to-face', book: 'Book a face-to-face slot', hedged: false },
  telephone: { id: 'telephone', label: 'Telephone', book: 'Book a telephone slot', hedged: false },
  unsure: { id: 'unsure', label: 'Face-to-face', book: 'Book a face-to-face slot', hedged: true },
};

/** One appointment mode by id. Anything unrecognised is "unsure", which books a room. */
export const appointmentMode = (id) => APPOINTMENT_MODES[String(id || '')] || APPOINTMENT_MODES.unsure;

// WHERE THE QUESTION IS EVEN ASKED. Only the two routes that end in a booked
// appointment with a doctor — the guide asks for a slot type on both of them and
// nowhere else. A pharmacist, a physiotherapist or a nurse clinic is not the
// practice's appointment book, and an ambulance is not an appointment at all.
export const BOOKED_WITH_A_DOCTOR = ['gp', 'dutyDoctor'];

/** Does this destination need a telephone-or-face-to-face answer beside it? */
export const needsAppointmentMode = (id) => BOOKED_WITH_A_DOCTOR.includes(String(id || ''));
...
// A PATIENT WHO MIGHT BE PREGNANT IS NOT A PHARMACY FIRST PATIENT.
//
// The guide says it outright on the Pharmacy First page — "pregnant patients
// with a UTI" is one of the things that pathway refuses — and it says it again
// on the interrupt page, where a recent pregnancy changes what a new symptom
// means.
//
// MIGHT is the word that matters, and it is why this is not just the word
// "pregnant". A patient wrote in with urinary symptoms and added that she and
// her husband were trying for a baby and her period was five days late. Nobody
// had told her she was pregnant, so nothing in the message said so — and the
// possibility is exactly the thing that decides where she goes. What reception
// needs is not a diagnosis; it is to stop and let a clinician ask.
export const PREGNANCY_POSSIBLE = new RegExp([
  '\\bpregnan\\w+', '\\bexpecting\\b', '\\bantenatal\\b', '\\bmidwife\\b',
  '\\b(?:trying|ttc)\\b[^.!?]{0,30}\\b(?:baby|conceive|pregnant|family)\\b',
  '\\b(?:period|periods|bleed)\\w*\\b[^.!?]{0,30}\\b(?:late|missed|overdue|not (?:come|arrived))\\b',
  '\\bmissed\\b[^.!?]{0,20}\\bperiods?\\b',
  '\\bivf\\b', '\\bfertility\\b',
  '\\bpositive\\b[^.!?]{0,25}\\b(?:test|pregnancy test)\\b',
].join('|'), 'i');

/**
 * Might this patient be pregnant, on the strength of what they wrote?
 *
 * Deliberately generous, and deliberately not a judgement about whether they
 * are. It is read next to a pathway that excludes pregnancy, and everything it
 * does is send the message to somebody allowed to ask the question.
 */
export const mightBePregnant = (text = '') => PREGNANCY_POSSIBLE.test(String(text || ''));
...
export function routingGuidance() {
  ...
  return [
    ...
    '7. Anything about a medicine the patient is already prescribed — pharmacyTeam. Minor illness, a Pharmacy First pathway, or anything on the self-care list (not routinely prescribed: ' + SELF_CARE_CONDITIONS.join(', ').toLowerCase() + ') — pharmacy.',
