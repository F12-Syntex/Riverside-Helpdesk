...
// value is the message's own detail. The free sentence is only shown when the
// reading gave no criteria.
//
...
export const ROUTING_CRITERIA = [
  ...
  { id: 'worsening', bare: true, label: 'Getting worse', hint: 'deteriorating — "worse each day"' },
  ...
  { id: 'recurrent', bare: true, label: 'Recurring', hint: 'keeps coming back — "third time this year"' },
  ...
  { id: 'housebound', bare: true, label: 'Housebound', hint: 'cannot attend the surgery — "bedbound"' },
  ...
  { id: 'unclear', bare: true, label: 'Not enough detail', hint: 'the message does not say enough to place it' },
];
 ...
 * values trimmed to a few words, at most four. A value is required except on
 * the labels marked `bare` — "Getting worse" says enough on its own; "Age"
 * does not, and one without its value is dropped rather than shown empty.
 */
export function cleanCriteria(list) {
  ...
  for (const c of Array.isArray(list) ? list : []) {
    ...
    // "Patient asked for" with nothing after it says nothing. Only the few
    // labels that are complete on their own may come without a value.
    if (!value && !BY_ID.get(id).bare) continue;
...
export const criteriaForPrompt = () => ROUTING_CRITERIA.map((c) => `  · ${c.id} — ${c.label}${c.bare ? ' (no detail needed)' : ''}: ${c.hint}`);
