...
//
// `screened` says whether the patient-data screen runs on a message sent under
// this command, and only one command sets it: Coding, which is false.
//
// WHY THE ONE EXCEPTION. Everything typed into the field goes past a screen that
// refuses a message identifying a particular patient (lib/safety/patient-data.mjs)
// — an NHS number, a date of birth, a hospital number, the shapes that can be
// recognised but not removed. A discharge summary carries all three by
// definition, so on this one mode that screen refuses the exact thing the mode
// exists to answer: paste the letter, and nothing is sent at all. The same
// argument already exempts an ATTACHED document, which is not screened for this
// reason; a letter pasted into the box is the same letter and was refused only
// because of how it arrived.
//
// WHAT IS NOT SWITCHED OFF WITH IT. The deterministic redaction
// (lib/safety/identifiers.mjs) still runs, in the browser and again on the
// server: names and addresses come out of a coding paste as they do everywhere
// else, and the reader is still told. The filing title never carries a name, a
// number or a date of birth either — DOC_CODING_RULES in lib/templates/writing.mjs
// says so and the prompt is built from that list. What is dropped is the model
// call that HOLDS THE SEND, not the guard that edits the words.
//
...
export const COMMANDS = [
  ...
  },
  {
    ...
    detail: 'Paste a letter or discharge summary. Answers with the filing title — the clinical event date, the site and department, and the actions the practice is left with. The patient-data screen does not run on this mode: a letter about a patient is the thing it is for.',
...
/**
 * Does the patient-data screen run on a message sent under this command?
 *
 * True for everything except Coding, whose whole job is a pasted letter about a
 * patient — see `screened` at the head of this file for what that does and does
 * not switch off. Takes the command rather than its name so a caller that has
 * already resolved a typed command cannot be handed a different answer than the
 * one the mode picker gets.
 */
export const screensPatientData = (command) => !(command && command.screened === false);
