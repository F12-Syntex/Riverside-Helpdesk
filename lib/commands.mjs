...
export const COMMANDS = [
  {
    // THE ONE COMMAND THAT NEVER LEAVES THE BROWSER. Contact is a search, not
    // a question: whatever is typed under it is matched against the practice
    // directory as it is typed (lib/contacts.fuzzy.mjs), with the CQC register
    // beneath, and the results are the page. Nothing is sent to a model, so
    // `local` says so and COMMAND_TEMPLATES leaves its template out — the
    // server will not honour a template that has no card to render.
    //
    // WHY A MODE RATHER THAN A PANEL. The directory used to float over the
    // field as soon as a few letters were typed, gated by a guess at whether
    // the text "looked like a lookup". The guess was wrong both ways — a
    // number wanted mid-sentence never showed, and a short question got a
    // list of telephone numbers over it — and a panel that appears on its own
    // is a panel somebody is always closing. Choosing Contact says what the
    // typing is for, and while it is chosen the whole page is the list.
    name: 'contact',
    aliases: ['contacts', 'number'],
    template: 'contactSearch',
    fill: 'contacts',
    local: true,
    label: 'Contact',
    icon: 'phone',
    placeholder: 'A name, a department, a town, a postcode, a number',
    summary: 'Find a number in the practice directory',
    detail: 'Type a name, a department or part of a number and the practice directory is searched as you type, with the CQC register — every service registered in England — beneath it. Nothing is sent anywhere: the list is matched in the browser and every number is shown exactly as the directory holds it. Enter copies the top match; ↑ ↓ then Enter copies another.',
    usage: '/contact who you are looking for',
    example: '/contact homerton',
  },
  {
    name: 'accurx',
  ...
  {
    name: 'medication',
    // What people say at the desk. Resolved, never listed — see `coding`.
    aliases: ['meds'],
...
// The templates the server will render for a command. A `local` command is
// answered in the browser and has no card on the server, so its template is
// not here: sent up anyway, it is ignored and the message answered the
// ordinary way, rather than handed to a renderer that returns nothing.
export const COMMAND_TEMPLATES = COMMANDS.filter((c) => !c.local).map((c) => c.template);

/** Is this command answered in the browser, with nothing sent to a model? */
export const isLocalCommand = (command) => Boolean(command && command.local);
