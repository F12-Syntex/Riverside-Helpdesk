 ...
 *
 * /accurx and /document are no longer offered — not in the switch above
 * the box, and not in the list a "/" brings up. They are still parsed,
 * still force their own template and still render exactly as they did:
 * both are everyday answers and the people who use them use them by
 * habit, so withdrawing them to tidy a list would break the two commands
 * the practice reaches for most.
 *
