...
import { appointmentReasonAnswer, codedDocumentAnswer, consultationNoteRulesAnswer, documentCodingAnswer } from './writing.mjs';
...
export const TEMPLATES = [
  ...
  },
  {
    id: 'bloodForm',
    group: 'Procedures',
    label: 'Blood form — a health check',
    why: 'The card is the Test Requests screen rather than a list of test names the reader then has to find boxes for. Ordered Items is what actually gets sent, so it is shown as the screen shows it; Clinical Details is the box most often left empty, and it is the type of health check.',
    sample: 'What bloods do I order for an NHS health check?',
    render: () => bloodFormAnswer({ check: 'NHS Health Check' }),
  },
  {
    id: 'bloodForm.fromNotebook',
    group: 'Procedures',
    label: 'Blood form — read off the practice’s own page',
    why: '"Mental health review blood form" is not really a question about a review: it is somebody wanting to know which items to select. The array holds one filled-in form, so the Notebook is read by name before the card gives up — and the tests it lists tick the boxes they unambiguously name, with the page and the line quoted. "U&E" and "HbA1c" name no single box, so they stay on the list rather than ticking a guess.',
    sample: 'Mental health review blood form',
    render: () => bloodFormAnswer({
      check: 'mental health review',
      pages: [{
  ...
  {
    id: 'bloodForm.unrecorded',
    group: 'Procedures',
    label: 'Blood form — nothing written down for it',
    why: 'No filled-in form, and no Notebook page either. Nothing is ticked — the health-check panel under another review’s name is a patient bled for the wrong tests — and the gap is flagged for the practice. But the reader is not sent away empty: the form’s own boxes are shown, because which item to select was the question and the boxes are the same boxes whatever the review.',
    sample: 'Which tests go on the blood form for a learning disability review?',
    render: () => bloodFormAnswer({ check: 'learning disability review' }),
  },
  {
    id: 'documentCoding',
    group: 'Writing',
  ...
  },
  {
    ...
    why: 'Writing up a contact with a patient as the one-line record entry the next person reads. The examples are built by the same function that builds a real entry.',
    sample: 'How do I write up a phone call on the patient’s record?',
    render: () => consultationNoteRulesAnswer(),
  },
  {
    id: 'appointmentReason',
    group: 'Writing',
    label: 'Appointment reason — the house style',
    why: 'The rules for the one line a clinician reads. It is all plain Q&A does with a reason for appointment now: writing the line from a patient’s pasted message is the AccurX mode’s job, because that mode also decides where the request goes — and a wording card on its own answered the smaller half of the question.',
...

  {
    ...
    sample: 'Discharge summary, Homerton Ophthalmology, 07-Aug-2026, discharged',
...

  {
    ...
    sample: 'I have had heartburn for about 3 weeks and it is getting worse, gaviscon is not helping. Best to call after 2pm.',
    render: () => accurxAnswer({
      condition: 'heartburn',
  ...
  },
  {
    ...
    sample: 'My knee is playing up again. I saw Dr Okafor about it in July, she gave me exercises and said to come back if it did not settle. It has not settled.',
    render: () => accurxAnswer({
      condition: 'knee pain',
  ...
  },
  {
    ...
    sample: 'I have had a crushing pain in my chest since this morning and I am short of breath',
    render: () => accurxAnswer({
