...
import { CLINICAL_PATHWAYS, CPSAS_CONDITIONS, MINOR_ILLNESS, URINARY, findClinicalPathway, mentionsCondition, pharmacyFirstAnswer, pregnantUrinaryAnswer } from './pharmacy.mjs';
...
import { diabetesNeedsGp, mightBePregnant, needsDiabetesNurse, nurseTask, nurseTaskNeedsGp } from '../triage/destinations.mjs';
 ...
 */
export function triagePatientAnswer({ condition = '', text = '', complaint = '', whole = '' } = {}) {
  const said = String(condition || '').trim();
  const full = [said, String(text || '')].filter(Boolean).join('\n');
  // `full` is this card's own request; `everything` is the message it arrived
  // in. What the card may CLAIM comes from the first — that is span locality,
  // and it is why a knee card cannot describe somebody's voice. What rules a
  // service OUT comes from the second: an exclusion written three paragraphs
  // away is still an exclusion, and reading only the routed request is how a
  // possible pregnancy went unnoticed on the card about the urine.
  const everything = [full, String(whole || '')].filter(Boolean).join('\n');
  if (!said) {
    return answer({
  ...
  // A POSSIBLE PREGNANCY, BEFORE ANY OF THE LISTS.
  //
  // Checked here rather than inside the pharmacy card because it decides where
  // somebody goes, and a check that only runs once a message has already reached
  // one card misses every message that reaches a different one. Read over the
  // WHOLE message: the sentence that rules the pharmacy out is very often in a
  // different request from the symptom the card is about.
  if (URINARY.test(everything) && mightBePregnant(everything)) {
    return pregnantUrinaryAnswer({ condition: said });
  }

  // THE LISTS ARE MATCHED AS WORDS THE PATIENT MEANT, not as substrings.
  //
  // This was `norm(full).includes(...)`, and "no back pain" includes "back
  // pain": a patient listing what she had NOT got was put on the CPSAS
  // musculoskeletal line, and a urinary infection came back as a Pharmacy First
  // card for back pain. See mentionsCondition, which matches the phrase as words
  // and drops an occurrence the patient ruled out.
  const pathway = findClinicalPathway(full);
  const onList = pathway
    || CPSAS_CONDITIONS.some((c) => mentionsCondition(full, c.split(' / ')[0].replace(/\s*\(.*\)/, '')))
    || MINOR_ILLNESS.some((c) => mentionsCondition(full, c.split(/[,(]/)[0]));

  if (onList) {
    const card = pharmacyFirstAnswer({ condition: said, text, complaint, whole });
    return answer({
      title: card.title,
...
/**
 * Somebody walks to the duty doctor now.
 *
 * WHY THIS EXISTS. `dutyInterrupt` was added to the ladder without a card of its
 * own, so a message raised to it fell through to raisedAnswer above — a
 * perfectly calm card offering a destination to copy, and ending "if anything
 * about it has got worse since they wrote, pass it to the duty doctor instead".
 * The message it did that on described three weeks of back pain radiating into a
 * leg, new bladder symptoms and saddle numbness. No 999, no A&E, no instruction
 * to interrupt anybody, and a closing line advising the reader to wait and see.
 *
 * The lesson is not about this one card. A destination that means "now" must
 * never be able to render as a destination that means "book it", so the card is
 * chosen by how senior the destination is (see accurxAnswer), not by whether the
 * model happened to write the word "emergency" — the same message produced the
 * full emergency card when it was called "cauda equina syndrome" and none of it
 * when it was called "lower back pain with neurological symptoms".
 *
 * No Copy, for the reason at the top of this file: nobody types anything off
 * this one, they stand up.
 */
export function interruptAnswer({ condition = '', because = '', wouldHave = '' } = {}) {
  const said = String(condition || '').trim() || 'This request';
  return answer({
    title: heading(said),
    subtitle: 'Interrupt the duty doctor — now',
    destination: 'dutyInterrupt',
    blocks: [
      fields([field('Send it to', 'The duty doctor, now — walk to them')], 'Where this goes'),

      note('This does <mark>not</mark> go in a task queue and does <mark>not</mark> wait for a call-back slot. Tasks are not read in real time.', 'critical'),

      because
        ? note('What moved it — the patient’s own words: “' + because + '”', 'warn')
        : null,

      bullets([
        'Take a **call-back number** before doing anything else.',
        '**Walk to the duty doctor** and say what the patient wrote, in their words.',
        'If you cannot reach a duty clinician immediately, send the patient to **A&E**.',
        'If the patient is acutely unwell or deteriorating while you are speaking to them, call **999** and stay with them.',
        'Do not end the call until you have been told what to tell the patient.',
      ]),

      wouldHave
        ? note('Matching this message on its words alone would have sent it to **' + wouldHave + '**. It was read as a whole instead. Nothing about that reading makes this wait.', 'info')
        : null,

      note('Do not reassure the patient and do not tell them it is probably nothing. If they have talked themselves out of it — patients often do — that is not a reason to agree with them.', 'warn'),
    ],
    source: ['Triage guidelines'],
  });
}
