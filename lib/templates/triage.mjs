...
import { caudaEquinaAnswer, fcpAnswer, isSpinalEmergency, mskFeatures, needsFcp } from './fcp.mjs';
 ...
 * this goes.
 */
export function triagePatientAnswer({ condition = '', text = '' } = {}) {
    ...
    if (EYE_TO_AE.test(full)) {
      return answer({
        ...
        blocks: [
          ...
          fields([
            field('Send it to', 'Moorfields Eye Hospital (MEH) — eye A&E'),
          ], 'Where this goes'),
  ...
  if (needsFcp(msk)) return fcpAnswer({ condition: said, text });
  ...
  if (onList) {
    const card = pharmacyFirstAnswer({ condition: said, text });
    return answer({
      title: card.title,
  ...
  if (msk.msk) return fcpAnswer({ condition: said, text });
