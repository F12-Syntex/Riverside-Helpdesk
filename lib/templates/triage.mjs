...
// The eye exclusions, taken from the practice's own MECS page rather than from
// caution. That distinction cost an answer: an earlier version sent blurred
// vision, a foreign body, mild trauma and contact lens wearers to the duty
// doctor, and the page says MECS treats all four. Only this list is excluded,
// and the page sends these to A&E rather than to us.
const EYE_TO_AE = /\b(sudden loss of vision|lost (?:my |their |all )?(?:sight|vision)|considerable (?:eye )?pain|severe (?:eye )?pain|penetrat\w+|laceration\w*|chemical|burn\w*|after (?:eye )?surgery|recent eye surgery)\b/i;
...
export function triagePatientAnswer({ condition = '' } = {}) {
  ...
  if (EYE.test(said)) {
    ...
    return minorEyeServiceAnswer({ condition: said });
  }
