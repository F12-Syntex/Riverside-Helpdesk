...
export function appointmentBookingAnswer({ reason = '', details = [], booking = [] } = {}) {
  ...
  return answer({
    ...
    blocks: [
      // The card is titled "the line to copy" and the panel is titled "copy
      // into the appointment", and until now neither of them came with one. It
      // is the whole output of the command: one line that gets typed into the
      // appointment, by somebody with the patient still on the phone.
