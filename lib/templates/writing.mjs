...
// THE EMIS APPOINTMENT REASON, in the practice's own house style. One line the
// clinician reads before the consultation, as short as it can be while still
// saying what the patient wants and why.
export const REASON_RULES = [
  'One line, as short as possible — aim for under 25 words.',
  'Lowercase clinical shorthand: pt, req, bk’d, hx, inj, bilat, ?condition, sx, yday, tmrw, f/u, abx, meds.',
  'Durations as /7, /52, /12 for days, weeks and months — 3/52, not 3 wks.',
  'Separate clauses with semicolons; use commas within a clause.',
  'Order: what pt wants; main sx/problem (with duration or trend if relevant); relevant hx; other key context (pregnancy, breastfeeding, travel dates, deadlines).',
  'Spell out anything ambiguous — write "breastfeeding", not "bf".',
  'Keep a cause the patient gives as theirs (pt attributes to…), and any concern about a specific drug or interaction.',
  'No full sentences, no greetings, no emotional wording, no contact times, no repetition.',
  'Never include the patient’s name, NHS number, date of birth or address.',
];

// Worked examples of the finished line. Handed to the model as "example
// output" and shown on the house-style card, so the two cannot disagree.
export const REASON_LINE_EXAMPLES = [
  'pt req bloods bk’d sooner ?b12 def; worsening bilat tingling face/limbs; hx severe b12 def (inj); breastfeeding, travelling 22/10',
];
...
// THE REASON LINE IS NOT WRITTEN FROM A PLAIN QUESTION ANY MORE, and the card
// that did it is gone with the template that rendered it.
//
// It was reachable from ordinary Q&A: paste what a patient wrote, and a line to
// put in the appointment came back. That is the AccurX mode's job — the mode is
// armed deliberately, and it reads the message for WHERE THE REQUEST GOES as
// well as for the wording, so the same paste on this path produced the wording
// with nothing having decided the destination. The rules below are still handed
// to that mode (see accurxReadPrompt), and appointmentReasonAnswer still
// answers "how is a reason written" as the house style it is.
...
const REASON_EXAMPLES = [
  {
    ...
    to: 'pt req r/v heartburn; 3/52, worsening, gaviscon not helping; on clopidogrel, pt concerned re omeprazole interaction',
  },
  {
    ...
    to: 'pt req physio; knee pain 2/12 since fall from bike, pt attributes to twisting on landing',
  },
];
...
export function appointmentReasonAnswer() {
  return answer({
    ...
    subtitle: 'One line, lowercase shorthand, under 25 words',
    blocks: [
      bullets([
        'What the patient wants',
        'The main symptom or problem, with how long and which way it is going',
        'Relevant history',
        'Other key context — pregnancy, breastfeeding, travel dates, deadlines',
      ], 'In this order, semicolons between'),

      bullets([
        'Greetings, full sentences and the patient’s emotional wording',
        'Contact times and preferences',
        'Anything said twice',
      ], 'Drop'),
      ...
      fields([field('Example', REASON_LINE_EXAMPLES[0])], 'A full line'),

      // Where the line actually gets written. Plain Q&A does not do it: the
      // mode is armed deliberately, and it decides where the request goes as
      // well as how the line reads.
      note('**To have the line written for you, choose the AccurX mode** and paste the patient’s message into it. It comes back with the reason line to copy, and with where the request goes. Asking here gets the house style — this card — and not a written line.', 'info'),
    ],
