...
import {
  ...
  DIABETIC_NURSE_COVERS, DIABETIC_NURSE_REFUSES, NURSE_COVERS, NURSE_REFUSES, sendTo,
...
export function practiceNurseAnswer({ condition = '', task = null } = {}) {
  ...
  return answer({
    ...
    blocks: [
      fields([
        field('Send it to', sendTo('nurse'), { copy: true }),
        field('What for', what),
...
export function diabetesNurseAnswer({ condition = '' } = {}) {
  ...
  return answer({
    ...
    blocks: [
      fields([
        field('Send it to', sendTo('diabeticNurse'), { copy: true }),
        field('Days', 'Mondays, Wednesdays and Fridays'),
      ], 'Where this goes'),
      ...
      expand('What the diabetes nurse covers', [
        bullets(DIABETIC_NURSE_COVERS),
        bullets(DIABETIC_NURSE_REFUSES, 'What goes elsewhere'),
      ]),
    ],
    source: ['Triage guidelines', ...NURSE_SOURCES],
  });
...
export function diabetesGpAnswer({ condition = '', foot = false } = {}) {
  ...
  return answer({
    ...
    blocks: [
      ...
      expand('What the diabetes nurse does take', [bullets(DIABETIC_NURSE_COVERS)]),
    ],
    source: ['Triage guidelines', ...NURSE_SOURCES],
  });
}
