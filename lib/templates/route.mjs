...
import {
  BOOKING_RULES, CONSULTATION_NOTE_RULES, CONTINUITY_RULES, DOC_CODING_RULES, REASON_LINE_EXAMPLES, REASON_RULES,
 ...
 *
 * EVERY PAGE, WHOLE. There is no index mode and no per-page cap any more.
 * Both existed to keep one call cheap, and both did it by deciding, in
 * code and without telling anybody, which part of the practice's own
 * writing the model was allowed to see. A page cut at twelve thousand
 * characters is a page whose last section cannot be answered from, and an
 * index is a list of titles the model is invited to guess the contents of.
 * Both failure modes look exactly like a wrong answer.
 *
...
// a prompt that showed titles and withheld the pages invited the model to
// answer about a page it had not read.
export function proseSystemPrompt(notebookText) {
  ...
  return [
    ...
    notebook
      ? [
        ...
        '- Copy titles exactly as they appear in the headings. Never invent one.',
        '',
...
export function commandPrompt({ template, question, attached = '', decompose = false, notebook = '', images = 0 }) {
  ...
  if (template === 'accurxTriage') {
    ...
    return accurxReadPrompt({
      ...
      reasonExamples: REASON_LINE_EXAMPLES,
...
export function renderCommand(template, values = {}, question = '', { complaint = '', gist = '', route = null, images = 0, failed = '' } = {}) {
  if (template === 'accurxTriage') {
    ...
    return accurxAnswer({
      ...
      noAnswer: String(values.noAnswer || '').trim(),
...
/**
 * The Notebook page a turn is about to show, when it is a live typed page — an
 * e-RS referral, an email referral, a blood test set — whose card can be drawn
 * from the values saved on it. Null in every other case.
 *
 * The picker has already chosen; this only looks at WHAT it chose. So a page's
 * type can change how an answer is drawn and can never change which answer it
 * is.
 *
 ...
 * extracted; a page's type "forcing" one of those would be the notebook
 * overruling the referral card about what a referral card is.
 *
 * A DRAFT IS NOT DRAWN. Its boxes are not all filled in, and half a referral
 * card drawn like a whole one is worse than the page's own prose.
 *
 * @param {object} selection what the picker returned
 * @param {Array}  pages     the Notebook, as loaded for this turn
 * @returns {{page: object, kind: object}|null}
 */
export function typedNotebookPage(selection, pages = []) {
  const sel = selection || {};
  ...
  const page = (pages || []).find((p) => norm(p.docTitle) === norm(wanted))
    ...
    || null;
  if (!page || !isTypedKind(page.noteKind) || page.status === 'draft') return null;
  return { page, kind: noteKind(page.noteKind) };
}

/**
 * The card, with the typed page's screen drawn above the page.
 *
 * The screen is built in code from the page's saved, validated fields — no
 * model reads the page for it. Returns the card UNCHANGED when the screen
 * cannot be drawn, so the reader still gets the page.
 */
export function withKindCard(card, typed) {
  if (!card || !typed || !typed.page) return card;
  const block = renderKind(typed.page.noteKind, typed.page.fields);
