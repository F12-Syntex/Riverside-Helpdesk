...
import { bloodFormAnswer } from './bloods.mjs';
import { outputTag } from './output-tags.mjs';
import {
...
export const SELECTION_SCHEMA = z.object({
  ...
  // The blood form's one variable. It is what gets typed into Clinical Details
  // on the form itself, so it is taken in the reader's own words rather than
  // chosen from an enum: the practice writes "CVS" on one form and "NHS Health
  // Check" on the next, and neither is this schema's to correct.
  healthCheck: z.string().default('')
    .describe('For "bloodForm": the health check or review the bloods are for, in the fewest words — "NHS health check", "new patient health check", "diabetes review", "CVS". Empty when the message does not say which.'),
  ...
  // THE ROW IT READ, IN THE PRACTICE'S OWN WORDS.
  //
  // referralName above is deliberately the bare service — "dermatology" — and
  // that is what broke: the practice's dermatology page records five pathways
  // whose speciality and clinic type are identical, told apart only by a
  // qualifier, and the field asking for the fewest possible words is the field
  // that throws the qualifier away. Code then re-derived which of the five was
  // meant from that one word, by string matching, having never read the page as
  // prose — and sent a community referral to the first hospital that is NOT a
  // community one.
  //
  // The page is in the prompt in full. So the heading is copied out instead of
  // being reconstructed: naming what you just read is extraction, which is what
  // this model is for, and it leaves code with an exact string to match rather
  // than a word to interpret. Empty is normal and costs nothing.
  referralVariant: z.string().default('')
    .describe('For "referral": when the Notebook page records SEVERAL versions of the same referral — normal, telederm, community, 2WW — the exact heading or row name of the one being asked for, copied character for character from the page ("Normal Community Dermatology", "2WW Telederm"). Leave empty when the page records only one version, or when you are not working from a page.'),
...
/**
 * The Notebook in full, for the model to read rather than skim.
 *
 * The catalogue above gave the picker titles and a first line, which cannot
 * tell "Booking a blood test" from "Booking a diabetes blood test", and
 * cannot see a pairing written inside a page. The whole Notebook is about
 * 100k characters — 26k tokens — which one call reads comfortably, so the
 * bodies go in. The page is still rendered from the database, exactly as the
 * practice wrote it: the model reads it to CHOOSE, never to rewrite.
 *
 * Kept as one block with a stable shape and put at the very top of the
 * prompt, so the prefix is byte-identical from one question to the next and
 * a provider that caches prompt prefixes gets a hit on every turn after the
 * first. Anything that varies per message comes after it.
 *
 * Bodies are cut at a generous per-page cap so one runaway page cannot push
 * the rest out; the cap is above the longest page today.
 */
const PAGE_CHARS = 12000;
export function notebookFullText(pages = []) {
  return pages.map((page) => {
    const body = String(page.text || '').trim().slice(0, PAGE_CHARS);
    return `### ${page.docTitle}\n${body}`;
  }).join('\n\n');
}

/**
 * Whether the full text fits the picker's budget. Above this the catalogue
 * is used instead, so a Notebook that grows past what one call can read
 * degrades to the older, thinner prompt rather than failing every turn.
 */
export const NOTEBOOK_FULL_MAX_CHARS = 320000;
export const notebookFits = (pages = []) => pages.reduce((n, p) => n + String(p.text || '').length, 0) <= NOTEBOOK_FULL_MAX_CHARS;

/* --------------------------------------------- the prose turn’s prompt */
//
// When no template fits, the turn is written by the model rather than rendered
// from a card. These are the rules it writes under.
export const PROSE_SYSTEM_WITHOUT_NOTEBOOK = [
...
// THE SAME RULES, FOR A TURN THAT CAN SEE THE NOTEBOOK.
//
// The prose path used to be the one path with no practice material on it at
// all: the picker read the whole Notebook to choose a template, and a turn
// that fitted no template was then asked to write with nothing in front of it.
// A reader whose question was slightly off the shape of a card got "ask the
// practice manager" about something written down two clicks away.
//
// So the Notebook goes in here too, in full, on every turn that reaches the
// model. It sits at the END of the system prompt, which is itself the first
...
export const PROSE_SYSTEM_WITH_NOTEBOOK = [
  'You are the reception assistant for The Riverside Practice, a UK GP surgery. You are answering a member of practice staff — reception, admin, nursing, clinical or management.',
  '',
...
//
// `full` is false when the Notebook has outgrown what one call can read and
// only the page titles are available. An index is not the pages, so the prompt
// says which of the two it is rather than inviting the model to quote a page
// it has not been shown. No Notebook at all falls back to the no-access rules.
export function proseSystemPrompt(notebookText, full) {
  if (!notebookText) return PROSE_SYSTEM_WITHOUT_NOTEBOOK;
  ...
  return [
    PROSE_SYSTEM_WITH_NOTEBOOK,
    '',
    full
      ? 'THE NOTEBOOK, IN FULL. Every page the practice has written, exactly as it is saved.'
      : 'THE NOTEBOOK IS TOO LARGE TO SHOW IN FULL, so what follows is its INDEX: one line per page, the title and its opening line. Treat it as a list of what exists. You may say which page covers the question; you cannot quote what that page says, because you have not been shown it.',
    '"""',
    fenced,
    '"""',
  ].join('\n');
...
export function selectionPrompt({ question, attached = '', notebook = '', full = false, decompose = false, images = 0 }) {
  const fence = (t) => String(t || '').replace(/"{3,}/g, '""');
  return [
    // THE NOTEBOOK FIRST. Everything above it would vary per message and
    // break the cacheable prefix — see notebookFullText.
    notebook
      ? [
        full
        ...
        'RULES FOR USING IT',
        full
          ? '- READ THE PAGES, NOT JUST THE TITLES. Several pages have similar titles and differ only in their body; the one whose body actually answers the message is the one to name. A value the message needs — a speciality, a clinic type, a slot name, a number — that is written inside a page is what tells you that page is the answer.'
          : '',
        ...
        '- Copy titles exactly as they appear' + (full ? ' in the headings' : ' in the list') + '. Never invent one.',
        '',
    ...
    '   ALWAYS also fill in "referralName" with what is being referred to, in the fewest words and without the word "referral" itself. Do this even when referralService is "none", and ESPECIALLY then: the practice\'s own Notebook pages under "Referral pathways" (including "All Clinic types and their specialities") and its page of referrals sent by email are longer than the choices above and are checked by that name. Leaving it empty is how a referral that is written down somewhere gets reported as one nobody has a process for.',
    '   WHERE THE PAGE RECORDS SEVERAL VERSIONS of that referral — normal, telederm, community, 2WW — also copy the heading of the one being asked for into "referralVariant", character for character as the page writes it. "community dermatology referral" is "Normal Community Dermatology"; the bare "dermatology referral" is "Normal Dermatology". Leave it empty when the page records only one version. This is copying, not choosing a wording: a heading you have edited matches nothing.',
    '   "referral" WINS over "notebook" and over "fcp" whenever the message is about SENDING or MAKING a referral — "how to refer to physio", "physio referral", "refer for an ECG" — even when a Notebook page describes that very referral. Name the referral in referralName and the card reads the page\'s own speciality, clinic type and hospital. "fcp" is for a PATIENT with a musculoskeletal problem; staff asking how to refer somebody to physio are asking for "referral". "notebook" is for a referral page only when the message asks something the card cannot show, such as who books the follow-up.',
    ...
    '',
    '4. THE REASON FOR APPOINTMENT IS NOT WRITTEN ON THIS PATH. Plain Q&A never turns a patient’s pasted words into a reason line — that is the AccurX mode, which the reader arms deliberately, and it reads the message for where it goes as well as for the wording. So a patient’s own words pasted in here are a patient described to you: choose "triage", which says where they go. If the reader wants the line written, the answer is to choose the AccurX mode.',
    '',
    '5. "appointmentReasonRules" — the message ASKS how a reason for appointment is written. The house style, not a line written from somebody’s message.',
    '',
    ...
    '   THIS IS THE DEFAULT for any described symptom, INCLUDING first-person text a patient wrote and somebody pasted in. Where it goes is the answer here; the reason line is the AccurX mode’s job and is not written on this path.',
    ...
    '',
    '13. "bloodForm" — the message is about a BLOOD FORM: which tests to order or tick, what goes on the pathology request, how the form is filled in for a health check or a review — "what bloods for an NHS health check", "blood form for a new patient check", "which tests do I tick", "what goes in clinical details". Put the check or review it is for into "healthCheck", in the fewest words, and leave it empty when the message does not say which. This is the FORM being raised, not the result: a patient chasing results, or a doctor’s task about results, is not this.',
    '',
    images
...
/**
 * The Notebook page a turn is about to show, when the practice has tagged the
 * folder it sits in with a shape. Null in every other case.
 *
 * The picker has already chosen; this only looks at WHAT it chose. So a tag can
 * change how an answer is drawn and can never change which answer it is — the
 * property that makes tagging a folder safe for the practice to do without
 * thinking about routing.
 *
 * Only the `notebook` template, deliberately. Every other template already
 * renders in a shape of its own, decided in code from values the model
 * extracted; a tag "forcing" one of those would be a folder overruling the
 * referral card about what a referral card is.
 *
 * @param {object} selection what the picker returned
 * @param {Array}  pages     the Notebook, as loaded for this turn
 * @returns {{page: object, tag: object}|null}
 */
export function taggedNotebookPage(selection, pages = []) {
  const sel = selection || {};
  if (sel.template !== 'notebook') return null;
  const named = Array.isArray(sel.pages) ? sel.pages.slice(0, 2) : [];
  if (named.length !== 1) return null; // two pages are two answers; draw neither
  ...
  const page = (pages || []).find((p) => norm(p.docTitle) === norm(wanted))
    || (pages || []).find((p) => leaf(p.docTitle) === leaf(wanted))
    || null;
  if (!page || !String(page.text || '').trim()) return null;
  const tag = outputTag(page.outputTag);
  return tag ? { page, tag } : null;
}
  ...
  switch (sel.template) {
    ...
    case 'referral': {
      ...
      return referralAnswer({
        ...
        variant: String(sel.referralVariant || '').trim(),
        pages: notebookPages,
      });
    }
    ...
    // The Notebook goes in: the practice writes down the bloods for reviews
    // this file holds no filled-in form for, and "no form recorded" about
    // something they have written down is worse than a gap.
    case 'bloodForm':
      return bloodFormAnswer({ check: String(sel.healthCheck || '').trim(), pages: notebookPages });
