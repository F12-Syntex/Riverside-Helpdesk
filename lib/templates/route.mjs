 ...
 *
 * Neither command falls through to prose. That is the point of typing
 * one: /triage always answers with a triage, and /document with a
 * filing card, or with how documents are titled when there was nothing
 * in the message to build a title from.
 * ------------------------------------------------------------------ */
...
export function commandPrompt({ template, question, attached = '' }) {
  const fence = (t) => String(t || '').replace(/"{3,}/g, '""');
  const rules = template === 'documentCoding'
    ? [
 ...
 */
export function renderCommand(template, values = {}, question = '') {
  if (template === 'triage') {
    return triagePatientAnswer({
      condition: String(values.condition || '').trim() || question,
      text: question,
    });
  }
...
export function renderSelection(selection, question = '', notebookPages = []) {
  ...
  switch (sel.template) {
    ...
    case 'triage':
      return triagePatientAnswer({ condition: String(sel.condition || '').trim() || question, text: question });
    case 'pharmacyFirst':
      return pharmacyFirstAnswer({ condition: String(sel.condition || '').trim() || question, text: question });
    case 'fcp':
      return fcpAnswer({ condition: String(sel.condition || '').trim() || question, text: question });
