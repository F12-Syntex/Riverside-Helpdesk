...
import { notebookAnswer } from './notebook.mjs';
import {
...
export const SELECTION_SCHEMA = z.object({
  ...
  answerTitle: z.string().default('')
    .describe('For "notebook": a short heading for the card, at most six words, in sentence case.'),
  answer: z.string().default('')
    .describe('For "notebook": the answer itself, as markdown, written ONLY from the Notebook pages. Numbered steps for a procedure, short and direct. Empty for every other template.'),
  pages: z.array(z.string()).default([])
    .describe('For "notebook": the exact titles of the Notebook pages you used, copied as they appear. At most four.'),
...
export function selectionPrompt({ question, attached = '', notebook = '' }) {
  ...
  return [
    ...
    notebook
      ? [
        'THE PRACTICE NOTEBOOK — every page, in full. This is the ONLY thing you know about this practice. Nothing else you have ever read applies here: not how other surgeries work, not what is usual in the NHS, not what sounds sensible.',
        '',
        notebook,
        '',
        'RULES FOR USING IT',
        '- If the Notebook answers the message and no other template fits, choose "notebook" and write the answer from the pages. Combine pages freely when the answer needs more than one.',
        '- Write only what the pages say. If they cover part of the question, answer that part and say plainly what is not covered and who to ask.',
        '- If the Notebook does not cover it at all, choose "none". Do not answer it from general knowledge — an invented practice fact is worse than no answer, because it is indistinguishable from a real one.',
        '- List the exact titles of the pages you used in "pages", copied as they appear above. Never name a page you did not use, and never invent a title.',
        '- Do not write a telephone number, email address or name that is not on a page.',
        '',
      ].join('\n')
    ...
    '',
    '8. "notebook" — the Notebook answers it and none of the shapes above fit. Most questions land here: how something is done, what a service covers, who does what on which day. Write the answer in "answer", give it a heading in "answerTitle", and list the pages in "pages".',
    '',
 ...
 */
export function renderSelection(selection, question = '', knownPages = []) {
  const sel = selection || {};
  switch (sel.template) {
    case 'notebook':
      // Null when the model chose this and then wrote nothing, which is a
      // non-answer rather than an answer to show.
      return notebookAnswer({
        title: sel.answerTitle,
        markdown: sel.answer,
        pages: Array.isArray(sel.pages) ? sel.pages.slice(0, 4) : [],
        known: knownPages,
      });
    case 'referral': {
