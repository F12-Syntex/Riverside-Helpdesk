...
import { contractTemplateAnswer } from './contracts.mjs';
...
export const SELECTION_SCHEMA = z.object({
  ...
  // Naming the contract is extraction; which template implements it is a lookup
  // against PCIT's list. Same split as referralName above, for the same reason.
  contractName: z.string().default('')
    .describe('For "contractTemplate": the contract, service or EMIS template being asked about, in as few words as possible — "ADHD shared pathway", "simple wound care", "housebound winter vaccination", "OneTemplate NonPrescriber". Never write a template name out yourself; name what was asked about and it is looked up.'),
...
export function selectionPrompt({ question, attached = '', notebook = '', decompose = false }) {
  ...
  return [
    ...
    '   ALWAYS also fill in "referralName" with what is being referred to, in the fewest words and without the word "referral" itself. Do this even when referralService is "none", and ESPECIALLY then: two lists longer than the choices above are checked by that name — the Notebook page of referrals sent by email, and the NEL Referral Tree, which holds every referral form in North East London. Leaving it empty is how a referral that is written down somewhere gets reported as one nobody has a process for.',
    '   NEVER write a form name out yourself. Name the thing being referred for and the form is looked up and shown exactly as it is listed; a form name you compose is one that finds nothing when it is typed in.',
    '',
    ...
    '',
    '1b. "contractTemplate" — the message asks which EMIS TEMPLATE records a NEL contract or enhanced service, or what a named template is for, or whether one is ready yet: "which template for the ADHD shared pathway", "what template do I use for simple wound care", "is the housebound winter vacs template live", "what is OneTemplate NonPrescriber for". Fill in "contractName" with what was asked about.',
    '   IT IS A NARROW TEMPLATE AND THE NOTEBOOK OUTRANKS IT. It answers one thing: which EMIS template a contract is recorded on, and how far Primary Care IT had got building it. It is NOT about referring a patient — that is "referral" — and it is NOT about how this practice actually runs a service. "How do we do wound care", "who runs the phlebotomy clinic", "what does the care home service cover", "when are the housebound visits" are questions about the practice, and the practice\'s own Notebook answers them: choose "notebook". Naming a NEL contract is not enough on its own. Choose this ONLY when a template, a contract\'s paperwork, or the build status is what is actually being asked about — otherwise the reader gets a template name and a build status from a dated bulletin in place of the procedure they asked for.',
    '',
 ...
 * /document with a filing card (or with how documents are titled, when
...
// file is one document however many paragraphs it has, so /document is not here
...
// Two commands reach a model: /accurx and /document. (/practice searches the
// documents and never gets here.) /accurx returns above; what is left is the
// document one, and anything else arriving would be a command with no prompt of
// its own — which COMMAND_SCHEMAS has already refused to give a schema for.
export function commandPrompt({ template, question, attached = '', decompose = false, notebook = '' }) {
  const fence = (t) => String(t || '').replace(/"{3,}/g, '""');
 ...
 */
export function renderCommand(template, values = {}, question = '', { complaint = '', gist = '', route = null } = {}) {
  if (template === 'accurxTriage') {
...
export function renderSelection(selection, question = '', notebookPages = [], { complaint = '', gist = '' } = {}) {
  ...
  switch (sel.template) {
    ...
    // Null when PCIT's list has nothing by that name, which falls through to
    // prose rather than showing a card with an empty table on it.
    // The Notebook goes in, so the practice's own page about a service can win
    // over PCIT's row about the contract that pays for it. Null when neither has
    // anything, which falls through to prose rather than an empty card.
    case 'contractTemplate':
      return contractTemplateAnswer(String(sel.contractName || '').trim() || question, { pages: notebookPages });
    case 'registration': {
