...
export const COMMAND_SCHEMAS = {
  ...
  // /accurx. Two extractions on one call: the condition that titles the routing
  // card, and the wording that goes into whatever gets booked. Deliberately kept
  // as separate fields with separate descriptions rather than one merged
  // instruction — the model is doing two separate extractions, not one blended
  // one, and a condition that started summarising the booking notes would be a
  // routing card titled with somebody's interpreter request.
  //
  // The reason line and the booking notes are two lists on purpose, so the model
  // cannot solve the tension between them by putting "call after 2pm" into a
  // clinical shorthand line.
...
export const DECOMPOSING_COMMANDS = ['accurxTriage'];

// Two commands reach a model: /accurx and /document. (/practice searches the
// documents and never gets here.) So the second branch is the document one, and
// anything else arriving would be a command with no prompt of its own — which
// COMMAND_SCHEMAS above would already have refused to give a schema for.
...
export function renderCommand(template, values = {}, question = '', { complaint = '', gist = '', route = null } = {}) {
  if (template === 'accurxTriage') {
    ...
    return accurxAnswer({
      ...
      // What reading the WHOLE message made of where it goes, when that call
      // ran. It is checked against the patterns' own answer inside accurxAnswer
      // and can only ever raise it — see lib/templates/accurx-route.mjs.
      route,
