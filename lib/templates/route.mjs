...
export const COMMAND_SCHEMAS = {
  ...
  consultationNote: z.object({
    contact: z.string().default('')
      .describe('How the contact happened and who started it, in shorthand: "tel c/w pt", "pt attended desk", "pt msg via AccurX", "tel c/w daughter (with pt consent)", "tel c/w pt (call back)". Empty only if the note does not say.'),
    summary: z.string().default('')
      .describe('What it was about, in one short clause of clinical shorthand: the problem or request, how long, what has been tried, what was asked for. E.g. "sore throat 1/52, req abx". No name, no judgement, nothing the note did not say.'),
    actions: z.array(z.string()).default([])
      .describe('What was DONE, one item each, as terse facts with who and when where the note gives them: "booked tel appt Dr Okafor 03-Sep pm", "tasked secretaries re referral letter", "adv not prescribed without assessment". Only what the note says happened. Empty if nothing was done.'),
...
export function renderCommand(template, values = {}, question = '', { complaint = '', gist = '', route = null } = {}) {
  ...
  if (template === 'consultationNote') {
    ...
    if (!hasParts) return consultationNoteRulesAnswer();
  ...
  if (template === 'repeatMedication') {
    ...
    // is used, never an invented list.
    if (!groups.length) return repeatMedicationRulesAnswer();
