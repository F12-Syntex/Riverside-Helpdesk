  ...
  return answer({
    ...
    blocks: [
      note(named
        ...
        : 'More than one contract specification matches. Names and statuses are exactly as the Sitrep records them.', 'info'),
 ...
 *
 * Exported for lib/templates/lookup-command.mjs, which has to know whether the
 * practice's own page answered before it decides to try the other list: a
 * contract the practice has written about must not be crossed over to a
 * referral form because the Sitrep happens not to list it.
 */
export function notebookPageFor(name, pages = []) {
