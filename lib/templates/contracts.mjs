...
import { pageOnTemplate } from '../referrals/one-templates.mjs';
...
/* ------------------------------------------------------------------ *
 * WHERE ON THE TEMPLATE, not just which template.
 *
 * The Sitrep names a template and, in brackets, often a page —
 * "OneTemplate NonPrescriber (Treatment Room page)". A reader handed that
 * still has to open the template and hunt: which of its pages is Treatment
 * Room, and is the thing they came to record actually on it?
 *
 * PCIT's OneTemplate page specifications answer both, so the bracket is
 * resolved against them (lib/referrals/one-templates.mjs) and the card says
 * the launcher to press, the page to open and what is listed on it. Silent
 * when the page specifications do not have that template or that page: a
 * card may say less, never guess.
 * ------------------------------------------------------------------ */

// "OneTemplate NonPrescriber (Treatment Room page)" → the two parts of it.
const PAGE_IN_NAME = /^(.*?)\s*\(([^()]+?)\s*page\)\s*$/i;

export function splitTemplateName(name) {
  const m = PAGE_IN_NAME.exec(String(name || '').trim());
  if (!m) return { template: String(name || '').trim(), page: '' };
  return { template: m[1].trim(), page: m[2].trim() };
}

/** Where to find each named template's page, as far as PCIT's page lists say. */
function whereToFind(names = []) {
  const blocks = [];
  const seen = new Set();
  for (const name of names) {
    const { template, page } = splitTemplateName(name);
    if (!page) continue;
    const found = pageOnTemplate(template, page);
    if (!found || seen.has(`${found.template}>${found.page}`)) continue;
    seen.add(`${found.template}>${found.page}`);
    blocks.push(fields([
      field('Press', found.launcher || 'OneLauncher', { missing: 'OneLauncher' }),
      field('Then open', found.template),
      field('The page', found.heading ? `${found.heading} → ${found.page}` : found.page, { copy: true }),
    ], `Where to find it — ${found.page}`));
    if (found.detail) blocks.push(note(`What that page is for: ${found.detail}`, 'info'));
    if (found.entries.length) {
      blocks.push(bullets(found.entries, `Listed on the ${found.page} page`));
    }
  }
  return blocks;
}
...
function contractCard(contract, result) {
  return answer({
    ...
    blocks: [
      ...
      ...whereToFind(realTemplates(contract)),
  ...
  return answer({
    ...
    blocks: [
      ...
      pcit.length
        ...
        : null,
      note('All forty-two rows of the NEL Local Contract Specifications were searched, by contract name and by template name, including the other boroughs’ localisations — and then Primary Care IT’s OneTemplate page specifications, page by page, for a template that carries this even where no contract pays for it. Nothing else was searched: choosing Contract template means Primary Care IT’s documents and no others.', 'info'),
      bullets([
        'These lists cover the NEL contracts Primary Care IT build templates for, and the pages those templates carry — not every service the practice runs.',
