...
export function contractReasonedAnswer({
  contract, template = '', why = '', confident = false, sources = [], areas = PRACTICE_AREAS,
} = {}) {
  if (!contract) return null;
  const result = findContracts('', { areas });
  const card = contractCard(contract, result);
  const lead = [
    confident
      ? 'This contract was not named in the question, so it was worked out from what was asked.'
      : 'This contract was not named in the question. It is the best fit on the document, worked out '
        + 'from what was asked — check it before recording anything against it.',
    why,
  ].filter(Boolean).join(' ');
  const blocks = [
    note(lead, confident ? 'info' : 'warn'),
    template
      ? fields([field('Template page to open', template, { copy: true })], 'Where it is recorded')
      : null,
    sources.length ? bullets(sources, 'What that was based on') : null,
    ...card.blocks,
  ].filter(Boolean);
  return { ...card, subtitle: card.subtitle + ' — matched by meaning', blocks };
}
