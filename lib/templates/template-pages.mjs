...
const howToOpen = (row) => (row.launcher
  ? `Open ${row.launcher} (the F12 button), then ${row.template}.`
...
function pageCard(row, result) {
  ...
  return answer({
    title: row.page,
    subtitle: `A page on ${row.template}`,
    blocks: [
      fields([
        field('Template', row.template, { copy: true }),
        field('Open it from', row.launcher, { missing: 'OneLauncher' }),
        field('Page', row.page, { copy: true }),
...
function pageShortlistCard(result) {
  return answer({
    ...
    blocks: [
      ...
      table(['Page', 'Template', 'What it records'], result.matches.map((row) => [
        row.page,
        row.template,
...
function templateCard(template, result) {
  return answer({
    ...
    blocks: [
      ...
      table(['Page', 'What it records'], template.pages.map((p) => [
        p.name,
