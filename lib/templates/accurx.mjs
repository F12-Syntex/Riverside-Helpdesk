...
import { destinationLabel, signpostsFrom } from './accurx-route.mjs';
...
export function accurxAnswer({
  ...
  const panel = others.length > 1
    ? bullets(
      others.map((r) => {
        ...
        return '**' + r.what + '**' + who + goes + (r.note ? ' — ' + r.note : '');
      }),
  ...
  const conflicts = (verdict.conflicts || []).length
    ? note('**This will not book the usual way.** ' + verdict.conflicts.join(' '), 'warn')
    : null;
  ...
  return answer({
    ...
    blocks: hoist
      ? [where, ...why, ...wording, conflicts, flags, panel, ...blocks.slice(1), clinicNote(clinics), couldNotRead, ...wordingRules(), footer()]
      : [...blocks, handover, ...why, ...wording, conflicts, flags, panel, clinicNote(clinics), couldNotRead, ...wordingRules(), footer()],
...
function footer() {
  return note(
    'Where this goes was decided by **reading the whole message** against the practice’s own routes, and the account of that reading is on this card. There is no keyword matching underneath it: if the reading has misread the patient in front of you, route it yourself. The reason line only **rewrites** what the patient wrote — it judges nothing.',
    'warn',
  );
}
