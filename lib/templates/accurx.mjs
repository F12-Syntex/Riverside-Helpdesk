...
import { endsInAnEmergency, triagePatientAnswer } from './triage.mjs';
...
export function accurxAnswer({
  ...
  const routing = triagePatientAnswer({ condition, text, complaint });
  ...
  // it goes below everything the routing card has to say.
  const urgent = endsInAnEmergency({ condition, text });
  ...
  return answer({
    ...
    blocks: hoist
      ? [where, ...wording, ...blocks.slice(1), ...wordingRules(), footer()]
      : [...blocks, handover, ...wording, ...wordingRules(), footer()],
...
function footer() {
  return note('Where this goes is decided by the practice’s own triage order, from the whole message. The reason line only **rewrites** what the patient wrote — it judges nothing. Anything the safety checks found is shown **above** this card.', 'warn');
}
