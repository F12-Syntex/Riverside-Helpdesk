...
import {
  clinicianAnswer, endsInAnEmergency, eyeEmergencyAnswer, interruptAnswer, raisedAnswer, raisedEmergencyAnswer, triagePatientAnswer,
} from './triage.mjs';
import { applyRoute, destinationLabel, rankOfDestination, signpostsFrom } from './accurx-route.mjs';
import { BOOKING_RULES, REASON_RULES } from './writing.mjs';

// Where "book it" stops and "go now" starts, on the ladder in
// lib/triage/destinations.mjs. Everything at this rank or above — the duty
// doctor interrupt, A&E under their own steam, the eye A&E, 999 — is somebody
// moving immediately, and none of them is an appointment.
const URGENT_RANK = 5;
...
function raisedCard(decided, pattern, condition) {
  ...
  // Somebody stands up and walks. It is not a booking, so it must never render
  // as one — see interruptAnswer, and see `urgent` below, which takes the Copy
  // off the wording for the same reason.
  if (decided.destination === 'dutyInterrupt') {
    return interruptAnswer({ condition, because: decided.because, wouldHave });
  }
...
export function accurxAnswer({
  ...
  // The whole message goes down with it. The card is DECIDED from the routed
  // request, but the checks that rule a service OUT — a possible pregnancy, a
  // red flag — are message-wide, and the request they are written in is very
  // often not the request the card is about.
  const pattern = triagePatientAnswer({ condition, text, complaint, whole: message || text });
  ...
  const urgent = rankOfDestination(decided.destination) >= URGENT_RANK
  ...
  return answer({
    ...
    destination: decided.destination,
    blocks: hoist
      ? [where, ...why, ...wording, ...blocks.slice(1), clinicNote(clinics), ...wordingRules(), footer(decided.raised)]
      : [...blocks, handover, ...why, ...wording, clinicNote(clinics), ...wordingRules(), footer(decided.raised)],
