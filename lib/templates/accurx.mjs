 ...
 * that answers that: who we tried to book them with, and what for. Written
 * from what the card already decided, with no model in the path.
 */
...
export function accurxAnswer({
  ...
  const view = {
    ...
    // unanswered booking call to record on one.
    noAnswer: urgent ? '' : noAnswerNote({
      destination: decided.destination,
      reason,
