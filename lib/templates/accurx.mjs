...
import { destinationCard, howTo, meansNow } from './route-card.mjs';
import { signpostsFrom } from './accurx-route.mjs';
import { spanWithin } from '../safety/spans.mjs';
import {
  APPOINTMENT_MODE_DEFAULT, APPOINTMENT_MODE_RULES, APPOINTMENT_MODE_TEST,
  appointmentMode, coveredBy, destination as destinationById, needsAppointmentMode, refusedBy,
} from '../triage/destinations.mjs';
import { BOOKING_RULES, CONTINUITY_RULES, REASON_RULES } from './writing.mjs';
...
// WHICH KIND OF DOCTOR APPOINTMENT — TELEPHONE OR FACE-TO-FACE.
//
// THE CARD THAT MADE THIS NECESSARY was every GP card the app has ever
// rendered. "A GP appointment here" is not one thing to book: the practice's own
// guide calls it "a booked telephone or face-to-face appointment", and the duty
// doctor page opens its actioning steps with "decide whether an examination is
// likely — no means a telephone consultation, yes means a face-to-face slot".
// That is an instruction to make a decision, printed on a card that gave no help
// with making it, to somebody with a queue behind them.
//
// So the reading answers it and the card says which, with the practice's own
// test folded away underneath. It is NOT a routing decision and cannot become
// one: the destination panel is built before this runs, nothing here is passed
// to it, and this renders only for the two destinations that end in a booked
// appointment with a doctor (needsAppointmentMode). A pharmacist is not the
// practice's appointment book.
//
// NO COPY ON IT, for the reason continuityBlocks gives: two Copy buttons on one
// card are two the reader can tell apart at a glance, and a third on two words
// they are about to pick from a dropdown costs the other two that.
//
// AND AN "UNSURE" READING IS SAID OUT LOUD RATHER THAN ROUNDED OFF. The guide's
// tie-break is that in doubt means face-to-face — so it books the same slot a
// confident face-to-face books, and the note says nothing in the message decided
// it. A card that presented a coin toss as a judgement would be worse than one
// that never asked.
//
// AND NOTHING AT ALL WHERE NOTHING WAS READ. A turn with no key, a timeout or a
// refusal lands on the duty doctor because that is what the practice does with
// what it cannot place — not because anything about the message was assessed,
// and the card says so in its own words. A slot type underneath that would be a
// second hedge dressed as an answer, worked out from a reading that never
// happened. Silence is the honest output; see couldNotRead in accurxAnswer.
function appointmentBlocks({ mode, why, destination, bookable, read }) {
  if (!read || !bookable || !needsAppointmentMode(destination)) return [];

  const chosen = appointmentMode(mode);
  const said = String(why || '').trim();

  return [
    fields([
      field('Book as', chosen.label),
      said ? field('Because', said) : null,
    ], 'Which kind of appointment'),

    note(
      chosen.hedged
        ? '**Nothing in the message decided this.** ' + APPOINTMENT_MODE_DEFAULT
          + ' If you can see from the message that a call would do, it is your call to make.'
        : '**' + chosen.book + '.** ' + APPOINTMENT_MODE_TEST
          + ' If the patient tells you something the message did not, book what that says instead.',
      chosen.hedged ? 'warn' : 'info',
    ),
  ];
}

// HAS SOMEBODY ALREADY DEALT WITH THIS, AND SO WHO SHOULD IT BE BOOKED WITH.
//
// The third panel, and the last question of the three: where it goes, what the
// line says, and — now — who the appointment is with. It is deliberately the
// same shape as the two above it, a labelled panel of short values with its
// rules folded away underneath, because it is read in the same second as they
// are by somebody who has a queue behind them.
//
// It never moves the destination and it cannot: the routing panel is built
// before this runs and nothing here is passed to it. A follow-up about the same
// knee still goes wherever the reading sent it — this only says that the person
// it goes to should be the one who saw the knee.
//
// NO COPY ON IT, which is a deliberate exception to the exception. This card
// already carries two Copy buttons and says so in wordingBlocks: one for the
// task that passes the patient on and one for the appointment itself. A third
// on a clinician's name — three or four words the reader is about to type into a
// booking screen anyway — would cost the other two the thing that makes them
// worth having, which is that the reader can see at a glance which value they
// came for.
function continuityBlocks({ seen, message, bookable }) {
  const said = seen || {};
  const who = String(said.who || '').trim();
  const when = String(said.when || '').trim();
  const what = String(said.what || '').trim();

  // The quote is what makes this checkable, and it is checked exactly as the
  // routing evidence is: a panel that could not prove its words keeps the panel
  // and loses the quote rather than telling reception the patient wrote
  // something they did not.
  const quote = String(said.evidence || '').trim();
  const because = quote && spanWithin(quote, message) ? quote : '';

  // NOTHING FOUND IS SILENCE, NOT A FINDING — the same rule the booking notes
  // learned the hard way. "Nothing in this message says it has been dealt with
  // before" under a first-time request is a sentence reception reads as a
  // negative somebody established, on a card where the model was simply told to
  // leave the field empty when there was nothing in it.
  if (!who && !when && !what && !because) return [];

  // Told to come back, or seen before, with nobody named. Still worth the panel:
  // who dealt with it is in the record, and knowing to go and look is most of
  // the job. What it must not do is name somebody to make the panel look
  // complete.
  const named = who
    ? who + (when ? ' — ' + when : '')
    : (when ? 'not said who — ' + when : 'not said who');

  return [
    fields([
      field('Dealt with before', named),
      what ? field('What came of it', what) : null,
      // The action, and it is only a row where there is an appointment to book
      // AND somebody at this practice to book it with. "Book back with the
      // hospital" is not a thing reception can do, and a Book with row that
      // resolves to "not here" is a row that has to be read before it can be
      // dismissed. Where it was somebody else's, the panel says so in the note
      // below instead and asks for nothing.
      bookable && said.here
        ? field('Book with', who ? who + ' if they have a slot' : 'whoever dealt with it — check the record')
        : null,
    ], bookable ? 'Already been dealt with' : 'Already been dealt with — for the handover'),

    // WHAT THIS PANEL HAS AND HAS NOT DONE, in the place the reader is deciding
    // what to do about it. Nothing here has seen the record: it is the patient's
    // account of their own earlier contact, which is worth acting on and is not
    // the same thing as a consultation somebody can look up. The quote is the
    // whole of the evidence, so it goes in this sentence rather than a line down.
    note(
      (because ? 'The patient’s own words: “' + because + '”. ' : '')
      + (said.here
        ? (bookable
          ? 'Find that consultation in the record before you book, and book it back with whoever held it — the same problem twice is one consultation continued, not two.'
          : 'Say that they have been seen about this before when you hand it over — whoever takes it can find the consultation.')
        : 'That was not this practice, so there is nobody here to book it back with. It is history for the clinician to read.')
      + ' This is what the **message** claims; nothing here has read the record.',
      'info',
    ),
  ];
}
...
// `slotType` is only true where the telephone-or-face-to-face panel rendered:
// the practice's test is a disclosure OF that panel, and on a card going to the
// pharmacy it would be an answer to a question nobody asked.
const wordingRules = (slotType = false) => [
  expand('How the reason was written', [bullets(REASON_RULES)]),
  expand('What belongs in a booking note', [bullets(BOOKING_RULES)]),
  expand('How earlier contact was looked for', [bullets(CONTINUITY_RULES)]),
  slotType ? expand('How telephone or face-to-face was decided', [bullets(APPOINTMENT_MODE_RULES)]) : null,
];
...
// WHO THE CALL WAS TO BOOK WITH, in the shorthand an EMIS entry uses. Only the
// destinations the practice books into itself: anywhere else the call was to
// point the patient elsewhere, and the note says that instead.
const BOOKED_WITH = {
  gp: 'GP',
  dutyDoctor: 'duty dr',
  doctorTask: 'GP',
  nurse: 'practice nurse',
  diabeticNurse: 'diabetes nurse',
  fcp: 'FCP',
  pharmacyTeam: 'practice pharmacist',
  socialPrescriber: 'social prescriber',
};

/**
 * The line for the consultation when the patient did not pick up.
 *
 * AccurX is where the booking call is made from, so an unanswered call leaves
 * nothing on the record — and the colleague the patient rings back reaches a
 * patient saying "you called me" with no idea why. This is the short entry
 * that answers that: who we tried to book them with, and what for.
 *
 * THIS IS THE FALLBACK. The line on the card is the reading's own (`noAnswer`
 * in ./accurx-route.mjs), because what the call was for is rarely only the
 * reason line. This one is built from what the card decided, with no model in
 * the path, for a turn where the reading gave nothing back.
 */
export function noAnswerNote({ destination = '', reason = '', mode = '', sendTo = '' } = {}) {
  const what = String(reason || '').trim().replace(/[.;,\s]+$/, '');
  const re = what ? ' re: ' + what : '';
  const with_ = BOOKED_WITH[destination];
  if (with_) {
    const slot = mode === 'telephone' ? 'tel appt' : mode === 'faceToFace' ? 'F2F appt' : 'appt';
    return 'Tried to call pt to book ' + slot + ' with ' + with_ + re + '. No answer.';
  }
  const to = String(sendTo || '').trim();
  return 'Tried to call pt' + re + (to ? ', to signpost to ' + to : '') + '. No answer.';
}
...
export function accurxAnswer({
  ...
  reason = '', details = [], booking = [], noAnswer = '',
  route = null, message = '',
} = {}) {
  ...
  // WHICH KIND OF APPOINTMENT, on the two routes that book one with a doctor.
  // Read from the same verdict as everything else, gated on the destination the
  // card actually settled on rather than on what the reading named — a card that
  // ended up somewhere else must not carry a slot type decided for somewhere it
  // is not sending anybody. Empty everywhere else; see appointmentBlocks.
  const slot = appointmentBlocks({
    mode: (verdict.appointment || {}).mode,
    why: (verdict.appointment || {}).why,
    destination: decided.destination,
    bookable: !urgent,
    read: !unread,
  });

  // WHO IT SHOULD BE BOOKED WITH, when the message says somebody has already
  // dealt with it. Read from the same verdict as everything else on this card,
  // checked against the WHOLE message like every other quote here, and never
  // allowed near `destination` — see continuityBlocks.
  const continuity = continuityBlocks({
    seen: verdict.seenBefore,
    message: [said, whole].filter(Boolean).join('\n'),
    bookable: !urgent,
  });
  ...
  // THE SAME CARD, AS DATA, for the AccurX view (app/_components/chat/
  // AccurxCard.jsx). The blocks above are still the card's record: they are
  // what the question log stores and what every test reads. But drawn as a
  // stack they put a paragraph between reception and each of the three things
  // they came for (where it goes, the line to copy, and which slot), so the
  // view draws those three large and folds the rest away. Nothing here is
  // decided afresh: every value is one the blocks were built from, under the
  // same gates (urgent cards book nothing, a quote is only shown when the
  // patient wrote it, empty lists say nothing).
  const place = destinationById(decided.destination) || {};
  const chosen = slot.length ? appointmentMode((verdict.appointment || {}).mode) : null;
  const seen = verdict.seenBefore || {};
  const seenQuote = String(seen.evidence || '').trim();
  const seenAny = String(seen.who || seen.when || seen.what || seenQuote || '').trim();
  const view = {
    destination: decided.destination,
    label: place.label || routing.subtitle,
    sendTo: place.sendTo || routing.subtitle,
    rank: place.rank || 0,
    urgent,
    copySendTo: !(decided.destination === 'emergency' || decided.destination === 'dutyInterrupt'),
    unread,
    because,
    reasoning: String((route || {}).reasoning || '').trim(),
    ruledOut: (Array.isArray((route || {}).ruledOut) ? route.ruledOut : [])
      .filter((r) => r && r.why).map((r) => ({ label: r.label || r.id, why: r.why })),
    reason: String(reason || '').trim(),
    details: (details || []).filter(Boolean).slice(0, 5),
    booking: (booking || []).filter(Boolean).slice(0, 5),
    appointment: chosen ? {
      label: chosen.label,
      hedged: !!chosen.hedged,
      why: String((verdict.appointment || {}).why || '').trim(),
      rule: chosen.hedged ? APPOINTMENT_MODE_DEFAULT : APPOINTMENT_MODE_TEST,
    } : null,
    seen: continuity.length && seenAny ? {
      who: String(seen.who || '').trim(),
      when: String(seen.when || '').trim(),
      what: String(seen.what || '').trim(),
      here: !!seen.here,
      quote: seenQuote && spanWithin(seenQuote, [said, whole].filter(Boolean).join('\n')) ? seenQuote : '',
    } : null,
    requests: others.length > 1 ? others.map((r) => ({
      what: String(r.what || ''),
      who: String(r.who || ''),
      goes: String(r.label || ''),
      note: String(r.note || (r.label ? '' : 'not placed — your call')),
    })) : [],
    flags: (verdict.flags || []).filter(Boolean),
    conflicts: collisions.filter(Boolean),
    clinics: clinics.map((c) => ({ label: c.label, because: c.because || '' })),
    steps: howTo(decided.destination),
    // Nobody rings to book off a card that means now, so there is no
    // unanswered booking call to record on one. Otherwise the reading's own
    // line, which has read the whole message and knows everything the call was
    // for; the fixed line only when there is no reading to take it from.
    noAnswer: urgent ? '' : String(noAnswer || '').trim() || noAnswerNote({
      destination: decided.destination,
      reason,
      mode: chosen && !chosen.hedged ? chosen.id : '',
      sendTo: place.sendTo || routing.subtitle,
    }),
    covers: coveredBy(decided.destination),
    refuses: refusedBy(decided.destination),
  };
  ...
  return {
    ...answer({
      ...
      destination: decided.destination,
      // The continuity panel sits with the wording rather than with the routing,
      // because it is answering the booking question and not the routing one: what
      // to type, then who to type it against. On a card nobody books from it keeps
      // the same place — it is history for whoever takes the handover, and it is
      // named as that in continuityBlocks rather than moved.
      // The slot type sits directly under where it goes and above the reason line,
      // because that is the order reception works in: which doctor list, which
      // kind of slot, then what to type into it. It is empty on every card that is
      // not booking a doctor's appointment, so on those the card is unchanged.
      blocks: hoist
        ? [where, ...why, ...slot, ...wording, ...continuity, conflicts, flags, panel, ...blocks.slice(1), clinicNote(clinics), couldNotRead, ...wordingRules(slot.length > 0), footer(why.length > 0)]
        : [...blocks, handover, ...why, ...slot, ...wording, ...continuity, conflicts, flags, panel, clinicNote(clinics), couldNotRead, ...wordingRules(slot.length > 0), footer(why.length > 0)],
