...
import { cleanCriteria } from '../triage/criteria.mjs';
import {
...
export function accurxAnswer({
  ...
  const view = {
    ...
    // Why, in the fixed vocabulary of lib/triage/criteria.mjs. Nothing on an
    // unread card: no reading happened, so nothing decided anything.
    criteria: unread ? [] : cleanCriteria((route || {}).criteria),
...
function justification(route) {
  ...
  const criteria = cleanCriteria(said.criteria);
  const ruledOut = Array.isArray(said.ruledOut) ? said.ruledOut.filter((r) => r && r.why) : [];
  if (!reasoning && !criteria.length && !ruledOut.length) return [];
  ...
  return [
    criteria.length ? bullets(criteria.map((c) => '**' + c.label + '**' + (c.value ? ' — ' + c.value : '')), 'Why here') : null,
    reasoning ? note('**' + (criteria.length ? 'In full.' : 'Why here.') + '** ' + reasoning, 'info') : null,
    ruledOut.length
