...
import { clinicianAnswer, endsInAnEmergency, raisedEmergencyAnswer, triagePatientAnswer } from './triage.mjs';
...
export function accurxAnswer({
  ...
  const routing = decided.raised
    ? (decided.destination === 'emergency'
      ? raisedEmergencyAnswer({ condition, because: decided.because })
      : clinicianAnswer({
        condition,
        sameDay: decided.destination === 'dutyDoctor',
        wouldHave: destinationLabel(pattern.destination || ''),
