...
function justification(route) {
  ...
  if (!reasoning && !ruledOut.length) return null;

  return expand('Why this, and not something else', [
    reasoning ? note(reasoning, 'info') : null,
    ruledOut.length
      ? bullets(ruledOut.map((r) => `**${r.label || r.id}** — ${r.why}`), 'Considered and not chosen')
      : null,
