...
const ERS_STEPS = [
  ...
  'Pick the hospital — the first listed, unless the doctor said otherwise.',
...
// Where the referral goes, as opposed to what the reader types. Behind a
// disclosure because it is reference, not an instruction: a long pathway string
// sitting between the clinic type and the steps was read as another box to fill
// in, which is exactly what it is not.
const detailExpander = (svc) => (svc.hospital || svc.pathway || svc.form
  ? expand('Hospital, pathway and form', [
    fields([
