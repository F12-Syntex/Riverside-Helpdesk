...
import { findPathwayReferral } from '../referrals/pathways.mjs';
...
export function referralAnswer({ question = '', service = null, name = '', pages = [] } = {}) {
  ...
  const recorded = findPathwayReferral({ name, question, pages });
  if (recorded) return recorded.route === 'email' ? emailReferral(recorded) : ersReferral(recorded);
