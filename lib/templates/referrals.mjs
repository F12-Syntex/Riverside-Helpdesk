...
import { findPathwayReferrals } from '../referrals/pathways.mjs';
...
export function referralAnswer({ question = '', service = null, name = '', pages = [] } = {}) {
  ...
  const found = findPathwayReferrals({ name, question, pages });
  if (found.length) {
    ...
    const key = (e) => [e.specialty, e.clinicType, e.hospital, e.hospitalRule].map((s) => String(s || '').toLowerCase()).join('|');
