...
function ersReferral(svc) {
  return answer({
    ...
    blocks: [
      ...
      // Read off the practice's own Referral pathways page rather than out of
      // the array in this file, so the reader can see WHERE the pairing came
      // from and check it against the page.
      svc.fromNotebook && svc.page
        ? note(`Speciality and clinic type are as the practice records them under Referral pathways: <mark>“${svc.page.replace(/^Notebook:\s*/i, '')}”</mark>.`, 'info')
        : null,
