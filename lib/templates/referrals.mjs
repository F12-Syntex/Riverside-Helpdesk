 ...
 */
function nelFormCard(form, result) {
  const areas = areaNamesFor(form);
  const everywhere = areas.length === 5;
  return answer({
    title: form.name,
    subtitle: `On the NEL Referral Tree — ${form.category}`,
    blocks: [
      fields([
...
function nelElsewhereCard(result) {
  return answer({
    ...
    blocks: [
      note(`The Referral Tree lists ${result.elsewhere.length === 1 ? 'a form' : 'forms'} by this name for other North East London areas only, so ${result.elsewhere.length === 1 ? 'it' : 'they'} will not appear on this practice’s tree.`, 'warn'),
