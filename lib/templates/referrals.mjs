...
export const REFERRAL_SERVICES = [
  ...
  // Speciality is stored bare and the 2WW prefix is applied from `cancer` — see
  // ersReferral. The clinic type is left empty on purpose: the practice records
  // that it is "the 2WW skin option", which is a description of where to look,
  // not a value anybody can type into e-RS.
  { name: 'Suspected skin cancer (2WW)', route: 'ers', aliases: ['skin cancer', '2ww skin', 'melanoma'], specialty: 'Dermatology', clinicType: '', priority: '2WW', cancer: true, note: 'Normally goes through Telederm. Message the patient the summary ONLY for a 2WW referral.' },
...
// On a two week wait referral the speciality itself is prefixed — the reader
// picks "2WW Dermatology", not "Dermatology". Applied here from `cancer` rather
// than typed into each row, so a cancer service added later cannot be recorded
// with a bare speciality and quietly send the referral down the routine list.
function ersSpecialty(svc) {
  if (!svc.specialty || !svc.cancer) return svc.specialty;
  return /^2ww\b/i.test(svc.specialty) ? svc.specialty : `2WW ${svc.specialty}`;
}
