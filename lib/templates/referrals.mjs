...
import { answer, bullets, ers, expand, field, fields, note, profMessage, steps, table } from './blocks.mjs';
import { PRACTICE_AREA, areaNamesFor, findReferralForms } from '../referrals/nel-tree.mjs';
import { findPathwayReferrals, pairingOf } from '../referrals/pathways.mjs';
...
//
// `emailLine` is the one extra sentence the receiving service is owed on an
// emailed referral, where the practice records one. Most do not need it: the
// attachment says what the referral is, and a sentence saying it again is a
// sentence somebody has to read before they can get to the work.
export const REFERRAL_SERVICES = [
  ...
  { name: 'Minor surgery', route: 'email', to: 'NELONDONICB.NIGHTINGALEPRACTICE@NHS.NET', org: 'The Nightingale Practice', emailLine: 'Please could this patient be added to your minor surgery list.', note: 'Minor surgery is NOT general surgery.' },
  { name: 'Occupational therapy (OT)', route: 'email', aliases: ['ot'] },
  { name: 'District nurse / community nurse', route: 'email', aliases: ['dn', 'acn', 'adult community nursing'], form: 'RP ACN 2022', emailLine: 'The documents are attached as one PDF, most recent first.', note: 'Order the documents latest to earliest, print the set to PDF, then email the PDF.' },
 ...
 *
 * WHOLE WORDS ONLY. This used to be a substring test, and "physiotherapy"
 * contains "ot" — so a question about physio came back as an Occupational
 * Therapy card. A name has to appear as its own word or words now, which is
 * the only way a two-letter alias can be safe to keep.
 */
export function findReferralService(query) {
  ...
  for (const svc of REFERRAL_SERVICES) {
    for (const name of [svc.name, ...(svc.aliases || [])]) {
      const n = norm(name);
      if (!n) continue;
      const whole = new RegExp('\\b' + n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b');
      if (whole.test(q) && (!best || n.length > best.len)) best = { svc, len: n.length };
    }
  }
...
const ersSteps = (svc) => [
  ...
  'Fill the screen in as shown above. Referring clinician is **the doctor who created the task** — if it came from someone who is not a doctor, use **Dr Goel**.',
  svc.hospitalRule
    ? `Hospital: ${svc.hospitalRule} — **not** simply the first one listed.`
...
const EMAIL_STEPS = [
  ...
  'Copy the wording above into the message.',
  '**Send**.',
  'On My Inbox, **Message → Mark done**.',
];

/* -------------------------------------------- the wording, for the emails */
//
// An emailed referral is the same message every time, and every time somebody
// writes it again from nothing at the front desk. So the card writes it: the
// greeting and the sign-off are constant, and the ONE line in the middle names
// the referral. That is the whole adjustment between one service and the next,
// which is why this is ten lines of code and not a wording library.
//
// BUILT IN CODE, NEVER BY A MODEL. This text leaves the practice over the
// practice's name and goes to another organisation, so no part of it may be
// composed by something that could be inventive about it.
//
// It carries NO PATIENT DETAIL. The record is attached, so a name or a date of
// birth typed into the wording is a second copy of what is already there — and
// the copy is the one that can be wrong, and the one that ends up in the wrong
// mailbox.
const GREETING = 'Dear Colleague,';
const SIGN_OFF = 'Thanks,\nThe Riverside Practice';

// The service, said the way a sentence needs it. The practice's own name for
// the referral, with only the parts that are not part of a name taken off:
// its gloss ("OT (Occupational Therapy)" → "OT"), the second of two names
// ("District nurse / community nurse" → "District nurse"), and the reader's
// trailing "referral" ("Gym referral" → "Gym"). Nothing is reworded: the
// practice's wording is the wording that goes out.
function emailedThing(name) {
  return String(name || '')
    .replace(/\([^)]*\)/g, ' ')
    .split('/')[0]
    .replace(/\s*referrals?\s*$/i, '')
    .replace(/\s+/g, ' ')
    .trim();
}

// "an ECG", "a CAMHS". An acronym takes the article its FIRST LETTER SOUNDS
// like when it is read out — "an F", "an S" — which is not the article its
// spelling would suggest, and getting it wrong is the kind of small wrongness
// that makes a practice's email look automated.
const SAID_AS_VOWEL = 'AEFHILMNORSX';
function articleFor(phrase) {
  const first = String(phrase || '').trim().split(/\s+/)[0] || '';
  if (!first) return 'a';
  if (/^[A-Z0-9][A-Z0-9-]+$/.test(first)) return SAID_AS_VOWEL.includes(first[0]) ? 'an' : 'a';
  return /^[aeiou]/i.test(first) ? 'an' : 'a';
}

/**
 * The email that goes with an emailed referral, ready to send.
 *
 * Four lines, because four lines is what the job needs: who it is to, what is
 * attached, anything the receiving service has asked the practice to say, and
 * who it is from.
 */
export function referralEmailBody(svc = {}) {
  const thing = emailedThing(svc.name);
  const attached = thing
    ? `Please find attached ${articleFor(thing)} ${thing} referral for this patient.`
    : 'Please find attached a referral for this patient.';
  return [GREETING, '', attached, ...(svc.emailLine ? ['', svc.emailLine] : []), '', SIGN_OFF].join('\n');
}

// The send itself, shut by default. Most staff have done it a hundred times
// and came for the two boxes; the ones who have not open it.
const stepsExpander = (svc) => expand('Full steps on e-RS', [steps(ersSteps(svc))]);
...
// WHAT THE PAGE DOES NOT RECORD, SAID ON THE CARD.
//
// A referral card with an empty Speciality box reads, to somebody in a hurry,
// as a referral with nothing to set — which is exactly what the emailed cards
// mean by an empty box. So where a required value is missing the card names
// it, in the words of the thing the reader would have typed, and the rest of
// the card still stands: the page records the route and the hospital even
// when it never got round to the clinic type, and three quarters of an answer
// beats being told to ask the secretaries.
function gapsNote(svc) {
  const gaps = (svc.gaps || []).filter(Boolean);
  if (!gaps.length) return null;
  const said = gaps.length === 1
    ? gaps[0]
    : gaps.slice(0, -1).join(', ') + ' and ' + gaps[gaps.length - 1];
  return note(`The practice’s page does not record ${said}. Everything else below is what the page says; take that from the doctor’s task or ask the secretaries.`, 'warn');
}
...
function ersReferral(svc) {
  return answer({
    ...
    blocks: [
      gapsNote(svc),
      // NEITHER OF US CHOSE THIS ONE. Two pathways on the page fit every word
      // the reader wrote and differ in what gets typed into e-RS, so the card
      // that shows one of them and nothing else is a coin toss presented as an
      // answer. Naming the other and what separates them costs two lines and is
      // the difference between a reader who checks and a reader who cannot.
      svc.ambiguous && (svc.alternatives || []).length
        ? note('The page records more than one version of this referral and the message does not say which. Check it is this one — the other'
          + ((svc.alternatives.length === 1 ? ' is' : 's are') + ': ')
          + svc.alternatives.map((a) => `**${a.name}**` + (a.hospitalRule || a.hospital ? ' — ' + (a.hospitalRule || a.hospital) : '')).join('; ') + '.', 'warn')
        : null,
      // The e-RS screen itself, filled in. The hospital and service go on it
      // too: when the practice records one, the first hospital listed is
      // usually the wrong one. Where the pairing came from is the card's
      // source line, not a sentence above the steps.
      ers({
        priority: svc.cancer ? '2WW' : (svc.priority || 'Routine'),
        specialty: ersSpecialty(svc),
        clinicType: ersClinicType(svc),
        hospital: svc.hospital || '',
        hospitalRule: svc.hospitalRule || '',
        pathway: svc.pathway || '',
      }),
      svc.note ? note(svc.note, 'warn') : null,
      // The same page records another clinic under the same name — standard
      // physio and ESP. The first is shown, as the page intends; the other is
      // one line so the reader who was asked for it knows where it is.
      (svc.alternatives || []).length
        ? note('Also on this page: ' + svc.alternatives.map((a) => `**${a.name}** — ${[a.specialty && 'Speciality ' + a.specialty, a.clinicType && 'Clinic type ' + a.clinicType, a.hospital, a.hospitalRule].filter(Boolean).join(', ')}`).join('; ') + '.', 'info')
        : null,
      stepsExpander(svc),
      formExpander(svc),
      letterExpander(),
    ...
    source: svc.fromNotebook && svc.page
      ? ['Standard referral flow (e-RS)', svc.page]
      : ['Standard referral flow (e-RS)', `${svc.name} referral`],
  });
}
    ...
    blocks: [
      ...
      svc.ambiguous && (svc.alternatives || []).length
        ? note('The page records more than one version of this referral and the message does not say which. Check it is this one — the other'
          ...
          + svc.alternatives.map((a) => `**${a.name}**` + (a.hospitalRule || a.hospital ? ' — ' + (a.hospitalRule || a.hospital) : '')).join('; ') + '.', 'warn')
        : null,
      // The window, written. It replaced a panel of labels — Send to,
      // Organisation, Form — for the same reason the e-RS screen replaced one:
      // the reader was being handed the values and left to work out which box
      // each went in, and the boxes are on the next screen along. The wording
      // is the part that was never on the card at all, and it is the part
      // somebody retypes from nothing every time.
      profMessage({
        to: svc.to || svc.email || '',
        org: svc.org || '',
        form: svc.form || '',
        body: referralEmailBody(svc),
      }),
...
// Said in as few words as it can be. The reader is not going to read a
// paragraph about what the notes do not say; they want to know it is not
// here and who to ask. The gap is flagged so the practice hears about it.
function unknownReferral(name) {
  return answer({
    ...
    flag: 'not-recorded',
    blocks: [
      note(`I don’t know — the practice’s notes do not say how ${name || 'this'} is referred.`, 'warn'),
      bullets([
        'Ask the secretaries or the referring GP.',
        // The tree is not read on this path, so the card says where it IS read.
        'For the form, choose **Referral form** and ask again.',
      ]),
    ],
    source: [],
  });
 ...
 * @param {string} variant the heading the model copied off the page when the
 *                       page records several versions of one referral. An exact
 *                       string to match, not a word to interpret, and it beats
 *                       every other signal because it is the only one that was
 ...
 */
export function referralAnswer({ question = '', service = null, name = '', variant = '', pages = [] } = {}) {
  // THE PRACTICE'S OWN PATHWAY PAGES COME FIRST. The Notebook section
  // "Referral pathways" (with "All Clinic types and their specialities" under
  // it) is where the practice records the speciality and clinic type for each
  // referral it makes, and it is theirs to change. The array in this file is a
  // copy of an earlier version of it, so where the two disagree the page wins,
  // and a referral only the page records is answered rather than reported as
  // "not recorded". See lib/referrals/pathways.mjs.
  const found = findPathwayReferrals({ name, question, variant, pages });
  if (found.length) {
    const [recorded, ...rest] = found;
    // Siblings from the same page with a different pairing, each once. The
    // hospital rule counts as part of the pairing: the dermatology page's five
    // pathways share two pairings between them and are told apart by nothing
    // else, so leaving it out of the key collapsed four of them into one.
    const key = pairingOf;
    const seen = new Set([key(recorded)]);
    const alternatives = rest.filter((e) => e.page === recorded.page && !seen.has(key(e)) && seen.add(key(e)));
    const chosen = alternatives.length ? { ...recorded, alternatives } : recorded;
    return chosen.route === 'email' ? emailReferral(chosen) : ersReferral(chosen);
  }
...
export function referralProcessAnswer() {
  return answer({
    ...
    blocks: [
      ers({ missing: 'Depends on the service — name it and this fills in' }),
      note('Only the speciality and clinic type change between referrals. Name the service and they fill in.', 'info'),
      stepsExpander({}),
      letterExpander(),
    ],
...
/**
 * THE PAGE RECORDS THE REFERRAL BUT NOT HOW IT IS SENT.
 *
 * The e-RS card and the emailed card each assert a route by existing, and one
 * of them is wrong whenever the page does not say. Drawing the e-RS screen
 * anyway is the expensive mistake — somebody spends ten minutes searching for a
 * service that was never going to be on it — so a page that records the
 * pairing and not the route gets its own card: the values as the page has them,
 * and the missing route said in the first line rather than implied by a screen.
 */
function routeUnclearReferral(svc) {
  return answer({
    title: `${svc.name} referral`,
    subtitle: 'The page does not say whether this goes on e-RS or by email',
    flag: 'route-unrecorded',
    blocks: [
      gapsNote(svc),
      fields([
        field('Speciality', svc.specialty, { missing: 'Not recorded on the page' }),
        field('Clinic type', svc.clinicType, { missing: 'Not recorded on the page' }),
        svc.priority ? field('Priority', svc.priority) : null,
        svc.hospital ? field('Hospital', svc.hospital) : null,
        svc.hospitalRule ? field('Which hospital', svc.hospitalRule) : null,
        svc.form ? field('Form', svc.form) : null,
      ], 'What the page records'),
      svc.note ? note(svc.note, 'warn') : null,
      bullets([
        'Ask the secretaries which way this one goes before you start.',
        'For the form, choose **Referral form** and ask again.',
      ]),
      letterExpander(),
    ],
    source: svc.page ? ['Referral pathways', svc.page] : ['Referral pathways'],
  });
}

/**
 * The card for a referral the MODEL read off the practice's page.
 *
 * Same three cards, same blocks, chosen by the route the page recorded — the
 * read changes where the values came from, never what the reader is looking at.
 * See lib/agent/referral-read.mjs for what is checked before any of it is
 * drawn.
 */
export function referralCardFromRead(svc) {
  if (!svc || !svc.name) return null;
  if (svc.route === 'email') return emailReferral(svc);
  if (svc.route === 'ers') return ersReferral(svc);
  return routeUnclearReferral(svc);
}
