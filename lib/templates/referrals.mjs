...
const treeSource = (captured) => [`NEL Referral Tree (Primary Care IT), as of ${saidDate(captured)}`];
...
export function nelFormNotFound(query, { area = PRACTICE_AREA } = {}) {
  ...
  return answer({
    ...
    blocks: [
      note('The whole Referral Tree was searched, including the forms listed for other North East London areas. Nothing came close enough to name.', 'info'),
      bullets([
        'Try the words the form itself would use — the service or the condition, not the task ("dexa", "tongue tie", "suspected skin cancer").',
        'If it is a referral the practice sends by email rather than a tree form, ask without the command and the practice’s own notes are checked too.',
...
/**
 * The whole /form answer, in the order it has to be decided.
 *
 * Kept here rather than in the route so it can be tested, and so the ordering
 * lives beside referralAnswer's — the two must not drift, because they are the
 * same judgement reached by two doors.
 *
 ...
 *   2. the tree.
 *   3. the tree saying it has nothing — never prose, which is the whole point
 *      of having typed the command. The caller in
 *      lib/templates/lookup-command.mjs tries the contract list between 2 and
 *      3; this function is the tree-only door and is what the tests hold.
 */
export function referralFormCommandAnswer({ query = '', pages = [] } = {}) {
  return treeFormAnswer({ query, pages }) || nelFormNotFound(String(query || '').trim());
}

/**
 * Steps 1 and 2 on their own: the practice's own page, then the tree, and null
 * when neither has it.
 *
 * Split out so the miss is visible to a caller. /form no longer ends at the
 * tree — a miss is offered to Primary Care IT's contract list before the card
 * that says nothing was found, because the two lists answer the same question
 * asked two ways and a reader does not know which list their words belong to.
 * That crossing lives in lib/templates/lookup-command.mjs, which can see both
 * lists; this module can only see the tree.
 */
export function treeFormAnswer({ query = '', pages = [] } = {}) {
  ...
  return nelReferralFormAnswer(asked);
}
 ...
 *   3b. failing that, THE NEL REFERRAL TREE — PCIT's list of every referral
 *      form in North East London. Which form to open, and whether this
 *      practice's tree shows it;
 *   4. failing all of it, the honest "not recorded" card.
 *
 * The Notebook step is deliberately last of the practice's own three. A pairing
 * the practice recorded for a named service is more specific than a name on a
 * list, and a service that takes both routes must resolve to the recorded one.
 *
 * THE TREE COMES AFTER ALL THREE, and the order is the whole argument. The
 * practice's own material answers a different question: how a referral is SENT —
 * which address, which form, which quirk gets it rejected. The tree answers
 * which form to open in EMIS. Where the practice has written something down,
 * that is the more specific answer and it wins; PCIT say as much themselves,
 * that they rely on local intelligence about particular services. The tree is
 * what turns four hundred-odd "not recorded in the practice's notes" cards into
 * the name of a form.
 *
...
export function referralAnswer({ question = '', service = null, name = '', pages = [] } = {}) {
  ...
  const onTree = nelReferralFormAnswer(named);
