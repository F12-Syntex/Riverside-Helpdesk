  ...
  return answer({
    ...
    blocks: [
      ...
      (svc.alternatives || []).length
        ? note('Also on this page: ' + svc.alternatives.map((a) => `**${a.name}** — ${[a.specialty && 'Speciality ' + a.specialty, a.clinicType && 'Clinic type ' + a.clinicType, a.hospital].filter(Boolean).join(', ')}`).join('; ') + '.', 'info')
        : null,
      stepsExpander(svc),
      formExpander(svc),
      letterExpander(),
...
export function referralAnswer({ question = '', service = null, name = '', pages = [] } = {}) {
  ...
  if (found.length) {
    ...
    // Siblings from the same page with a different pairing, each once.
    const key = (e) => [e.specialty, e.clinicType, e.hospital].map((s) => String(s || '').toLowerCase()).join('|');
