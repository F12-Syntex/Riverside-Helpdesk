...
import { answer, images, text } from './blocks.mjs';
...
export function notebookPageAnswer({ pages = [], all = [] }) {
  ...
  found.forEach((page, i) => {
    ...
    // Whatever the practice attached to the page. The text has its inline image
    // markdown stripped on the way in (it is noise to a model), so without this
    // a page whose point IS the screenshot arrived as a description of one.
    if ((page.images || []).length) blocks.push(images(page.images, 'From the page'));
  });
