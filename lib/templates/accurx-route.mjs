...
export const ACCURX_READ_SCHEMA = z.object({
  ...
  reason: z.string().default('')
    .describe('The one-line EMIS appointment reason, under 25 words: what pt wants; main sx/problem with duration or trend; relevant hx; other key context. Lowercase clinical shorthand (pt, req, bk\'d, hx, inj, bilat, ?condition, sx, wks, yday, tmrw, f/u, abx, meds), semicolons between clauses, commas within one. No sentences, greetings, feelings or contact times. Empty only if the message describes no problem at all.'),
...
export function accurxReadPrompt({ question, attached = '', reasonRules = [], reasonExamples = [], bookingRules = [], continuityRules = [], extra = '' }) {
  ...
  return [
    ...
    '- Carry a disagreement rather than smoothing it. If the message gives two different durations, the line says both — "3-4 months in one place, 2 months in another" — because that disagreement is often the clinically interesting part. Never invent an approximation to cover it: no "~", no "about", no rounding the patient did not do.',
    '',
