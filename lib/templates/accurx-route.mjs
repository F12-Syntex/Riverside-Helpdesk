...
import { CRITERIA_IDS, cleanCriteria, criteriaForPrompt } from '../triage/criteria.mjs';
import {
...
export const ACCURX_READ_SCHEMA = z.object({
  ...
  // WHY, IN A FIXED VOCABULARY. The reasoning sentence above is where the
  // answer is worked out; this is the same answer said in the terms the card
  // shows — a fixed label per factor, so "under 10" reads the same on every
  // card. Asked AFTER the destination: it labels a decision, it does not make
  // one. See lib/triage/criteria.mjs.
  criteria: z.array(z.object({
    id: z.enum(CRITERIA_IDS),
    value: z.string().default('').describe('The detail from the message, FOUR WORDS AT MOST — "under 10", "3-4 months", "ear drops". Required, except on the few marked "no detail needed". A factor whose detail you cannot give is left out.'),
  })).default([])
    .describe('The one to four factors that DECIDED the destination, most decisive first, from the fixed list. Only what the message says: no age unless it is given.'),
...
export function accurxReadPrompt({ question, attached = '', reasonRules = [], bookingRules = [], continuityRules = [], extra = '' }) {
  ...
  return [
    ...
    '',
    'WHY, IN THE CARD’S TERMS — "criteria"',
    '- Name the one to four factors that decided the destination, most decisive first, each from this fixed list with at most four words of detail from the message:',
    ...criteriaForPrompt(),
    '- These restate your reasoning in fixed terms; they must agree with it and with the destination. Never state a factor the message does not — no age unless it is given.',
    '',
...
export function readingVerdict(read) {
  ...
  return {
    ...
    // The same account in the fixed vocabulary the card shows first.
    criteria: cleanCriteria(read.criteria),
