...
export function foldChecks(checks = []) {
  ...
  for (const d of DESTINATIONS) {
    if (best && d.rank <= best.rank) continue;
    const said = answered.find((c) => String(c.id || '') === d.id);
    if (said && String(said.belongs || '') === 'yes') best = { ...d, said };
  }
  if (!best) return { destination: 'unsure', evidence: '', page: '' };
