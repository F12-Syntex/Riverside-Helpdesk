...
export const ACCURX_READ_SCHEMA = z.object({
  ...
  requests: z.array(z.object({
    ...
    note: z.string().default('').describe('One short sentence: what reception does about this one, or why it cannot be done here.'),
  })).default([])
    .describe('EVERY separate thing asked for, including the ones the card is not about, the ones asked on somebody else’s behalf, and the ones the patient played down. A symptom somebody mentions and then dismisses belongs here.'),
  ...
  flags: z.array(z.string()).default([])
    .describe('Anything that changes how this is handled rather than where it goes: an interpreter and which language, somebody writing on the patient’s behalf, a carer, no internet or no English, a reasonable adjustment, a safeguarding or domestic abuse concern. In the patient’s own terms, at most five.'),
...
export function accurxReadPrompt({ question, notebook = '', attached = '', reasonRules = [], bookingRules = [], extra = '' }) {
  ...
  return [
    ...
    '- Give each one where it goes and one sentence on what reception does about it. If the practice cannot do it at all, say that there rather than sending it to a queue that will find out later.',
