...
import {
  APPOINTMENT_MODE_DEFAULT, APPOINTMENT_MODE_TEST, APPOINTMENT_MODES, BLOODS_FIRST,
  BOOKED_WITH_A_DOCTOR, DESTINATIONS, FACE_TO_FACE_EXAMPLES, HARD_GATES,
  NURSE_CLINIC_DAYS, NURSE_RULES, TELEPHONE_EXAMPLES,
...
export const ACCURX_READ_SCHEMA = z.object({
  ...
  // WHICH KIND OF DOCTOR APPOINTMENT, WHICH IS A SECOND QUESTION AND NOT A
  // SECOND ROUTE.
  //
  // A GP appointment is not one thing to book. The guide calls it "a booked
  // telephone or face-to-face appointment", and the duty doctor page tells
  // reception to decide which — on a card that named the destination and then
  // said nothing more about it. So it is asked here, with the practice's own
  // test (APPOINTMENT_MODE_TEST): what would the doctor have to look at, listen
  // to, or feel?
  //
  // ASKED AFTER THE DESTINATION, ON PURPOSE, exactly as `seenBefore` is. It is a
  // question about an appointment that has already been decided on, and a
  // reading that worked out "this needs examining" BEFORE it named a destination
  // would have an argument for a doctor in hand before it had considered the
  // pharmacy. What kind of slot must never be what chooses the clinician.
  //
  // IT IS NOT ASKED AT ALL FOR ANYWHERE ELSE. A pharmacist, a physiotherapist
  // and a nurse clinic are not the practice's appointment book, and the card
  // renders this only for the two destinations in BOOKED_WITH_A_DOCTOR — so
  // "unsure" on a message going to the pharmacy costs nothing and is thrown
  // away.
  appointment: z.object({
    // The justification first, for the reason `reasoning` is the first field on
    // the whole schema: a value worked out in writing is a different value from
    // one explained afterwards. Six words, because it renders as a clause on a
    // card beside the answer and not as a paragraph.
    why: z.string().default('')
      .describe('What the doctor would have to look at, listen to or feel — or why a call will do. A FEW WORDS, six at most: "needs the rash looked at", "results discussion, nothing to examine". Not a sentence.'),
    mode: z.enum(['faceToFace', 'telephone', 'unsure']).default('unsure')
      .describe('faceToFace if the doctor will probably need to look at, listen to or feel something — rashes, lumps, swelling, wounds, ear or throat problems, chest or abdominal pain, anything the patient wants shown. telephone for medication side effects, a review of a previous consultation, results, or advice about a condition they already have. "unsure" when the message does not say enough to tell, and it is a real answer — the practice books face-to-face when in doubt, because turning that into a phone call is easier than the reverse.'),
  }).default({ why: '', mode: 'unsure' })
    .describe('ONLY when you named "gp" or "dutyDoctor" above: which kind of appointment reception should book. For every other destination leave it exactly as it is — nothing reads it, and the practice does not book those.'),
  ...
  reason: z.string().default('')
    .describe('The one-line EMIS appointment reason, under 25 words: what pt wants; main sx/problem with duration or trend; relevant hx; other key context. Lowercase clinical shorthand (pt, req, bk\'d, hx, inj, bilat, ?condition, sx, yday, tmrw, f/u, abx, meds; durations as /7, /52, /12), semicolons between clauses, commas within one. No sentences, greetings, feelings or contact times. Empty only if the message describes no problem at all.'),
  ...
  // THE LINE FOR WHEN THE PATIENT DOES NOT PICK UP. AccurX is where reception
  // rings to book, and an unanswered call leaves nothing on the record unless
  // somebody writes it. Asked of the reading rather than assembled in code
  // because the call is not always "book them for the reason line": it is
  // often several things at once, a signpost, or a question to ask them —
  // and only something that has read the whole message knows which.
  noAnswer: z.string().default('')
    .describe('The EMIS consultation entry for when reception rings the patient about this message and they do not pick up, under 30 words: that we tried to call, what the call was FOR (book with whom, signpost where, or what to ask/tell them — everything the message needs actioned, not only the main problem), and "no answer". Clinical shorthand as in "reason". Empty only when nobody would ring the patient about this message.'),
});
 ...
 */
export function accurxReadPrompt({ question, attached = '', reasonRules = [], reasonExamples = [], bookingRules = [], continuityRules = [], extra = '' }) {
  ...
  return [
    ...
    '',
    // THE PHARMACY WAS BEING SKIPPED. The reader knew the pharmacy entry and
    // still sent hay fever, a cold and heartburn to a GP because the patient
    // asked for one. The practice's own pages are blunt about it: medicines that
    // can be bought are not routinely prescribed for the self-care list, and a
    // Pharmacy First pathway in its age range is the pharmacist's to treat. So
    // the less senior services are ruled out in writing, in order, before a
    // doctor may be named — and the step is phrased as a check, not a preference,
    // so the exclusions still win.
    'RULE OUT THE SERVICES BELOW A DOCTOR FIRST, IN THIS ORDER, before you may name gp or dutyDoctor:',
    '  1. A PHARMACY FIRST PATHWAY — sore throat 5+, sinusitis 12+, earache 1 to 17, infected insect bite 1+, impetigo 1+, shingles 18+, uncomplicated UTI in a woman 16 to 64. Inside its age range and clear of that pathway’s exclusions (on the pharmacy entry) → pharmacy. The pharmacist can prescribe, so "I need antibiotics" is not a reason for a GP.',
    '  2. THE SELF-CARE LIST AND MINOR ILLNESS — anything on the pharmacy entry’s self-care or minor illness list, infrequent, not already failed, in a patient who is not unwell with it → pharmacy, EVEN IF they asked for a GP appointment or a prescription. Say in "reasoning" that it is not routinely prescribed here, because reception is about to tell them so.',
    '  3. AN EYE — minorEyeService, unless it is on the eye emergency list.',
    '  4. AN ADULT MSK PROBLEM with no red flag on the fcp entry → fcp.',
    '  5. A MEDICINE THEY ALREADY TAKE — repeats, doses, side effects in a patient who is well → pharmacyTeam. A NURSE PROCEDURE they name → nurse.',
    '- Each of those is closed ONLY by something written in the message: the age, how long it has gone on, a treatment that already failed, it keeps coming back, a red flag, pregnancy or breastfeeding, being unwell with it, a young child or baby who is unwell, a housebound or frail patient. Name that thing in "reasoning", and put the service you passed over in "ruledOut". "The patient asked for a GP" does not close any of them on its own.',
    '',
    ...
    '',
    // ONLY TWO DESTINATIONS EVER READ THIS, and the prompt says which so the
    // reading does not spend tokens deciding a slot type for a Pharmacy First
    // referral. The rules are the practice's own, from the same guide as the
    // ladder — see APPOINTMENT_MODE_TEST in lib/triage/destinations.mjs.
    'TELEPHONE OR FACE-TO-FACE — "appointment", and ONLY if you named ' + BOOKED_WITH_A_DOCTOR.join(' or ') + ':',
    '- A GP appointment is not one thing to book. The practice books either a telephone consultation or a face-to-face slot, and reception has to choose before it can book anything.',
    '- ' + APPOINTMENT_MODE_TEST,
    '- Face-to-face: ' + FACE_TO_FACE_EXAMPLES.join(', ').toLowerCase() + '.',
    '- Telephone: ' + TELEPHONE_EXAMPLES.join(', ').toLowerCase() + '.',
    '- ' + APPOINTMENT_MODE_DEFAULT + ' So "unsure" is a real answer here and it is not a failure — it books a room and says on the card that nothing decided it.',
    '- This is about what the doctor would have to DO, not about how serious it is or how soon it is. Where the message goes is already decided and nothing you write here may revisit it.',
    '- For every other destination, leave "appointment" alone. Nothing reads it.',
    '',
    ...
    '',
    '"reason" — the one-line EMIS appointment reason, for the clinician to read:',
    ...reasonRules.map((r) => '- ' + r),
    ...reasonExamples.map((e) => '- Example output: ' + e),
    ...
    '',
    '"noAnswer" — the line reception pastes into the EMIS consultation when they ring the patient about this message and get no answer:',
    '- So the colleague the patient rings back knows why we called. Say that we tried to call, what the call was for, and "no answer".',
    '- WHAT THE CALL WAS FOR IS THE WHOLE MESSAGE, NOT ONLY THE REASON LINE. If it was to book them, say with whom (GP, duty dr, practice nurse, FCP, practice pharmacist…) and, where you decided it, tel or F2F. If it was to signpost them elsewhere, say where. If the call also had to tell them something or ask them something — a request the practice cannot do, a form to fill in, bloods first, another request in the same message — say that too, briefly.',
    '- Under 30 words, clinical shorthand as in "reason": "Tried to call pt to book tel appt with GP re: heartburn 3/52, gaviscon not helping; also to advise repeat req sent to pharmacy team. No answer."',
    '- Never a greeting, never the patient’s name, never contact times.',
    '',
...
export function readingVerdict(read) {
  ...
  const slot = (read.appointment && typeof read.appointment === 'object') ? read.appointment : {};
  ...
  return {
    ...
    // WHICH KIND OF DOCTOR APPOINTMENT. Carried whatever the destination turned
    // out to be, and thrown away by the card for every destination that is not a
    // booked appointment with a doctor — one gate, in ./accurx.mjs, rather than
    // a second copy of the list here.
    //
    // A mode the schema does not have is "unsure", which is the same thing an
    // absent one is: the practice's own rule then books a room and the card says
    // that nothing decided it. There is no path where a broken value quietly
    // becomes a telephone slot.
    appointment: {
      mode: Object.prototype.hasOwnProperty.call(APPOINTMENT_MODES, String(slot.mode || ''))
        ? String(slot.mode)
        : 'unsure',
      why: String(slot.why || '').trim().slice(0, 90),
    },
