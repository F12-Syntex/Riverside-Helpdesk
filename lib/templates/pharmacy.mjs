...
// What a urinary presentation looks like in the words patients use for it. Read
// only next to the pregnancy exclusion above, and deliberately written from how
// somebody describes the symptom rather than from what the pathway is called:
// "burning when I wee" is the commonest way this arrives and is on no list under
// that name.
export const URINARY = new RegExp([
  '\\buti\\b', 'urine infection', 'urinary tract', 'cystitis', 'water infection',
  '\\b(?:burning|stinging|sting|pain\\w*)\\b[^.!?]{0,25}\\b(?:wee|weeing|pee|peeing|urinat\\w+|passing (?:water|urine))\\b',
  '\\b(?:wee|weeing|pee|peeing|urinat\\w+|passing (?:water|urine))\\b[^.!?]{0,25}\\b(?:burn\\w*|sting\\w*|hurts?|painful)\\b',
  '\\bfrequency\\b[^.!?]{0,25}\\burin\\w+', '\\bgoing (?:to the (?:loo|toilet) )?constantly\\b',
  '\\bcannot empty\\b', '\\bcan\'?t empty\\b',
].join('|'), 'i');
 ...
 */
export function pharmacyFirstAnswer({ condition = '', text = '', complaint = '', whole = '' } = {}) {
  ...
  // READ OVER THE WHOLE MESSAGE, NOT THE ONE REQUEST THIS CARD IS ABOUT.
  //
  // A message that asks for several things is split, and each card is decided
  // from its own request's words — which is right for what the card CLAIMS and
  // wrong for what rules a service out. The patient who prompted this wrote her
  // urinary symptoms in one paragraph and "we are trying for a baby and my
  // period is five days late" in another, so the card about the urine never saw
  // the sentence that takes it off the pathway. Exclusions are message-wide for
  // the same reason red flags are.
  const everywhere = [full, String(whole || '')].filter(Boolean).join('\n');

  // AND IT IS THE PRESENTATION THAT DECIDES, NOT WHICH LIST IT MATCHED.
  //
  // The check was written against the named UTI pathway, so it missed the same
  // patient the moment her message was split: the request the card was about
  // said "burning when I wee and going constantly", which matches no alias of
  // "uncomplicated UTI" and lands on the minor illness line instead. A pharmacy
  // referral for urinary symptoms is a pharmacy referral for urinary symptoms
  // however the lists happened to catch it.
  const urinary = URINARY.test(everywhere) || (pathway && /uti|urin|cystitis/i.test([pathway.name, ...(pathway.aliases || [])].join(' ')));

  if (urinary && mightBePregnant(everywhere)) return pregnantUrinaryAnswer({ condition: named });
...
/**
 * A urinary presentation in somebody who might be pregnant.
 *
 * ITS OWN ANSWER, AND CHECKED IN THE CASCADE rather than inside the pharmacy
 * card, because it was inside the pharmacy card and the patient who needed it
 * never got there: her message was split, the request the card was about said
 * "burning when I wee" and matched no list, and she fell through to the generic
 * "not on a pharmacy list — ask the duty doctor" card. Right destination, and
 * the one thing that decided it went unsaid.
 *
 * An exclusion that changes where somebody goes has to be checked where the
 * routing is decided. See triage.mjs, next to the red flags.
 */
export function pregnantUrinaryAnswer({ condition = '' } = {}) {
  const said = String(condition || '').trim();
  return answer({
    title: said ? `${said} — not for pharmacy` : 'Urinary symptoms — not for pharmacy',
    ...
    blocks: [
      ...
      bullets([
        'Pass it to the **duty doctor today**, not to a pharmacy and not to a routine appointment.',
        'Say that a pregnancy has not been confirmed either way — the clinician will want that established.',
      ...
      note('If the patient asked for a pharmacy referral or said they did not want an appointment, tell them what is happening instead. Somebody is about to ring them and say no to what they asked for.', 'warn'),
    ],
