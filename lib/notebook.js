...
// { note } on success or { error } with a human-readable reason. Embeddings
// need no resync: chunk hashes cover title + text only, and the breadcrumb
// shown in citations is rebuilt from the tree at ask time.
...
// headings/blank lines, pack to ~900 chars, cap at 1500. Inline images are
// reduced to their alt text — blob URLs are noise to embeddings and to the
// source passages shown in the chat (the pictures themselves reach the chat
// through the note's attachments instead).
