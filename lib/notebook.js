...
export async function syncNoteEmbeddings(note) {
  ...
  const chunks = chunkBody(note.body || '').map((text, i) => ({
...
function keywordSources(rows, query) {
  ...
  const entries = rows.filter((r) => (r.body || '').trim()).map((r) => ({ name: crumb(r), text: r.body, origin: 'notebook' }));
...
export async function notebookContext(query) {
  try {
    ...
    const withBody = rows.filter((r) => (r.body || '').trim());
    ...
    const scored = emb
      ...
      .filter((x) => (x.sem >= NOTE_MIN_SCORE || x.lex > 0) && noteById.has(x.e.noteId))
