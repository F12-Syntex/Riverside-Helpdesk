...
export async function updateNote({ id, title, body }) {
  ...
  if (!nid) return null;
  // Update only the fields provided (title and/or body). Sections (top-level
  // notes) are name-only categories: their title can change but body writes
  // are ignored, whatever the client sends.
  const t = title == null ? null : String(title).slice(0, 200);
  let b = body == null ? null : String(body);
  // Sections (root notes) are name-only containers — content lives in their
  // pages. Ignore body writes so a section can never accumulate text.
  if (b != null) {
    const cur = await sql`SELECT parent_id FROM notes WHERE id = ${nid}`;
    if (cur[0] && cur[0].parent_id == null) b = null;
  }
  const rows = await sql`
    UPDATE notes
    SET title = COALESCE(${t}, title),
        body  = CASE WHEN parent_id IS NULL THEN body ELSE COALESCE(${b}, body) END,
        updated_at = now()
    WHERE id = ${nid}
    RETURNING id, parent_id AS "parentId", title, body, position, updated_at AS "updatedAt"
  `;
...
function keywordSources(rows, query) {
  ...
  const entries = rows.filter((r) => r.parentId && (r.body || '').trim()).map((r) => ({ name: crumb(r), text: r.body, origin: 'notebook' }));
...
export async function notebookContext(query) {
  try {
    ...
    // Pages only: sections (top-level notes) are name-only categories and
    // contribute nothing themselves — their names still appear in each page's
    // breadcrumb title.
    const withBody = rows.filter((r) => r.parentId && (r.body || '').trim());
