...
import { archiveKnowledgeEntry, upsertKnowledgeEntry } from '@/lib/knowledge';
...
export async function syncNoteEmbeddings(note) {
  ...
  if (isSection) {
    ...
    await upsertKnowledgeEntry({
      ...
      data: { legacyNoteId: nid, parentId: note.parentId }, sourceRef: `notebook:${nid}`, authority: 90,
      passages: chunks.map((c, i) => ({ heading: c.section, text: c.content, embedding: vecs[i], location: { images } })),
    }, { embed: false }).catch(() => {});
  }
}
