...
/**
 * Tag a note or section with the shape its answers come back in.
 *
 * Set on a SECTION and every page beneath it inherits it — that is the point:
 * the practice says "the referrals folder is an e-RS screen" once rather than
 * per page. A page may carry its own tag, which wins over the folder's; '' on a
 * page does NOT mean "plain", it means "whatever the folder says", and a folder
 * tagged '' is the plain page. Clearing is therefore the same operation as
 * setting, which is why there is no separate clear.
 *
 * An unknown tag is refused rather than stored: a tag nothing can render would
 * be a folder quietly formatted as nothing at all.
 */
export async function setNoteOutputTag({ id, outputTag }) {
  await ensureNotebookSchema();
  const sql = getSql();
  const nid = parseInt(id, 10);
  if (!nid) return null;
  const tag = String(outputTag || '');
  if (!isOutputTag(tag)) return null;
  const rows = await sql`
    UPDATE notes
    SET output_tag = ${tag}, updated_at = now()
    WHERE id = ${nid}
    RETURNING id, parent_id AS "parentId", title, body, position,
              is_section AS "isSection", output_tag AS "outputTag", updated_at AS "updatedAt"
  `;
...
export async function importNotebook({ notes = [], attachments = [], onProgress = null } = {}) {
  ...
  await sql`
    INSERT INTO notes (id, title, body, parent_id, is_section, output_tag)
    SELECT * FROM UNNEST(
      ...
      ${rows.map((n) => (isOutputTag(String(n.outputTag || '')) ? String(n.outputTag || '') : ''))}::text[]
    )
  `;
...
export async function syncNoteKnowledge(note, { autoAnalyse = true, attempt = 0 } = {}) {
  ...
  const latest = await sql`
    SELECT id,parent_id AS "parentId",title,body,is_section AS "isSection",output_tag AS "outputTag",updated_at AS "updatedAt"
    FROM notes WHERE id=${nid}
  `;
