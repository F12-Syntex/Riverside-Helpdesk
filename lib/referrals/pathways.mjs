...
export function findPathwayReferrals({ name = '', question = '', pages = [] } = {}) {
  ...
  entries.forEach((entry, order) => {
    ...
    if (best) scored.push({ entry, score: best, order, agrees: (!!entry.cancer === askedCancer) ? 1 : 0 });
  });
  // Tier first (it is what the reader's words actually matched), then agreement
  // on the two week wait, then the length of the name, then the page's order.
  const tierOf = (n) => Math.ceil(n / 1000);
  scored.sort((a, b) => tierOf(b.score) - tierOf(a.score) || b.agrees - a.agrees || b.score - a.score || a.order - b.order);
