...
function saidWords(recorded, asked) {
...
export function findPathwayReferrals({ name = '', question = '', pages = [] } = {}) {
  ...
  entries.forEach((entry, order) => {
    ...
    for (const n of namesOf(entry)) {
      ...
      said = Math.max(said, saidWords(n, q));
    }
    if (best) scored.push({ entry, score: best, order, said, agrees: (!!entry.cancer === askedCancer) ? 1 : 0 });
  });
  ...
  // then the length of the name, then the page's order.
  const tierOf = (n) => Math.ceil(n / 1000);
  scored.sort((a, b) => tierOf(b.score) - tierOf(a.score) || b.agrees - a.agrees
    || b.said - a.said || b.score - a.score || a.order - b.order);
