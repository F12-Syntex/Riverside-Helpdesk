...
export function createTools({ apiKey, searchModel, notebookChunks = [], evidence, onEvent, contacts = [], onUsage }) {
  ...
  const open_practice_source = tool({
    ...
    execute: async ({ title, part = 1 }) => {
      ...
      if (!doc) {
        notify({ type: 'tool-result', id, tool: 'open_practice_source', summary: 'No source with that title', items: [], ok: false });
      ...
      if (!loaded.parts.length) {
        notify({ type: 'tool-result', id, tool: 'open_practice_source', summary: 'That document holds no readable text', items: [], ok: false });
