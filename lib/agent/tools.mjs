...
// How much of a source's text a search result shows.
//
// THE RESEARCH MODEL NEVER QUOTES. It reads this to decide one thing — is this
// the source I want to open — and the writer that does the quoting is handed the
// source's FULL text from the evidence registry, which stores what the tool was
// given rather than what it returned (see evidence.mjs). So every character
// above what it takes to judge relevance is paid for twice over: once to
// generate the tool result, and again on every remaining step of the loop, which
// re-sends the whole conversation each time.
//
// Four wordings at five hits each used to put ~45,000 characters into the
// context of a decision that needed a title and a paragraph.
const SEARCH_EXCERPT = 600;
...
//
// What is cached is the PROMISE, not the result. One call now asks for several
// sources at once and runs them together, so two of them wanting the same
// document — two parts of one protocol, the commonest shape there is — would
// otherwise both miss the cache and both query, because neither has finished by
// the time the other looks. Caching the in-flight load makes the second wait on
// the first instead.
const DOCUMENT_CACHE_MAX = 24;
const documentCache = new Map();

function loadDocumentParts(doc) {
  ...
  const loaded = knowledgeEntryPassages(doc.id)
    .then((passages) => ({ stamp, title: doc.title, parts: packDocumentParts(doc.id, passages) }));
  const entry = { stamp, loaded };
  documentCache.set(doc.id, entry);
  // A load that failed is not this document's content. Forget it, so the next
  // call retries rather than inheriting the error for the life of the process.
  loaded.catch(() => { if (documentCache.get(doc.id) === entry) documentCache.delete(doc.id); });
...
export function createTools({ apiKey, searchModel, notebookChunks = [], evidence, onEvent, contacts = [], onUsage, customGuides = [] }) {
  ...
  async function practiceLookup(query, scope = 'all', limit = 5) {
    ...
    // A wording that found nothing is a finding. Recorded so the writer can be
    // told what was looked for and missed, rather than being left to infer an
    // absence from a gap in the sources — which it fills from the nearest thing
    // it does have. See evidence.addMiss.
    if (!sources.length) evidence.addMiss(query);
  ...
  const outline_practice_sources = tool({
    ...
    execute: async ({ titles }) => {
      ...
      const catalogue = await documents();
      // Together, for the same reason opening them is: the point of outlining
      // four files in one call is one wait, not four.
      const outlines = await Promise.all(wanted.map(async (title) => {
        const want = titleKey(title);
        const whole = notebookChunks.find((c) => titleKey(c.docTitle).includes(want))
          || guideChunks.find((g) => titleKey(g.docTitle).includes(want));
        if (whole) {
          return { title: whole.docTitle, kind: kindLabel(whole.kind), parts: 1, characters: String(whole.text || '').length, outline: ['1. Whole page'] };
        }
        const doc = catalogue.find((d) => titleKey(d.title) === want)
          || catalogue.find((d) => titleKey(d.title).includes(want));
        if (!doc) return { title, found: false, note: 'No source with that title.' };
  ...
  const open_practice_sources = tool({
    ...
    execute: async ({ sources }) => {
      ...
      // Together, not one after another. Every one of these is a cache hit or a
      // database read, and a model that asked for four sources in one call
      // should wait for the slowest of them rather than the sum of all four —
      // which was the whole point of making the tool take a list.
      const opened = await Promise.all(wanted.map((want) => openOne(want.title, want.part)));
  ...
  const search_web = tool({
    ...
    execute: async ({ queries }) => {
      ...
      //
      // Started alongside the web searches rather than in front of them. It is a
      // database read and they are network round trips through a model, so
      // waiting for one before beginning the others added its whole duration to
      // the slowest thing in a turn for no gain: the guard's job is to get the
      // practice's material into evidence before the answer is written, and it
      // is awaited below, well before that.
      const guard = evidence.practiceCount ? null : practiceLookup(wanted[0], 'all', 5);
      ...
      const [results] = await Promise.all([Promise.all(wanted.map(runOne)), guard]);
