...
import { isReferralRequest, allowsErsCard } from '../referrals/scope.mjs';
...
export const AnswerSchema = z.object({
  ...
  sections: z.array(z.object({
    ...
    basis: z.enum(['practice', 'web', 'reasoned']).describe('"practice" = stated by the practice’s documents or Notebook. "web" = from a web page. "reasoned" = YOUR OWN working, applying named practice sources to the case asked about.'),
    ref: z.string().describe('The source reference this is based on: P1/P2… for practice sources, W1/W2… for web pages. Leave empty for "reasoned" and fill in premises instead.'),
    quote: z.string().describe('For practice sources: a verbatim span of at least a dozen words copied exactly from that source, character for character. For web sources: a short quote from the page extract, or an empty string. Empty for "reasoned" — there is nothing to quote, because these are not the source’s words.'),
    premises: z.array(z.string()).default([])
      .describe('ONLY for basis "reasoned": every practice reference (P1, P2…) your working rests on. At least one is required, and each must be a source you were actually shown. This is what the reader checks your reasoning against.'),
  ...
  // Asking back, when — and only when — asking back is the honest thing to do.
  clarify: z.object({
    question: z.string().default('').describe('One short question back to the reader. Empty unless the question genuinely cannot be answered as asked.'),
    options: z.array(z.string()).default([]).describe('Two to four answers to that question, each naming one of the distinct routes the sources actually record. Each is what the reader taps.'),
  }).default({ question: '', options: [] })
    .describe('ONLY when the question is too vague to answer AND the sources record two or more different answers depending on which was meant. Leave both fields empty otherwise.'),
});
...
function evidenceBlock(evidence, keepRefs = null) {
  ...
  // What was looked for and not found. Without this the writer sees only an
  // absence, and cannot tell "nobody searched for this" from "this practice has
  // nothing on it" — so it reaches for the nearest source it does have and
  // writes that out. Stated as a result, it becomes something the answer can say.
  const misses = typeof evidence.missList === 'function' ? evidence.missList() : [];
  if (misses.length) {
    parts.push(
      '\nSEARCHED AND NOT FOUND — these wordings were searched in the practice’s own material and returned nothing:',
      misses.map((q) => `- "${q}"`).join('\n'),
      'That is a result, not a hole to fill. Where it means the practice records nothing about what was asked, SAY SO and put it in "gaps" — do not answer from the nearest source that did come back, and do not reason from one (see basis "reasoned"). It is also worth naming in the answer: "the practice’s material does not use that term" tells the reader something true and useful.',
    );
  }
  return parts.join('\n\n');
}
...
function composePrompt({ question, history, evidence, feedback, previous, ersSuggestion, selectedRefs, attachments }) {
  const lines = [
    ...
    '',
    'WORKING SOMETHING OUT — basis "reasoned"',
    'The practice cannot write a page for every case. Its material often settles a question without answering it in the shape it was asked: two pages have to be put together, a documented process has to be applied to a case the page does not name, or something follows from what is written. Reciting the nearest page instead is how a correct-looking answer misses the question, so there is a section type for exactly this.',
    '- USE basis "reasoned" when the practice’s material DOES settle the question but does not state the answer in so many words. Fill in "premises" with every practice reference your working rests on. Leave "quote" empty — these are your words, not the source’s, and there is nothing to quote.',
    '- SAY WHAT YOU ARE DOING, in the section text. "The Notebook covers X; this is the same situation because …, so …". The reader is shown this section as your working, not as something the practice wrote, and they must be able to follow the step you took and disagree with it.',
    '- IT IS NOT A GAP-FILLER. If the practice’s material does not cover the thing at all, reasoning from something adjacent is exactly the mistake rule 4a describes. That is a gap: say so, put it in "gaps", name who to ask, and write NO reasoned section. The test is whether you can name the sources your conclusion follows FROM. If you cannot, you are not reasoning — you are guessing.',
    '- AND IT IS NOT A WAY ROUND A QUOTE YOU COULD NOT FIND. If the practice’s material states something outright, that is a "practice" section with the quote copied exactly. Choosing "reasoned" because quoting was hard turns what the practice actually says into what you think, which is a downgrade the reader pays for.',
    '- NEVER for clinical judgement about a specific patient. No amount of working shown makes that acceptable — rule 5 governs, and the answer is that a clinician must decide.',
    '- AT MOST ONE reasoned section, and never the only section. If nothing in the practice’s material is solid enough to write a grounded section from, you have nothing to reason FROM either.',
    '',
    '4b. ASK BACK INSTEAD, when asking back is the honest thing to do. If the question is too vague to answer AND the sources record two or more genuinely different answers depending on what was meant — a different process per site, per form, per service, per route — do not pick one and do not write both out. Fill in "clarify": one short question, and two to four options naming the routes the sources actually record. Leave sections empty when you do. This is rare: use it only when choosing wrongly would send the reader down the wrong process, never to avoid the work of answering, never when one answer covers every reading, and never when the sources record nothing at all (that is a gap, rule 4).',
    '4c. The options must come from the sources. Never offer a choice the material does not record, and never offer "something else" — the reader will simply ask again.',
    ...
    '- AND LEAVE referralRoute EMPTY FOR AN EMAILED REFERRAL. There is no speciality or clinic type to set, because there is no e-RS form being filled in. A card of e-RS fields above steps that say to email it tells the reader to use the other route — and if the pairing came from the type-list match rather than the practice material, it is a made-up value for a form they will never open. referralRoute is for referrals sent ON e-RS, nothing else.',
    '- LEAVE IT EMPTY FOR ANYTHING THAT IS NOT SOMEBODY MAKING A REFERRAL. The word "referral" in the question is not the test — what the reader is about to DO is. A referral arriving from a hospital or from 111, one already sent and being chased or cancelled, a waiting time, a policy that happens to use the word: none of those is an e-RS form being filled in, so none of them gets a card, and none of them gets referral steps either. Answer the question that was actually asked.',
    '',
  ];
  ...
  // A file the reader dropped onto the question. Context, never a source: it
  // has no P-reference, nothing may be quoted from it, and an answer that turns
  // on what it says must say where that came from. It sits before the practice
  // sources so the question ("does this letter need a 2WW?") makes sense.
  const attached = attachmentsBlock(attachments);
  if (attached) {
    lines.push(
      attached,
      '',
      'THE FILE ABOVE IS NOT A SOURCE. It is the reader’s own document, handed to you so you can see what they are asking about. Never give it a P-reference, never quote it as practice material, and never let it settle what the practice does — only the PRACTICE SOURCES below do that. You may describe it, use it, or work from it, and you should say plainly when the practice’s material does not cover what is in it.',
      '',
    );
  }
...
//
// Only for a question that is actually asking for a referral to be MADE. These
// were gated on the word "refer" appearing anywhere, which meant an answer about
// a referral arriving from a hospital, or about chasing one already sent, was
// sent back for a repair demanding a speciality, a clinic type and a follow-up
// about writing the referral letter — and came back with e-RS material bolted
// onto an answer that had no e-RS in it. See lib/referrals/scope.mjs.
function referralProblems(sections, question, followUps = [], route = null) {
  const problems = [];
  if (!sections.length || !isReferralRequest(question)) return problems;
...
export function validateDraft(draft, evidence, question = '') {
  ...
  const check = (item, label) => {
    ...
    if (!text) return null;
    // The assistant's own working, applying named practice sources to the case
    // asked about. There is no quote to verify — that is the point of it — so
    // what is verified instead is the PREMISES: every reference it claims to
    // reason from has to be a source this turn actually read. A conclusion whose
    // premises do not resolve is not reasoning, it is invention with a citation
    // shape around it, and it is dropped like any other unverifiable claim.
    if (item.basis === 'reasoned') {
      const premises = [...new Set((item.premises || [])
        .map((p) => String(p || '').trim().toUpperCase())
        .filter(Boolean))];
      const found = premises.map((ref) => evidence.getPractice(ref)).filter(Boolean);
      if (!found.length) {
        critical(`${label}: is written as reasoning but names no practice source it follows from${premises.length ? ` (${premises.join(', ')} ${premises.length === 1 ? 'is not a source' : 'are not sources'} you were shown)` : ''}. Put every reference your working rests on in "premises", or drop the section — reasoning with nothing under it is a guess.`);
        return null;
      }
      return {
        ...
        markdown: text,
        basis: 'reasoned',
        // Never red. The critical callout means "the practice says this and it
        // is unsafe to skip"; putting an inference in it is precisely the
        // blurring this section type exists to avoid.
        critical: false,
        ref: found[0].ref,
        // Shown to the reader as what this working was built on, and openable
        // like any other citation, so the step taken can be checked.
        premises: found.map((chunk) => ({ ref: chunk.ref, ...citationFor(chunk, '') })),
        cite: null,
        web: null,
      };
    }
  ...
  // The two limits that keep reasoning from becoming a way of answering
  // unanswerable questions.
  //
  // NEVER THE ONLY SECTION. A reasoned section applies the practice's material
  // to the case asked about, so it needs practice material under it. An answer
  // that is nothing but working has no premises the reader can check and is
  // exactly the gap-filling rule 4a forbids — the model reaching for the nearest
  // thing it has because the material does not cover what was asked. Dropped
  // here whatever the model declared, which turns the answer into the honest
  // "the practice's material does not cover this".
  //
  // AT MOST ONE. Two or three of them is not an answer built on the practice's
  // material with a step taken on top; it is an essay with a citation attached.
  const grounded = kept.filter((sec) => sec.basis !== 'reasoned');
  let reasonedKept = 0;
  let ungrounded = false;
  let tooMany = false;
  const survivors = [];
  for (const sec of kept) {
    if (sec.basis !== 'reasoned') { survivors.push(sec); continue; }
    if (!grounded.length) { ungrounded = true; continue; }
    reasonedKept += 1;
    // The first is the working the model led with, which is the one it thought
    // mattered; the rest are trimmed, exactly as the critical flag is.
    if (reasonedKept > 1) { tooMany = true; continue; }
    survivors.push(sec);
  }
  kept.length = 0;
  kept.push(...survivors);
  if (ungrounded) {
    critical('Your only sections are your own working, with nothing from the practice’s material under them. That is not reasoning, it is answering a question the material does not cover. Write what the practice’s material actually states, or say plainly that it does not cover this, put it in "gaps" and name who to ask.');
  }
  if (tooMany) {
    critical('You wrote more than one reasoned section. At most one step may be taken beyond what the practice’s material states; put the rest on the sources that state them, or leave them out.');
  }
  ...
  // A reasoned section stands on several sources at once, so any of its premises
  // is a fair reference for a key point that summarises it.
  const survivingRefs = new Set(kept.flatMap((sec) => [sec.ref].concat((sec.premises || []).map((p) => p.ref))));
  ...
  let referralRoute = (route.specialty || route.clinicType || route.clinicTypeOptions.length || route.priority) ? route : null;

  // …and only when the question is about MAKING a referral, and only when the
  // referral is actually sent on e-RS.
  //
  // Both halves have been the source of cards that had no business being there.
  // A question that merely says "referred" — a referral arriving from a hospital,
  // one already sent and being chased, a policy that uses the word — is not a
  // form the reader is about to fill in. Nor is a referral that is emailed: the
  // card would be four e-RS fields at the top of an answer whose own steps say
  // to email it, which reads as an instruction to use the other route.
  //
  // A pairing the practice wrote down is shown unless the answer sends the
  // reader somewhere else. A pairing only MATCHED from the e-RS referral-types
  // list has to earn its place: the answer must actually put the referral on
  // e-RS, or the card is a guessed value for a form nobody is going to open.
  // The rule is in lib/referrals/scope.mjs, so the same one governs a pairing
  // filled in after this point (lib/referrals/route-determination.mjs).
  if (referralRoute) {
    const answerText = kept.map((sec) => `${sec.heading || ''}\n${sec.markdown || ''}`)
      .concat(keyPoints.map((p) => p.text || ''))
      .join('\n');
    if (!allowsErsCard({ question, answerText, source: route.source })) referralRoute = null;
  }
  ...
  // The question back to the reader, when the writer asked one. Trimmed,
  // deduplicated and capped: a list of six is not a clarification.
  const clarifyOptions = [...new Set((draft.clarify?.options || [])
    .map((o) => String(o || '').replace(/\s+/g, ' ').trim())
    .filter((o) => o.length > 1 && o.length <= 90))].slice(0, 4);
  const clarifyQuestion = String(draft.clarify?.question || '').replace(/\s+/g, ' ').trim().slice(0, 160);
  // Both halves or neither: a question with nothing to tap is a dead end, and
  // options with no question are a menu of nothing.
  const clarify = (clarifyQuestion && clarifyOptions.length >= 2)
    ? { question: clarifyQuestion, options: clarifyOptions }
    : null;

  return { sections: kept, keyPoints, problems, mustFix, messageCite, messageWeb, followUps, referralRoute, clarify };
}
...
export async function composeVerifiedAnswer({
  ...
  onStatus,
  ersSuggestion = null,
  // Files the reader dropped onto the question, as text. Context only — see
  // composePrompt, where they are labelled as unquotable.
  attachments = [],
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    const prompt = composePrompt({ question, history, evidence, feedback, previous: draft, ersSuggestion, selectedRefs, attachments });
  ...
  return {
    answerable: draft.answerable !== false && (result.sections.length > 0 || !!result.clarify),
