...
export function validateDraft(draft, evidence, question = '') {
  ...
  // …and only when the referral is actually sent on e-RS. A referral that is
  // emailed has no speciality or clinic type to set: the card would be four
  // e-RS fields at the top of an answer whose own steps say to email it, which
  // reads as an instruction to use the other route. Worse when the pairing was
  // only matched from the e-RS type list — it is then a guessed value for a
  // form the reader is never going to open.
  if (referralRoute) {
    const answerText = kept.map((sec) => sec.markdown || '')
      .concat(keyPoints.map((p) => p.text || ''))
      .join('\n')
      .toLowerCase();
    const saysNotErs = /\b(not|rather than|instead of)\b[^.]{0,40}\be-?rs\b/.test(answerText)
      || /\be-?rs\b[^.]{0,40}\b(is not|are not|isn.t|aren.t)\b/.test(answerText);
    const emailed = /\bemail(?:ed|s|ing)?\b/.test(answerText) || /\baccurx\b/.test(answerText);
    // A directive to set something on e-RS means the route really is e-RS,
    // whatever else the answer mentions in passing.
    const usesErs = /\b(on|in|via|through|onto)\s+e-?rs\b/.test(answerText)
      || /\bchoose and book\b/.test(answerText)
      || /\be-?referral service\b/.test(answerText);
    if (saysNotErs || (emailed && !usesErs && route.source === 'suggested')) referralRoute = null;
  }
