...
export const AnswerSchema = z.object({
  ...
  }).default({ requestType: '', priority: '', specialty: '', clinicType: '' })
    .describe('ONLY for a referral question, and only from a source. The exact values to set on e-RS. Leave every field empty when the question is not about making a referral, or when no source records them.'),
...
export async function composeVerifiedAnswer({ model, question, history, evidence, onStatus }) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    const prompt = composePrompt({ question, history, evidence, feedback, previous: draft });
