...
export const AnswerSchema = z.object({
  ...
  referralRoute: z.object({
    ...
    clinicType: z.string().default('').describe('The e-RS Clinic Type to set, exactly as the source names it — for example "2WW Skin".'),
  ...
  }).default({ requestType: '', priority: '', specialty: '', clinicType: '', source: '' })
...
function composePrompt({ question, history, evidence, feedback, previous, ersSuggestion }) {
  const lines = [
  ...
  // On a repair, keep the full text of the sources the previous attempt drew on
  // and any named in the defects — those are the ones a fix can need to re-quote.
  let keepRefs = null;
...
function referralProblems(sections, question, followUps = [], route = null) {
  ...
  if (PROCEDURAL_QUESTION.test(question)) {
    const inRoute = !!(route && route.specialty && route.clinicType);
...
export function validateDraft(draft, evidence, question = '') {
  ...
  const field = (v) => String(v || '').replace(/\s+/g, ' ').trim().slice(0, 80);
  const route = {
  ...
  const referralRoute = (route.specialty || route.clinicType || route.priority) ? route : null;

  problems.push(...structureProblems(kept, question));
  problems.push(...referralProblems(kept, question, followUps, referralRoute));

  return { sections: kept, keyPoints, problems, messageCite, messageWeb, followUps, referralRoute };
}
...
export async function composeVerifiedAnswer({ model, question, history, evidence, onStatus, ersSuggestion = null }) {
  let draft = null;
  let result = null;
  let attempts = 0;
  let feedback = '';
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    const prompt = composePrompt({ question, history, evidence, feedback, previous: draft, ersSuggestion });
