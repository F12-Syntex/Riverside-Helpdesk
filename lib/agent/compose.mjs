...
function composePrompt({ question, history, evidence, feedback, previous }) {
  const lines = [
    ...
    '',
    'REFERRALS — what belongs in the answer and what does not',
    '- The referral letter is a separate job from making the referral. ASSUME NO LETTER IS NEEDED: do not explain how to write, dictate, lay out or word one unless the question explicitly asks how to create a referral letter. If it does not ask, leave the letter out of the answer entirely.',
    '- Give the standard referral process instead, and always say which SPECIALITY and which CLINIC TYPE to set for this particular referral — the referral cannot be sent without both. Take them from the practice sources; the Notebook page for that referral is where the practice records them, and it is what you must quote.',
...
function referralProblems(sections, question) {
  const problems = [];
  if (!sections.length || !REFERRAL_QUESTION.test(question)) return problems;

  // 1. Making the referral and writing the letter are different jobs. The letter
  //    is only in scope when it was actually asked for.
  if (!LETTER_ASKED.test(question)) {
    const letter = sections.find((sec) => /\bletters?\b/i.test(sec.heading));
    if (letter) {
      problems.push(`This question does not ask how to create a referral letter, so the answer must not explain writing one. Drop the section headed "${letter.heading}" and give the referral process itself.`);
    }
  }
...
export function validateDraft(draft, evidence, question = '') {
  ...
  problems.push(...referralProblems(kept, question));

  return { sections: kept, keyPoints, problems, messageCite, messageWeb };
}
