...
import { OPENROUTER_URL, openRouterHeaders, chatBody } from '../ai/openrouter.mjs';
...
export function buildRequestBody({ model, query, withTool }) {
  ...
  // chatBody stamps the no-retention routing and turns extended reasoning off.
  // Searching and listing what came back is not a puzzle, and on a model that
  // deliberates first that wait is paid on every web search and contact lookup.
  return chatBody(body);
}
