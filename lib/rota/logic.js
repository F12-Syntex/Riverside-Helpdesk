...
export function generateGrid(staff, mondayISO, minStaff = 2, seed = 0, history = null, prevGrid = null) {
  ...
  staff.forEach((s) => {
    ...
    if (s.temporary) {
      ...
      result[s.id] = [0, 1, 2, 3, 4].map((d) => (cons[s.id][d] === 'AL' ? 'AL' : (prev[d] === 'E' || prev[d] === 'L' ? prev[d] : null)));
    } else {
  ...
  for (let d = 0; d < 5; d++) {
    ...
    // permanent staff in `avail` can be moved to fix it.
    const onE = () => staff.filter((s) => result[s.id][d] === 'E').length;
    const onL = () => staff.filter((s) => result[s.id][d] === 'L').length;
...
export function buildWhatsApp(grid, staff, mondayISO, times) {
  ...
  const blocks = staff.map((s) => {
    ...
    if (s.temporary) {
      ...
      const worked = days.filter((_, di) => { const c = shiftAt(grid, s.id, di); return c === 'E' || c === 'L' || c === 'AL'; });
      if (!worked.length) { lines.push('* No days set this week'); return lines.join('\n'); }
      days.forEach((day, di) => {
        const c = shiftAt(grid, s.id, di);
        if (c === 'E' || c === 'L' || c === 'AL') lines.push('* ' + day.short + ': ' + dayHours(c, times));
      });
      return lines.join('\n');
    }
    days.forEach((day, di) => {
...
export function sanitiseGrid(raw, staff, mondayISO) {
  ...
  staff.forEach((s) => {
    ...
    out[s.id] = [0, 1, 2, 3, 4].map((d) => {
      ...
      if (s.temporary) return (v === 'E' || v === 'L') ? v : null;
