...
export const SHIFT_META = {
  E: { bg: '#212124', color: '#f0817c' },
  L: { bg: '#26262a', color: '#80eae0' },
  F: { bg: '#1c1c1f', color: '#9566e6' },
  AL: { bg: '#51442a', color: '#ebc984' },
  OFF: { bg: '#0b0b0c', color: '#74747d' },
};
...
export function weekRangeLabel(mondayISO) {
  ...
  return first.date.split(' ')[0] + ' to ' + last.date + ' ' + lastIso[0];
}
...
export function cellView(code, times) {
  ...
  return { bg: '#141416', color: '#74747d', main: '' };
}
...
function dayHours(code, times) {
  if (code === 'E') return ampm(times.E.start, false) + ' to ' + ampm(times.E.end, true);
  if (code === 'L') return ampm(times.L.start, false) + ' to ' + ampm(times.L.end, true);
  if (code === 'F') { const t = times.F || DEFAULT_TIMES.F; return ampm(t.start, false) + ' to ' + ampm(t.end, true); }
