...
export function buildWhatsApp(grid, staff, mondayISO, times) {
  ...
  const blocks = staff.map((s) => {
    const hrs = days.map((day, di) => dayHours(shiftAt(grid, s.id, di) || 'OFF', times));
    const lines = ['*' + firstName(s.name) + '*'];
    let start = 0;
    for (let d = 1; d <= 5; d++) {
      if (d === 5 || hrs[d] !== hrs[start]) {
        const end = d - 1;
        const label = start === end ? DSHORT[start] : DSHORT[start] + '–' + DSHORT[end];
        lines.push('* ' + label + ': ' + hrs[start]);
        start = d;
      }
    }
    return lines.join('\n');
