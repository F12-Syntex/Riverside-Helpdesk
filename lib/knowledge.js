...
export async function enqueueKnowledgeAnalysis(entryId, targetHash, delayMs = 2000) {
  await ensureKnowledgeSchema();
  const sql = getSql();
  const seconds = Math.max(0, Math.min(60, Number(delayMs) || 0)) / 1000;
  await sql`
...
async function fillMissingKnowledgeEmbeddings(entry) {
  // Notebook pages are supplied whole and are never retrieved by embedding.
  // Documents and contacts remain hybrid RAG sources.
...
export async function fillMissingKnowledgeEmbeddingsByKind(kind) {
  const targetKind = ['document', 'contact'].includes(kind) ? kind : '';
...
export async function syncKnowledgeClaims(entryOrId, { refresh = true } = {}) {
  ...
  const model = process.env.OPENROUTER_ANALYSIS_MODEL || process.env.OPENROUTER_AI_MODEL || '';
...
export async function unifiedContacts(query, limit = 5) {
  const hits = await searchKnowledge(query, Math.max(limit, 8), { kind: 'contact' });
