...
async function fillMissingKnowledgeEmbeddings(entry) {
  // Notebook pages are supplied whole to the prompt, so they are not embedded.
  // Documents and contacts are a hybrid corpus: /practice asks searchKnowledge
  // for documents with `semantic: true`, and a caller asking for "the district
  // nurses" has no title to pick from, so wording has to be matched
  // semantically. A document passage left without a vector silently drops out
  // of the semantic arm — the CTE filters on `embedding IS NOT NULL` — and the
  // search degrades to lexical-only with nothing to say so. So every document
  // added or edited since the last `rag:ingest` is embedded here, as it is
  // synced, rather than waiting for a re-ingest that may never come.
...
// Efficiently backfill one RAG corpus in batches: the contact directory when it
// was upgraded from lexical-only rows to hybrid retrieval, and the documents
// that were synced while document embedding was switched off. Notebook pages
// are never embedded (they go into the prompt whole), so `note` is refused.
export async function fillMissingKnowledgeEmbeddingsByKind(kind) {
  const targetKind = kind === 'contact' || kind === 'document' ? kind : '';
...
export async function syncKnowledgeClaims(entryOrId, { refresh = true } = {}) {
  ...
  // The cache key has to name the model that will actually do the extraction,
  // which is the fast role (lib/ai/claims.js). Keyed on anything else, changing
  // the extracting model silently serves claims the old one produced, while
  // changing an unrelated model throws away a cache that was still good.
...
/**
 * Archive many entries at once — the same thing as calling the singular above
 * for each, done in three statements instead of three per entry.
 *
 * Clearing a notebook to load a save over it went through the singular once per
 * page, and each of those refreshed the conflict table. Three hundred pages was
 * about a thousand round trips and three hundred conflict refreshes to delete
 * something. The refresh happens once here, at the end, because its input is
 * the whole table either way.
 */
export async function archiveKnowledgeEntries(ids = []) {
  const list = [...new Set((ids || []).map((id) => String(id || '')).filter(Boolean))];
  if (!list.length) return 0;
  await ensureKnowledgeSchema();
  const sql = getSql();
  await sql`UPDATE knowledge_entries SET status='archived',updated_at=now() WHERE id = ANY(${list})`;
  await sql`DELETE FROM knowledge_analysis_jobs WHERE entry_id = ANY(${list})`;
  await refreshConflicts();
  return list.length;
}
