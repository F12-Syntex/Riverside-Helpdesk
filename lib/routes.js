...
export const PAGES = [
  ...
  },
  {
    ...
    group: 'Everyday tools',
    summary:
      'The questions nobody has an answer for yet: the ones staff ask here because the practice has not written the answer down anywhere, and — added on their own, off the question log — the ones the assistant was asked and could not answer. An answer written here is readable the same afternoon; the Notebook page is still what makes it citable.',
  },

  // The practice's own telephone directory has no path of its own: it is the
  // Contacts pill in the header (app/_components/ContactsSheet.jsx), which
  // opens over whatever page asked for it. There is nothing here to list.
  ...
  },
  {
    ...
    group: 'Practice management',
    summary:
      'Every save of the whole Notebook, newest first. Take one, download it as JSON, import a JSON file as a save, or load one — which puts the Notebook back exactly as it was. A load saves the state it discards first, so the load itself can be rolled back.',
  },
  ...
  },
  {
    ...
    group: 'Reference and documentation',
    summary:
      'What the app is built out of, in nine figures: the layers from browser to third party, one turn end to end, the eight model roles, the thirty-one Postgres tables, the document corpus, every dependency, how it ships, where the code is, and what leaves the practice.',
  },
  {
    path: '/dpia',
  ...
  },
  {
    ...
    local: true,
    summary:
      'Everything the app has stored, read-only: every table in the database with its rows, columns, keys and indexes, and every file in the blob store beside them. No edit, no delete, no query box — the endpoint behind it builds nothing but SELECTs. Development machine only; the middleware returns 404 for this path everywhere else.',
  },
  {
    path: '/knowledge',
...
export const API_ROUTES = [
  {
    ...
    tool: 'Practice Q&A',
    summary:
      'The assistant’s only answer path. The whole Notebook goes in, the model names the page or template that fits, and the card is rendered in code from what the practice wrote. Streams its progress to the browser as newline-delimited JSON.',
  },
  {
    path: '/api/routing/learn',
    title: 'Teach the router a phrasing',
    methods: ['POST'],
    group: 'Answering questions',
    tool: 'Practice Q&A',
    summary:
      'A tap on a "which did you mean?" option, when the router asked: the wording and the page it meant are kept as a trigger phrase, so the next person who says it that way gets the page straight away.',
  },
  {
    path: '/api/kb',
  ...
  {
    path: '/api/database',
    title: 'Database explorer (read-only)',
    methods: ['GET'],
    group: 'Administration',
    tool: 'database',
    summary:
      'Reads the catalogue, a page of rows from one table, one row in full, or the contents of the blob store. Table and column names are matched against the live catalogue before they reach a statement, values are always parameters, and no branch writes anything.',
  },
  {
  ...
  {
    path: '/api/notebook/map',
    title: 'Notebook map',
    methods: ['GET'],
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'Every page with its health: the rules the page breaks, whether the assistant’s parsers can read it, and what the question log says about answers built from it. Read-only, no model.',
  },
  {
    path: '/api/notebook/defrag',
    title: 'Notebook rewrite',
    methods: ['GET', 'POST'],
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'Propose a rewrite of one page, re-check an edited proposal, or apply one. A proposal is validated in code — nothing lost, nothing invented, names and numbers verbatim, the page still parses — and judged by a second model call for unchanged meaning. Apply snapshots the page first.',
  },
  {
    path: '/api/notebook/defrag/run',
    title: 'Notebook defragmentation run',
    methods: ['GET', 'POST'],
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'Defragment the whole Notebook in one pass. Before any page is rewritten, every page is read against every other and the pairs that disagree — two addresses for one service, two waiting times for one pathway, an instruction and its opposite — are put to a second model and flagged. A page named in a flag nobody has settled is not rewritten: the run waits. The rest are proposed one page at a time and wait in a review list, each with the same code checks and meaning check a single rewrite gets.',
  },
  {
    path: '/api/notebook/revert',
    title: 'Notebook revert',
    methods: ['POST'],
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'Put a page back to an earlier revision. The state being left is snapshotted first, so a revert can itself be undone.',
  },
  {
  ...
  },
  {
    ...
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'List the saves of the whole Notebook, take a new one, download one as JSON, or delete one. A save holds every page and attachment record in the same shape the backup download writes.',
  },
  {
    path: '/api/notebook/snapshots/load',
    title: 'Notebook rollback',
    methods: ['POST'],
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'Replaces the Notebook with what a save held. The state being left is saved first, so a load can itself be rolled back.',
  },
  {
    path: '/api/notebook/snapshots/import',
    title: 'Notebook save import',
    methods: ['POST'],
    group: 'Practice management',
    tool: 'Notebook',
    summary: 'Stores an uploaded backup file as a save. The Notebook is untouched until that save is loaded.',
  },
  ...
  {
    path: '/api/questions/open',
    title: 'Open questions',
    methods: ['GET', 'POST', 'PATCH', 'DELETE'],
    group: 'Answering questions',
    tool: 'Questions',
    summary:
      'The list behind /questions: read it, ask a question of your own, write the answer to one, or remove a row that should not be there. The other writer is the question log, which files every turn the assistant could not answer.',
  },
  {
